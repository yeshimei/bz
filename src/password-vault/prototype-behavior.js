/* 构建产物（勿手改）：node scripts/build-preview.mjs — src/password-vault/fake-sim.ts → window.BZW_password_vault（行为单源预览包，issue 245/ADR-0106） */
var BZW_password_vault = (() => {
  ...
  // src/password-vault/fake-sim.ts
  ...
  // src/password-vault/fake/fake-obsidian.ts
  var Platform = {
  ...
  var _UIManager = class _UIManager {
    ...
    renderDesktop() {
      ...
      if (this.asset === "overview") {
        ...
        (_b = area.querySelector('[data-hero="lock-note"]')) == null ? void 0 : _b.addEventListener("click", () => {
          var _a2;
          return (_a2 = this.onLockCurrentNote) == null ? void 0 : _a2.call(this);
        });
  ...
  var PasswordVaultUIManager = class {
    ...
    renderLock() {
      ...
      if (unlocked) return;
      const first = !this.dataManager.unlocked;
    }
    ...
    async handleAccountAction(d, act, which) {
    ...
    showLock() {
      void this.isFirstTime().then((firstTime) => {
        this.root.querySelectorAll(".bz-password-vault-lock").forEach((lockEl) => {
          ...
          const p1 = lock.querySelector("[data-lock-p1]");
          const p2 = lock.querySelector("[data-lock-p2]");
          const go = lock.querySelector("[data-lock-go]");
          const err = lock.querySelector("[data-lock-err]");
