...
async function autoMarkReading(id: string): Promise<void> {
  ...
  renderRail();
  renderReader();
  renderMobList(); // issue 224：移动列表同刷（落在读后让位重排需重绘）
}
...
function renderMobList(): void {
  if (!mobListEl) return;
  // issue 224：未读在前、组内最新在前（与桌面目录同序）——原时间序让落在读的条目
  // 永远钉在原位，用户感知「变为在读但不往下排」
  const list = sortedView();
  if (!list.length) {
    mobListEl.innerHTML = '';
    ...
    groups: [
      {
        ...
        name: '外观',
        rows: [
          { type: 'choiceCards', name: '布局', binding: { key: 'clipbookSkin' }, options: [{ value: 'default', label: '编辑部', prevClass: 'bz-sp-prev-panel' }] },
          { type: 'choiceCards', name: '主题', binding: { key: 'clipbookSkinTheme' }, layoutKey: 'clipbookSkin', options: [{ value: 'newsprint', label: '新闻纸', layout: 'default', prevClass: 'bz-sp-prev-newsprint' }] },
        ],
      },
      {
        icon: 'folder-open',
