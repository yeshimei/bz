...
const RESIDUAL_EPS = 1; // 与测试同口径：互穿 ≤ 1px 视为「贴合」
const RESIDUAL_GAP = 1; // 兜底右移后留 1px 缝
const RESIDUAL_ROUNDS = 30; // 残重叠先做 N 轮最小穿透轴微调（0 余量）
...
/** 视口裁剪接口：只对与视口相交的节点施力/施约束，其余保持种子位（仍计入归一化与世界尺寸） */
 ...
 * - 收尾 60 轮最小穿透轴硬推开 → 任意两盒不重叠；
 * - 超过 `opts.maxNodes`（默认 120）：只布局前 N 个，其余进 `culled`（无坐标）；
 * - `opts.viewport`：只对与视口相交（按种子位置判定）的节点施力，其余保持种子位（默认 null = 全量）。
 */
