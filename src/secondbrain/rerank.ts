...
/** 整列重排的条数上限：取设置项上限（「参考结果数 TopK」/「对话参考结果数」min 1 max 50，
 *  `panel.ts` number 行），保证面板任何合法 TopK 下**整列都进重排**、百分比与名次同尺
 *  （issue 429 诉求：相关度高的排上面；若上限小于 TopK，尾部条目只有余弦分、混排观感会复现）。
 *  超长列表（建链候选池等后台链路，最大 150）在 `applyRerank` 里整体跳过重排——不半重排，
 *  也不把交互检索的 6s 预算耗在无人排名的后台列表上。
 *  每周撞车（10）与面板 TopK 本就在限内。 */
