 ...
 * - ticket 113：初始向量化进行中关页重开 → 恢复实时进度视图（原缺陷：重开回引导态且按钮
...
export class SecondBrainPanel {
  ...
  /** 初始向量化视图进行中标记（ticket 113：runInitialIndexView 持有；进行中重复点击接回进度视图而非静默失效） */
   ...
   *  就绪 + 待处理 → 增量进度；就绪无变更 → 统计（ticket 113 补「空库但 refresh 在途」分支） */
  ...
  /**
   * 初始向量化运行器（ticket 113 自按钮处理器抽出共用）：进入进度视图并接住 refresh 实时进度，
  ...
  /**
   * 引导按钮（ticket 107/108；ticket 113 修「点了没反应」）：首次全量向量化。
