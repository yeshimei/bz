...
export const CHANGELOG_META = { generatedAt: '2026-09-26', total: 1313 } as const;
...
export const CHANGELOG_DOMAINS: ChangelogDomainData[] = [
  { id: 'diary', name: '日记本', entries: [
    ...
    { date: '2026-08-21', type: 'fix', text: '日记加密分类 交互收敛——去掉加密卡片时间后的🔐角标与只读预览弹窗（正文即预览，点击无操作），还原 merge 修复双时间标题与标题正文间缺空行，加密标签固定排筛选栏最后（count 排序同样），+回归测试，1014 测试全绿' },
    ...
    { date: '2026-08-12', type: 'feat', text: '黑匣子默认类型筛选设置项完成——blackboxDefaultTypeFilter（空=全部/概念/文献/想法，重启生效，与 diaryDefaultSelectedTag 同模式）全链路：MemoSettings+DEFAULT_SETTINGS、⚙️ 弹窗下拉、面板消费（ticket 46 已接），1 测试' },
  ...
  { id: 'memo', name: '备忘录', entries: [
    ...
    { date: '2026-08-11', type: 'fix', text: '黑匣子 UI 布局对齐备忘录——① 面板右上角动作区加 ✏️ 录入（⚙️ 设置之后 ❌ 关闭恒在最后）② 录入弹窗保存/取消从底部 footer 移右上角（💾 保存 + ❌，删除 footer）③ 对话弹窗头部统一（发送仍与输入框联动）④ 三弹窗头部改备忘录风格（18px 标题/无边框图标按钮 hover 背景）+ slideUp 入场动画，3 测试' },
  ...
  { id: 'clipping', name: '剪藏本', entries: [
    ...
    { date: '2026-08-23', type: 'fix', text: '聚合讯/阅读报告移动端默认全屏跟随剪藏本/书库，移除两窗口 ⚙️ 设置入口' },
    ...
    { date: '2026-08-22', type: 'fix', text: '主窗口头部行全页面统一——统一类 .bz-win-head 补 7 窗（password/review/belongings/clipping/movie 主/影视分析/阅读报告）+ 既有头部类并入同组：头行 16/24/10（头部↔列表间距 10px 统一）间距 8；头行按钮统一 22×26/14px 透明无阴影无边框圆角 4 text-muted hover 统一背景（news 浮动钮/番茄钟 ⚙️ 同规格、movie 分析 0.55rem 与 belongings 无 hover 收敛、冗余规则清理），ADR-0019 修订第 3 条，1075 测试全绿' },
    { date: '2026-08-22', type: 'fix', text: '头部按钮秩序与关闭钮——⚙️ 置于 ❌ 正前（favorites/encrypt/clipping 换序）+ 关闭按钮再小 2px（20×24/12px，新类 .bz-win-close 挂 7 处自定义关闭钮，icon-btn/--close、todo-btn-close、news-close-btn 并入）+ 移动端非真全屏（常规卡）隐藏关闭按钮（卡片态靠点遮罩关闭、全屏靠 ❌，桌面不受影响），ADR-0019 修订第 4 条，1075 测试全绿' },
  ...
  { id: 'favorites', name: '收藏本', entries: [
    ...
    { date: '2026-09-06', type: 'fix', text: '图标挂载补全+窄屏适配——renderTags/头行关闭钮 mountIcons 补挂（新收藏+/移动✕此前永不渲染），表单 min(430px,100vw-32px) 留边，面板 container query 窄于 520px 卡墙双列平摊' },
  ...
  { id: 'bookshelf', name: '书库', entries: [
    ...
    { date: '2026-09-11', type: 'feat', text: '借书卡详情改版——去✕钮/卡内分区滚动/印章随内容/在读可继续' },
  ...
  { id: 'review', name: '复习计划', entries: [
    ...
    { date: '2026-09-09', type: 'fix', text: '254 迭代批——移动端全屏/头行精简/搜索退役/冲刺✕直退/空库自动重播' },
    ...
    { date: '2026-08-09', type: 'fix', text: '通知系统修订——显式指定类型（删 emoji 自动归类）、类型图标 emoji（ℹ️/✅/⚠️/❌）、桌面右侧弹出/移动端顶部居中、dedupe 同键存活期单框合并（可切类型+重置计时）、连续任务常驻单框（复习循环/批量出题/自动摘要），788 测试' },
  ...
  { id: 'secondbrain', name: '第二大脑', entries: [
    ...
    { date: '2026-08-31', type: 'feat', text: '已有 related 不再触发自动双链（ticket 167）——新增 linkAgentRespectRelated 开关（默认开），创建/修改/队列消费三条自动路径对 related 非空笔记跳过（skipped-related，队列条目顺带移除）；手动命令 bz-secondbrain-rebuild-links 传 respectRelated:false 豁免；数据层 hasRelatedEntries 统一判定出口；⚙️ 弹窗新增 toggle「已有关联不再建链」；spec v1.7 节 + issues/167 + PROGRESS' },
  ...
  { id: 'knowledge', name: '知识盒', entries: [
    ...
    { date: '2026-08-30', type: 'feat', text: '主面板列表间距+相对日期，视频录入批量按钮单钮态机（ticket 146，原 144 与并行 UX 批次撞号重编）——卡片简介/日期间距分档加大（6→10/12px）且日期改 formatRelativeTime 相对显示（无效回退原文/空不显示）；去独立 ⏹ 改单钮（空闲▶️批量处理无工作禁用↔运行中⏹终止/仅失败项续跑⏹终止整批，runAll 启动即刷新态），移动端整钮隐藏（.bz-lit-run-btn 文本钮）；测试改写+新增三用例；spec/PROGRESS 含 145+146 文档节' },
    { date: '2026-08-30', type: 'feat', text: '视频录入批量按钮纯 emoji（ticket 148，用户拍板）——▶️/⏹ 去文字，终止 vs 终止整批区分移 title hover（中止批量处理 / 中止整批）；batchAbortLabel 逻辑与 #lit-btn-video-run 契约不动；用例断言改纯 emoji + 补 title 断言；issues/148 + spec/PROGRESS 记录 147/148' },
  ...
  { id: 'pomodoro', name: '番茄钟', entries: [
    ...
    { date: '2026-08-10', type: 'feat', text: '番茄钟 设置完成——BzSettings +9 项（预设/自定义时长/N/四开关）+ ⚙️ 域设置弹窗（12 档预设下拉、自定义时长动态显隐、变更即保存）+ durations/options 读设置即时生效（非法值回退默认），13 测试' },
    { date: '2026-08-10', type: 'fix', text: '番茄钟 弹窗层级——mask z-index 10200→9998（域主弹窗层级，password 先例），低于 Obsidian 设置页与域设置弹窗 10030，⚙️ 可正常覆盖，1 测试' },
  ...
  { id: 'smartcat', name: '小橘陪伴猫', entries: [
    ...
    { date: '2026-08-30', type: 'feat', text: '三层记忆流水线 + 巩固参数设置面板（ticket 160）——日小结产出 observation 入记忆流接通管道、反思只吃记忆流观察并对引用条目贴原文摘录、周报只吃本周新增洞察（旧统计字段退役）、shouldReflect 改间隔+素材双闸（删 158 合并池 behaviorToObservations 与 20 条快车道，素材计数重定义只随记忆流新增）；11 个巩固参数上 ⚙️「记忆巩固」组（smartcatReflect*/smartcatDigest*/smartcatWeeklyMinInsights/smartcatRefExcerptLimit，getConsolidationConfig 统一读取）；memory/report/adr0069/index-cov/settings 测试改写+新增；ADR-0075 + CONTEXT 三层记忆流水线词条 + spec/PROGRESS/issues/160' },
    ...
    { date: '2026-08-30', type: 'feat', text: '洞察条数上限 + 来源分布按追查目录 + 小橘对我的称呼（ticket 163）——新增 smartcatReflectMaxInsights（默认 3，⚙️记忆巩固组可调，反思 prompt 声明最多 N 条 + LLM 返回按序截断兜底）与 smartcatUserName（默认包仔，⚙️互动组可调，replaceUserReference 把记忆流/行为流喂 AI 的「你/你们/用户」替换为称呼，不写盘存储冻结，模板/人物设定句不替换）；dashboard 记忆来源分布：洞察（含周报）单列「洞察」行 + source=note 引用条目按记忆目录追查目录分行（resolveTrackedDirLabel，未命中回退记忆目录）+ 行为小结行保留，最近记忆列表 note 行同口径；memory/report/companion-context/mood 六处内容替换接入；issues/163 + spec/PROGRESS/CONTEXT + ADR-0076' },
  ...
  { id: 'encrypt', name: '保险库', entries: [
    ...
    { date: '2026-09-20', type: 'fix', text: '主线程合并收口——还原目录注入改 diary 域 DIARY_DIRECTORY 真源（D9 回归修复）、indigo 类替换内联、closeAllDialogs 收场 cancelActiveFlowDialog、搜索✕样式收编、secondbrain fake 层补 TFile 导出修 behavior 包构建' },
    ...
    { date: '2026-08-21', type: 'feat', text: '加密保险箱 顶部加「加密当前笔记」按钮（🔒 置于 ⚙️ 设置前，点击调 lockCurrentNote 弹二次确认），Controller 注入 onLockCurrentNote 回调；+1 测试；998 测试全绿' },
  ...
  { id: 'settings-panel', name: '通用与设置', entries: [
    ...
    { date: '2026-08-12', type: 'feat', text: '黑匣子主面板微调完成——标题只显示「黑匣子」（无 emoji 无条数，DOM id 保留）+ header 新增 🔍（⚙️设置 前，显示高亮）切换搜索框显隐 + 默认隐藏/每次打开复位 + 隐藏即清空关键词立即重渲染（防抖前也生效），1086 测试' },
    ...
    { date: '2026-08-11', type: 'feat', text: '38 黑匣子 完成——新域：感触录入（素材+感受+24词情绪+可选维度+AI辅助查概念/联想/追问）、中央弹窗对话（三层记忆：TF-IDF感触检索+人格档案+对话历史）、静默复盘生长（阈值10条自动+手动，产物公开写入对话）、包仔种子人设（有诗心的思辨者）、⚙️ 设置5项、ADR-0013/0014，48 测试' },
    ...
    { date: '2026-08-08', type: 'feat', text: '设置归属模型完成——ADR-0009 全局设置页单页化（AI+共享 storagePath）+ 10 域 ⚙️ 设置弹窗 + 筛选/排序统一 🔀 + storagePath 迁移（旧 7 字段兼容保留）+ AI Agent 设置不暴露，752 测试' },
    { date: '2026-08-08', type: 'fix', text: '设置弹窗 z-index 10030（盖过主面板 9999/面板内弹窗 10005/闪念窄窗 10020）+ 各面板 ⚙️ 统一紧挨 ❌ 前，752 测试' },
