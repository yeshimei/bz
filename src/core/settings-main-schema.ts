 ...
 * - issue 330 重新分组：「AI 与凭据」单组（ADR-0133）拆为「服务商」「模型配置」「数据源凭据」
 ...
 * issue 330 起归「模型配置」组——refreshKey 联动链是全 schema 级的，跨组不受影响。
 */
...
/**
 * 「服务商」组行（issue 330 拆组）：服务商下拉 + 每家注册表提供商一行密钥（visibleWhen 随
...
/** 「模型配置」组行（issue 330 拆组）：ticket 172 per-provider 三行——模型行 custom
 *  （内嵌「获取模型名」按钮）+ 上下文/最大输出 token 标准 number 行
 *  （refreshKey 随服务商切换联动，跨组生效）。 */
function modelGroupRows(): SettingsRow[] {
  return [providerModelCustomRow(), providerNumberConfigRow('context'), providerNumberConfigRow('maxTokens')];
}

/**
 * 「数据源凭据」组行（issue 330 拆组，ADR-0133「AI 与凭据」单组退役）：与 AI 服务商无关的
