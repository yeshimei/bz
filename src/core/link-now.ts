/**
 * 自动双链通道（issue 309）：知识盒录入面板 ←→ 第二大脑 link agent 的唯一接线口。
 *
 ...
 * 未注入（第二大脑未启用 / 自动双链关闭 / 测试与原型未接线）→ getLinkBridge() 返回 null，
 * 调用方按「自动双链未开启」呈现，不得报错、不得阻塞录入本身。
 */
...
export interface LinkBridge {
  /** 单篇即时建链（无预演结果时的兜底：完整跑一遍检索 + 裁判 + 写入） */
  now(path: string): Promise<LinkNowOutcome>;
