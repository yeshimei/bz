...
export class MemoSuiteSettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    const tabs: { id: string; label: string; build: (el: HTMLElement) => void }[] = [
      ...
      { id: 'memo', label: '备忘录', build: (el) => this.buildMemoTab(el, s, save) },
    ...
    const header = containerEl.createDiv({ cls: 'memo-suite-setting-header' });
    const tabGroup = header.createDiv({ cls: 'memo-suite-tab-group' });
    const contentWrap = containerEl.createDiv({ cls: 'memo-suite-setting-content' });

    const buttons: HTMLElement[] = [];
    const contents: HTMLElement[] = [];
    for (const t of tabs) {
      const btn = tabGroup.createDiv({ cls: 'memo-suite-tab', text: t.label });
      const content = contentWrap.createDiv({ cls: 'memo-suite-tab-content' });
      t.build(content);
      btn.addEventListener('click', () => {
        buttons.forEach((b, i) => b.toggleClass('memo-suite-tab-active', b === btn));
        contents.forEach((c, i) => c.toggleClass('memo-suite-tab-content-active', c === content));
      });
      buttons.push(btn);
      contents.push(content);
    }
    // 默认显示第一个 tab（AI）
    buttons[0].addClass('memo-suite-tab-active');
    contents[0].addClass('memo-suite-tab-content-active');
  }

  // ===== AI =====（服务商下拉 + 动态显示对应 API Key）
  private buildAiTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
    let deepseekRow: Setting | null = null;
    let opencodeRow: Setting | null = null;
    const refreshKeys = () => {
      const isDeepseek = s.aiProvider === 'deepseek';
      if (deepseekRow) deepseekRow.settingEl.toggleClass('memo-suite-setting-hidden', !isDeepseek);
      if (opencodeRow) opencodeRow.settingEl.toggleClass('memo-suite-setting-hidden', isDeepseek);
    };
    new Setting(el)
      .setName('AI 服务商')
  ...
  private buildMemoTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
  ...
  private buildDiaryTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
  ...
  // ===== 归物本 =====（自定义分类设置已移除）
  private buildBelongingsTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
  ...
  private buildClippingTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
    this.textSetting(el, '剪藏目录', '存放网页剪藏 markdown 文件的文件夹', s.articleDirectory, save, (v) => (s.articleDirectory = v));
  }

  // ===== 密码本 =====
  private buildPasswordTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
  ...
  // ===== 收藏本 =====
  private buildFavoritesTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
    this.textSetting(el, '数据存储路径', '存放 favorites.json 的路径', s.favoritesStoragePath, save, (v) => (s.favoritesStoragePath = v));
  }

  // ===== 书库 =====
  private buildLibraryTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
  ...
  // ===== 影视 =====（每页加载数量/海报整理设置已移除）
  private buildMovieTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
    this.textSetting(el, '影视文件夹', '存放影视笔记的文件夹路径', s.movieFolderPath, save, (v) => (s.movieFolderPath = v));
  }

  // ===== 做题家 =====（题目难度为下拉选择）
  private buildQuizTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
  ...
  // ===== 复习计划 =====
  private buildReviewTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
  ...
  // ===== 闪念 =====
  private buildFlashTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
  ...
  // ===== 自动摘要 =====（常驻监听新文件，懒加载开关）
  private buildAutoSummaryTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
  ...
  // ===== AI Agent =====（笔记同步，懒加载开关）
  private buildAIAgentTab(el: HTMLElement, s: MemoSettings, save: () => Promise<void>) {
