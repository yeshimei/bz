...
import { syncSkinPack, loadLocalSkinPack } from './core/skin-pack';
...
import BzSettings, { DEFAULT_SETTINGS, migrateMemoSettingKeys, migrateAutoLinkSettings, migrateRetiredAIKeys, migrateRetiredFavoritesSortKey, migrateLinkMinScoreScale, migrateRetiredSecondBrainKeys, migrateRetiredJevKeys, migrateAsrKeys, migrateExternalToolKeys } from './settings';
...
export default class BzPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.app.workspace.onLayoutReady(() => {
      ...
      // 皮肤包本地缓存（ADR-0199）：**不等**下面那 15 秒——先用已下载的皮肤起效，
  ...
  async onunload() {
    ...
    closeAllModals();
    // 自建 overlay 存活收口（CB10/A1）：UP/RSS 管理等 createOverlay 弹窗的登记表统一关——
    // closeAllModals 只管 uiModal 面，自建遮罩在此兜底（未打开时 no-op）
    closeAllOverlays();
