 ...
 * 头部标题旁状态计数（待处理/处理中/失败）；
 * 桌面端头部功能区「➕ 添加 / ⬇️ 下载 / ▶️ 处理 / ⏹ 中止 / 🕘 历史 → ⚙️ → ✕」；
 * 移动端仅暂存录入（无处理/下载按钮）。
 * 重试不设行内入口：批量处理范围本就含待处理+失败，失败后再次点 ▶️ 即重试（ADR-0070）。
...
/** 步骤完成态文案映射（工具步骤名 → 「已…」；无映射的「XX中」按通用规则去「中」加「已」，ADR-0070） */
...
export class UIManager {
  ...
  constructor(app: App, hooks?: UIManagerHooks) {
    ...
    this.onKeydown = (e: KeyboardEvent) => {
      ...
      // ESC 关最上层：历史弹窗 → 添加弹窗 → 主面板（各层显隐独立，ADR-0070）
      if (this.historyPopup && this.historyPopup.style.display === 'flex') this.hideHistory();
  ...
  createMainUI(): void {
    ...
    header.innerHTML = `
      <div style="display:flex;gap:8px;align-items:baseline;min-width:0;">
        ...
        <span id="bili-status-counts" class="bz-bili-counts"></span>
      </div>
      <div style="display:flex;gap:6px;align-items:center;">
        ...
        <button id="bili-btn-history" title="历史" style="width:30px;height:30px;">🕘</button>
  ...
  createAddDialog(): void {
    ...
    addMask.id = 'bili-add-mask';
    Object.assign(addMask.style, { position: 'fixed', inset: '0', background: 'rgba(0,0,0,.45)', display: 'none' });
  ...
  createHistoryUI(): void {
    const mask = document.createElement('div');
    mask.id = 'bili-history-mask';
    Object.assign(mask.style, { position: 'fixed', inset: '0', background: 'rgba(0,0,0,.45)', display: 'none' });
    mask.onclick = () => this.hideHistory();
    const popup = document.createElement('div');
    popup.id = 'bili-history-popup';
    Object.assign(popup.style, {
      position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)',
      width: 'min(720px, 94vw)', maxHeight: '86vh', display: 'none',
      flexDirection: 'column', background: 'var(--background-primary)', color: 'var(--text-normal)',
    ...
    header.className = 'bz-win-head';
    header.innerHTML = `
      <h3 style="margin:0;font-size:15px;font-weight:600;">文献盒 · 历史</h3>
      <div style="display:flex;gap:6px;align-items:center;">
        <button id="bili-history-close" class="bz-win-close" style="width:30px;height:30px;">✕</button>
      </div>`;
    const list = document.createElement('div');
    list.id = 'bili-history-list';
    list.style.cssText = 'flex:1;overflow-y:auto;padding:12px 16px;min-height:220px;';
    popup.appendChild(header);
    popup.appendChild(list);
    document.body.appendChild(mask);
    ...
    q<HTMLButtonElement>(popup, '#bili-history-close')!.onclick = () => this.hideHistory();
  }
  ...
  private renderRow(task: LiteratureTask): HTMLElement {
    ...
    // ADR-0067：解析到信息后（title 非空）链接行改「文字 + 链接」——标题即入口，点击浏览器打开；
    // 未解析显示短链接（BV 号/短码优先，ADR-0070），完整链接见悬浮 title
  ...
  private updateRowProgress(id: string): void {
    ...
    // 百分比/进度条**仅「下载中」显示**（ADR-0067 拍板：除下载外其余阶段不显示百分比，避免无意义数字跳动）
  ...
  /** 清空历史（⚙️ 设置面板入口，ADR-0070）：确认后移除全部归档记录 */  private async confirmClearHistory(): Promise<void> {
    const v = await openFlowDialog({
      title: '清空历史？',
  ...
  destroy(): void {
    ...
    this.historyMask?.remove();
    this.historyPopup?.remove();
    this.mask = null;
    this.popup = null;
    this.list = null;
    this.addMask = null;
