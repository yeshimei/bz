# 364 · 全域裁剪批：死代码 / review 评级四命令 / recap 退役 / encrypt 密码视图摘除
...
> 用户裁决：**26 个外观占位键不动（ADR-0159），其余全部采纳，一起修复**。
> 分支 `chore/global-trim-batch`（worktree `../.dsh-worktrees/global-trim`）。

## 一、用户裁决

**保留不裁**：26 个外观占位设置键（issue 246 有意设计，皮肤扩展预留，ADR-0159）。

**采纳裁剪**：死代码全档、review 评级四命令、recap 面板（ADR-0157）、encrypt 密码
资产视图 + 快速取密统一（ADR-0158）、vault 数据残留归档。

**后续演进（合并时点）**：#5 评级四命令在并行批次 issue 362（做题家独立面板）中被
重新起用为评级动作依赖，本批裁剪案对这四条**撤回**（命令恢复，见 main.ts 复习组注释）；
其余各项不受影响。
...
| 5 | ~~review 评级四命令裁剪~~（**撤回**） | 原案删 `bz-review-again/hard/good/easy`（QuickAdd 热键时代遗产不可达）；并行批次 issue 362 做题家独立面板起依赖评级动作，四条命令恢复保留，难度弹窗仍为无热键时的面板外评级入口 |
| 6 | recap 面板退役 | 删 `bz-recap-today` 命令与面板 UI；「生成今日总结」并入 home 今日摘要卡；`collectRecap` 等纯函数库保留供 home 消费（ADR-0157） |
| 7 | encrypt 密码视图摘除 | `vault-pw-view.ts`/`vault-data.ts` 退役，encrypt 面板收敛两资产；共享锁与 SafeManager 不动（ADR-0158） |
