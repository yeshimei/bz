...
> 上游：349 拍板票（已闭）、348 识图研究票（已闭）、367 账单导出研究票（CSV 通道细节）
...
- **CSV 批量回补**（2026-09-17 收窄：**微信 only**，用户拍板支付宝没必要；细节以研究票 367 为准）：微信「下载账单」导出的 CSV 纯本地解析（跳对账单表头/汇总行），批量进确认列表；分类缺失可 AI 批量建议（可选，本地解析零隐私外流）。
- **手记（兜底）**：composer 式快速录入。

## 实现口径

- 数据 `CONFIG/STORAGE/ledger.json`：条目 `{id, amount(整数分), direction: income|expense, categoryId, date, merchant?, note?, source: manual|photo|csv, created}`；零迁移首版直建。
