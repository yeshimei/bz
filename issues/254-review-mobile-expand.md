...
## 追加迭代批（2026-09-09，用户逐项拍板）

1. **移动端全屏**：`#review-popup` 钉满视口（原 `max-height:100%` 短内容缩成悬浮卡）；
2. **头行精简**：⚙设置钮两端退役（甩 settings-panel 动态 import，行为包 −3 万行）；✕ 桌面隐藏
   （≥769px 域内规则、锁 `#review-popup` 作用域；点遮罩/ESC 关），移动保留；
3. **搜索框整链退役**：markup+防抖+过滤分支+死样式，`queueViewHtml` 签名瘦身为 `items+ctx`；
4. **归档条**只留绿点+「已完成复习」；归档态左下角变「返回队列」提醒钮（undo-2 入表，点击即回）；
5. **冲刺 ✕ 一击直退**：「放弃本次做题？」确认弹窗退役（ESC 路径保留确认）；
6. **演示壳播种**：review.json/quiz.json 空缺**或空数据**均重播（治无种子启动落空态卡死幂等播种）；
7. 冲刺右栏条目标题换行不截断（原 nowrap+省略号）。

门禁：全量 4199/4199 + tsc 绿；selftest 45/45（http/file 双跑）；主构建部署
（f5946cb9；`main.js` 产物含并行会话在途源码未随批提交）。
