...
- 状态：已交付（`mount-data.ts`：`findSameNameNote` 同名对齐（顺序无关）、`readMounts`／`writeMount`／`removeMount` 纯字符串 frontmatter 读写、`readRelated`、`buildMountTree` 三源/可达/非回指/严格跨代/方向翻转、`refCounts`／`orphanCards`；这些用例与 314 同在 `tests/knowledge/mount-data.test.ts`（41 用例））
