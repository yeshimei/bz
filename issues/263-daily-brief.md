...
- 触发：守护 spawn 完成后落「待出稿」标记（或插件装载时扫描 `briefs` 中 `body` 为空且无 `error` 的条目）→ 插件读转录稿 → `createAI()`（沿用插件已配 provider）→ 产**要点列表（`## 小节` 分组，每条要点一句话、30 字以内）** → 写回 `briefs[].body` → 清标记。
- Prompt 口径：输入 = 视频标题 + UP 主 + 时长 + 转写全文；输出 = **只一句话**（不分段、不加标题与列表符号）；**不要时间轴**；60 字以内。
...
- 右栏阅读区：要点分节（小节 + bullets）+ 顶部「打开原视频」与视频元信息（时长 / 发布时间）+ 可展开转录稿（细节兜底）。
...
- 场景 C：有新视频且**有字幕** → 条目 `src: 'subtitle'`，正文要点非空（含 `##` 分组与 `- ` 条目）。
...
## 产出形态修订（2026-09-10 两轮实读后定稿）

**最终形态 = 按小节分组的要点列表，每条要点一句话。**

第一轮（14:02 左右）：用户打开 剪藏本 → 每日简报 实读初版，反馈「总结的太长了，我需要一句话的总结」。
当时误判为「整篇只输出一句话」，把形态改成了单行自然句（`cleanPoints` → `cleanSummary` 并压平）。
第二轮（14:09）：用户澄清**「我说的是每个要点一句话」**——即要点列表结构要保留，只是每条要点要短。
故回退为：保留 `## 小节` 分组与 `- ` 要点行，**只把每条要点压成一句话（30 字以内）**。

- `buildBriefPrompt` 改为：`## 小节标题` 分组、2–4 组 × 2–4 条、**每条要点只写一句话且 30 字以内**、
  短句直给结论不堆信息、不要时间轴、不要前后解释。
- `cleanPoints` 恢复原名与实现（只去代码围栏 + 首尾空白，**保留行结构**——`## `/`- ` 是渲染层 `briefPointsHtml` 的输入契约）。
- 阅读面文案恢复「要点」口径：「正在生成本期要点…」「本期要点已重新生成」「要点生成失败：…」。
- ADR-0119 §6/§7 与 CONTEXT.md「每日简报」「简报条目」两词条同步改写；「整篇一句话总结」列入 _Avoid_ 与 Considered Options。
- 细节由阅读面「完整转录稿」折叠区兜底（缓存保留期内可展开）。
- 顺带补 `src/checkup/checks-drift.ts` 的 `SEGMENT_FIELDS['news.json']`：初版遗漏 `briefUps`/`briefs` 两段，会让体检报告误报段漂移。

### 实现期两个环境事实（避免下次踩坑）

- **`deepseek-v4-flash` 是推理模型，思维链会随机吃光配额 —— 已修**：它先输出 `reasoning_content`，
  且 reasoning token **计入 `max_tokens`**。实测**同一条视频时好时坏**：一次 8192 全被思维链吃光
  （`reasoning_tokens: 8192`、`finish_reason='length'`、正文 `content` 为空，思维链写到 18832 字符仍未收敛），
  另一次 reasoning 只花 ~2000、正文正常——属**不确定性失败**，最难排查。插件 deepseek provider
  `defaultMaxTokens: 8192` 并不足以兜住，故真实链路同样会空产出。
  **修复**：`brief.ts` 首次调用显式带 `reasoning_effort:'none'`（实测可把 reasoning 压到 0；
  `thinking:{type:'disabled'}` 同样有效但兼容面窄）；参数不被 provider 接受（抛错）或产出仍为空时，
  **去掉参数兜底重试一次**；两次都空才写可见错误条目。
  ⚠️ 这是 **`core/ai.ts` 的共性风险**——自动摘要 / 第二大脑走同一个 `createAI()`，遇到「需要长思考」的 prompt
  同样可能随机空产出；本期只在简报链路做规避，未动共享层（改动面过大），建议另开 issue。
- **whisper-small 的误听会在短要点里更显眼**：要点句短、专名密度高，`Lipstick`（DeepSeek）、
  `Mistro`（Mistral）、`Firmace`（疑为某算力厂商）这类误听比在长文里更容易被当成事实。属 bili-dl 共享转写链路
  （文献盒同用），未擅自改；对策见「待定项」（升模型 / `initial_prompt` 术语提示）。

## 待定项（实现时确认）

- **要点条数上限**：现 prompt 只约束「2–4 组 × 每组 2–4 条」，但实测模型在信息密集视频上会突破——
  首条 173 秒的 AI 日报出了 **3 组 17 条**（另两条 7 条 / 13 条）。若嫌多，可在 prompt 加「全篇要点总数不超过 N 条」硬上限
  （并补一句「宁可精、不要多」）；**未擅自加**，因为用户只提「每条要点一句话」、未对条数表态，怕误砍信息。
...
- 是否给 whisper 转写加 `initial_prompt`：本机 faster-whisper 输出**繁体中文**（加「以下是普通话的句子。」可显著转简体），
  且 AI 类专名误听明显（`Lipstick`←DeepSeek、`Mistro`←Mistral），提示里可补常见产品名拼写。也可考虑升模型
  （`small` → `medium` / `large-v3`）。两者都属 bili-dl 的**共享行为**（文献盒同链路），改动影响既有功能，故未擅自改。
