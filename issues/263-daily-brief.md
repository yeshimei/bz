  ...
  "briefs": [                          // 简报条目（插件侧写）
    {
      ...
      "body": "一句话总结，60 字以内，单行。",   // AI 总结（单行自然句）
...
- 触发：守护 spawn 完成后落「待出稿」标记（或插件装载时扫描 `briefs` 中 `body` 为空且无 `error` 的条目）→ 插件读转录稿 → `createAI()`（沿用插件已配 provider）→ 产**一句话总结（单行，60 字以内）** → 写回 `briefs[].body` → 清标记。
...
- 右栏阅读区：一句话总结 + 顶部「打开原视频」与视频元信息（时长 / 发布时间）+ 可展开转录稿（细节兜底）。
...
- 场景 C：有新视频且**有字幕** → 条目 `src: 'subtitle'`，正文一句话总结非空且为单行。
...
## 产出形态修订（2026-09-10 用户实读后拍板）

初版按 ADR-0119 §7 产出「按小节分组的要点列表」。用户打开 剪藏本 → 每日简报 实读后反馈**总结过长**，改为**一句话总结**：

- `buildBriefPrompt` 重写：只输出一句、不分段、不加小标题与列表符号、60 字以内、禁前缀套话（`cleanPoints` 更名 `cleanSummary` 并**强制压成单行**、剥标题/列表符号，兼容模型偶尔仍吐的旧形态）。
- 阅读面文案同步：「正在生成本期总结…」「本期总结已重新生成」等。
- ADR-0119 §6/§7 与 CONTEXT.md「每日简报」「简报条目」两词条同步改写（旧形态列为 _Avoid_ 与 Considered Options）。
- 细节由阅读面「完整转录稿」折叠区兜底——一句话总结 + 折叠全文，取代原「要点列表」。
