...
- 状态：已交付（`src/knowledge/mount-types.ts` 共享契约 + `mount-data.ts` 解析层 `parseMountLinks`／`classifyKind`／`readSubpathBody`／`relocateAnchor`；`tests/knowledge/mount-data.test.ts` 41 用例覆盖六类／别名／全路径／大小写／重复／自链／断链与锚点重定位）
