...
/** memo 条目全字段样本（14 基础字段 + issue 353 recur + issue 354 checklist = 齐全无缺失漂移） */
function fullMemoItem(over: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    courseName: null, coursePath: null, linkedNote: null, url: null, recur: null, checklist: null, ...over,
  };
}
...
describe('检查二：字段漂移', () => {
  ...
  it('漂移字段常量与域 normalize 契约一致（16/15/3；memo 16 = 14 基础 + issue 353 recur + issue 354 checklist）', () => {
    expect(MEMO_ITEM_FIELDS).toHaveLength(16);
