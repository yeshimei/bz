...
describe('memoEditedText（编辑 α 合并）', () => {
  ...
  it('标题变 + 其他变更 → 主句「你编辑了备忘录」+（变更列表，示例逐字）', () => {
    ...
    expect(memoEditedText(old, next))
      .toBe('你编辑了备忘录「写周报」（课程改为「算法」，场景改为「工作」，关联笔记 书库/1984.md）');
  });
  it('仅标题变 → 以「改题为」为主句', () => {
    expect(memoEditedText(snap({ title: '旧标题' }), snap({ title: '新标题' }))).toBe('你改题为「新标题」');
  });
  it('标题没变 + 变更 → 「你更新了备忘录」+：+ 变更列表（示例逐字）', () => {
    ...
    expect(memoEditedText(old, next))
      .toBe('你更新了备忘录「写周报」：课程改为「算法」、删除脚本「主页」、截止延到 09-01 12:00');
  });
  it('无变化 → null（不产出）', () => {
    const s = snap({});
  ...
  it('课程/脚本 增改删、定位 关联/改/删、截止 设/延/清除、优先级改', () => {
    expect(memoEditedText(snap({ courseName: null }), snap({ courseName: '算法' }))).toBe('你更新了备忘录「写周报」：添加课程「算法」');
    expect(memoEditedText(snap({ courseName: '算法' }), snap({ courseName: null }))).toBe('你更新了备忘录「写周报」：删除课程');
    expect(memoEditedText(snap({ scriptName: null }), snap({ scriptName: 'main.py' }))).toBe('你更新了备忘录「写周报」：添加脚本「main.py」');
    expect(memoEditedText(snap({ scriptName: 'main.py' }), snap({ scriptName: 'app.py' }))).toBe('你更新了备忘录「写周报」：脚本改为「app.py」');
    expect(memoEditedText(snap({ notePath: null }), snap({ notePath: '书库/1984.md' }))).toBe('你更新了备忘录「写周报」：关联笔记 书库/1984.md');
    expect(memoEditedText(snap({ notePath: '书库/1984.md' }), snap({ notePath: '书库/1985.md' }))).toBe('你更新了备忘录「写周报」：笔记改为 书库/1985.md');
    expect(memoEditedText(snap({ notePath: '书库/1984.md' }), snap({ notePath: null }))).toBe('你更新了备忘录「写周报」：删除笔记关联');
    expect(memoEditedText(snap({ due: null }), snap({ due: '2026-08-25 18:00' }))).toBe('你更新了备忘录「写周报」：设截止 08-25 18:00');
    expect(memoEditedText(snap({ due: '2026-08-25 18:00' }), snap({ due: null }))).toBe('你更新了备忘录「写周报」：清除截止日期');
    expect(memoEditedText(snap({ priority: 'minor' }), snap({ priority: 'important' }))).toBe('你更新了备忘录「写周报」：优先级改为重要');
  });
});

describe('完成/恢复/延后/优先级/删除（仅标题）', () => {
  it('五种动作文案', () => {
    expect(memoCompletedText('写周报')).toBe('你完成了备忘录「写周报」');
    expect(memoRestoredText('写周报')).toBe('你把备忘录「写周报」恢复为未完成');
    expect(memoPostponedText('写周报', '2026-08-28 18:00')).toBe('你把备忘录「写周报」延后到了 08-28 18:00');
    expect(memoPriorityText('写周报', 'important')).toBe('你把备忘录「写周报」转为重要');
    expect(memoPriorityText('写周报', 'minor')).toBe('你把备忘录「写周报」转为次要');
    expect(memoDeletedText('写周报')).toBe('你删除了备忘录「写周报」');
  });
});
...
describe('memoDueObservation（每日到期扫描合并一条）', () => {
  ...
  it('今天到期且未完成 → 合并一条（HH:mm）', () => {
    ...
    expect(memoDueObservation(items, now))
      .toBe('你有 2 个备忘录今天到期：写周报（18:00）、买菜（20:30）');
  });
  it('超过 5 条 → 截断展示前 5，追加「等 N 个」', () => {
    const mk = (i: number) => ({ title: `备忘录${i}`, due: `2026-08-25 1${i}:00`, completed: null });
    const items = Array.from({ length: 7 }, (_, i) => mk(i));
    const text = memoDueObservation(items, now)!;
    expect(text).toContain('你有 7 个备忘录今天到期：');
    expect(text).toContain('备忘录0（10:00）、备忘录1（11:00）、备忘录2（12:00）、备忘录3（13:00）、备忘录4（14:00）');
    expect(text).toContain('等 7 个');
    expect(text).not.toContain('备忘录5');
  });
  it('恰好 5 条 → 不截断（无「等 N 个」后缀）', () => {
    const mk = (i: number) => ({ title: `备忘录${i}`, due: `2026-08-25 1${i}:00`, completed: null });
    expect(memoDueObservation(Array.from({ length: 5 }, (_, i) => mk(i)), now))
      .toBe('你有 5 个备忘录今天到期：备忘录0（10:00）、备忘录1（11:00）、备忘录2（12:00）、备忘录3（13:00）、备忘录4（14:00）');
  });
});
...
describe('buildMemoActionText（事件 → 观察文本）', () => {
  it('全动作映射', () => {
    expect(buildMemoActionText({ kind: 'added', title: '写周报', scene: '工作', priority: 'important', due: '2026-08-25 18:00', notePath: '书库/1984.md', scriptName: null, courseName: '算法' }))
      .toBe('你添加了备忘录「写周报」（场景：工作，课程：算法，优先级：重要，截止：08-25 18:00，笔记：1984.md）');
    expect(buildMemoActionText({ kind: 'completed', title: '写周报' })).toBe('你完成了备忘录「写周报」');
    expect(buildMemoActionText({ kind: 'restored', title: '写周报' })).toBe('你把备忘录「写周报」恢复为未完成');
    expect(buildMemoActionText({ kind: 'postponed', title: '写周报', due: '2026-08-28 18:00' })).toBe('你把备忘录「写周报」延后到了 08-28 18:00');
    expect(buildMemoActionText({ kind: 'priority', title: '写周报', to: 'minor' })).toBe('你把备忘录「写周报」转为次要');
    expect(buildMemoActionText({ kind: 'deleted', title: '写周报' })).toBe('你删除了备忘录「写周报」');
  });
  it('编辑无变化 → null', () => {
...
describe('buildMemoDueScanStructured（每日到期扫描 → StructuredMeta）', () => {
  ...
  it('有到期 → entityType=task, action=due, extras.text 含合并文案', () => {
    ...
    expect(s!.extras!.text).toBe('你有 1 个备忘录今天到期：写周报（18:00）');
  });
});