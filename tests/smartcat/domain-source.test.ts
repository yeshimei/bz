...
describe('DOMAIN_FILES（5 域新增感知；memo/news/favorites 均已移除）', () => {
  it('memo/news/favorites 均不再有 extract（ticket 075/076/078 移除），防双记录', () => {
  ...
  it('quiz/review/belongings：数量增长才产出（favorites 已移除不感知）', () => {
    const prev = new Map<string, string>();
