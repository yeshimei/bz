 ...
 * ticket 079：belongings 移除（改走 methods 监听 notifyBelongingsAction，防双记录）
 */
import { describe, it, expect } from 'vitest';
import { DOMAIN_FILES, snapshotDomains } from '../../src/smartcat/domain-source';

describe('DOMAIN_FILES（4 域新增感知；memo/news/belongings 均已移除）', () => {
  it('memo（ticket 075）、news（ticket 076）、belongings（ticket 079）不再有 extract，防双记录', () => {
  ...
  it('quiz/review/favorites：数量增长才产出', () => {
    const prev = new Map<string, string>();
