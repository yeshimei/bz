...
describe('KnowledgeData（knowledge.json）', () => {
  let vault: MockVault;
  ...
  afterEach(() => {
    // 重置单例 store，避免跨用例串文件
    (KnowledgeData as any)._store = null;
    (KnowledgeData as any).filePath = '';
  });

  it('首读自建空文件（[]），addTask 追加队尾且字段完整', async () => {
    const t = await KnowledgeData.addTask({ url: 'https://www.bilibili.com/video/BV1xx411c7mD', start: '1:02:03', end: '1:05:00', remark: '重点段落' });
    expect(t.id).toMatch(/^knowledge-task-/);
  ...
  it('title/uploader 可选字段（ticket 134）：addTask 去空白落库 + 旧数据补 null + updateTask 可改', async () => {
    const t = await KnowledgeData.addTask({ url: 'BV1xx411c7mD', title: '  某视频标题  ', uploader: ' UP主甲 ' });
    expect(t.title).toBe('某视频标题');
    expect(t.uploader).toBe('UP主甲');
    // 旧数据（无新字段）读入零迁移：补 null 不崩（追加一条，不覆盖已有任务）
    const cur = await KnowledgeData.read();
    cur.push({ id: 'legacy-2', url: 'BV1xx411c7mE' });
    await KnowledgeData.write(cur);
    const legacy = await KnowledgeData.loadTasks();
    expect(legacy.find((x) => x.id === 'legacy-2')).toMatchObject({ title: null, uploader: null });
    // 编辑可改可清空
    await KnowledgeData.updateTask(t.id, { title: '改标题', uploader: null });
    const all = await KnowledgeData.loadTasks();
  ...
  it('updateTask 保留 id 与 created，仅合并补丁', async () => {
    ...
    await KnowledgeData.updateTask(t.id, { status: 'success', notePath: '文献盒/测试.md' } as any);
    const all = await KnowledgeData.loadTasks();
    expect(all[0].id).toBe(t.id);
  ...
  it('updateTask 不存在的 id 抛错', async () => {
    await expect(KnowledgeData.updateTask('nope', { status: 'failed' })).rejects.toThrow('任务不存在');
  });

  it('deleteTask 移除指定任务；不存在静默', async () => {
    const t = await KnowledgeData.addTask({ url: 'BV1xx411c7mD' });
    const t2 = await KnowledgeData.addTask({ url: 'BV1xx411c7mE' });
    await KnowledgeData.deleteTask(t.id);
    const all = await KnowledgeData.loadTasks();
    expect(all.map((x) => x.id)).toEqual([t2.id]);
    await KnowledgeData.deleteTask('nope'); // 不抛
  });

  it('retryTask：失败项回到待处理并清 reason/processedAt（保留旧结果字段）', async () => {
    const t = await KnowledgeData.addTask({ url: 'BV1xx411c7mD' });
    await KnowledgeData.updateTask(t.id, { status: 'failed', reason: '视频已删除', processedAt: '2026-08-28 20:00:00', notePath: null, videoPath: null } as any);
    await KnowledgeData.retryTask(t.id);
    const all = await KnowledgeData.loadTasks();
  ...
  it('clearHistory 只清归档项（archived；失败/待处理/处理中保留，ADR-0067）', async () => {
    const ok = await KnowledgeData.addTask({ url: 'BV1xx411c7mD' });
    const bad = await KnowledgeData.addTask({ url: 'BV1xx411c7mE' });
    const pend = await KnowledgeData.addTask({ url: 'BV1xx411c7mF' });
    await KnowledgeData.updateTask(ok.id, { status: 'success', archived: true, archivedAt: '2026-08-28 21:00:00' } as any);
    await KnowledgeData.updateTask(bad.id, { status: 'failed', reason: 'x' } as any);
  ...
  it('loadTasks 字段形状：title/uploader/archived/quality/page 缺省补默认（ADR-0067 零迁移）', async () => {
    await KnowledgeData.addTask({ url: 'BV1xx411c7mD', quality: '720', page: 2 });
    const all = await KnowledgeData.loadTasks();
    expect(all[0]).toMatchObject({
      title: null, uploader: null, archived: false, archivedAt: null,
      quality: '720', page: 2,
    });
    await KnowledgeData.addTask({ url: 'BV1xx411c7mE' });
    const all2 = await KnowledgeData.loadTasks();
