/**
 * 文献盒批量处理器测试（src/knowledge/processor.ts，ticket 136/ADR-0071：AI 回迁插件侧）：
...
import type { KnowledgeTask } from '../../src/knowledge/types';
...
async function seedTasks(): Promise<KnowledgeTask[]> {
  await KnowledgeData.addTask({ url: 'BV1xx411c7mD', start: '1:02:03', end: '1:05:00' });
  await KnowledgeData.addTask({ url: 'BV1xx411c7mE' });
  return KnowledgeData.loadTasks();
}
...
describe('BatchRunner', () => {
  ...
  it('步骤文案持久化：[bz-step] 落库后 UI 重读可见（修「一直显示启动中」）', async () => {
    ...
    await tick();
    all = await KnowledgeData.loadTasks();
  ...
  it('严格串行 + close(0) 驱动插件侧 AI：读转录 → generateVideoNote → 删临时文件 → 成功落库归档 + converted 事件', async () => {
    const tasks = await seedTasks();
    const steps: string[] = [];
    const done: KnowledgeTask[] = [];
    const bus: any[] = [];
    const sub = onDomainEvent('knowledge:tasks', (e) => bus.push(e));
    try {
      const ev = {
        onTaskProgress: (t: KnowledgeTask, s: string) => steps.push(`${t.id}:${s}`),
        onTaskInfo: vi.fn(),
        onTaskDone: (t: KnowledgeTask) => done.push(t),
        onBatchDone: vi.fn(),
      };
      const p = BatchRunner.runAll(tasks, ev);
      await tick();
  ...
  it('AI 失败（含 AI 未配置）→ 该任务 failed、不落半成品笔记、临时文件清理、failed 域事件', async () => {
    ...
    const sub = onDomainEvent('knowledge:tasks', (e) => bus.push(e));
    try {
      const p = BatchRunner.runAll(await KnowledgeData.loadTasks(), ev);
      await tick();
      child.stdout.emit('data', Buffer.from(RESULT_LINE));
      child.emit('close', 0);
      await p;
  ...
  it('taskJson options 全量下发：compress/crf/vaultPath/ffmpegPath 等 + 任务级/全局设置映射（ADR-0071）', async () => {
    ...
    await KnowledgeData.addTask({ url: 'BV1xx411c7mD', quality: '720', page: 3 });
    const ev = makeEvents();
    const p = BatchRunner.runAll(await KnowledgeData.loadTasks(), ev);
    await tick();
  ...
  it('任务终态落库抛错（处理中被删除）→ 批量不挂起：尽力走回调、后续任务继续、onBatchDone 正常（P2-1）', async () => {
    const realUpdate = KnowledgeData.updateTask.bind(KnowledgeData);
    const spy = vi.spyOn(KnowledgeData, 'updateTask').mockImplementation(async (id: string, patch: any) => {
  ...
  it('[bz-p] 进度行解析：onTaskProgress 第三参带阶段百分比，瞬态不落库', async () => {
    ...
    const ev = {
      onTaskProgress: (t: KnowledgeTask, s: string, p?: any) => seen.push({ step: s, prog: p ?? null }),
      onTaskInfo: vi.fn(),
      onTaskDone: vi.fn(),
      onBatchDone: vi.fn(),
    };
  ...
  it('遇错即停（设置开启）：首部 CLI 失败后不再 spawn 剩余，onBatchDone stopped:true', async () => {
    setSettingsProvider(() => ({ knowledgeStopOnFailure: true }) as any);
    const tasks = await seedTasks();
    const ev = makeEvents();
    const p = BatchRunner.runAll(tasks, ev);
    await tick();
  ...
  it('[bz-info] 解析信息：落库 title/uploader + onTaskInfo 回调，不发射文献盒域事件（P3-2，契约收敛 converted/failed）', async () => {
    ...
    try {
      ...
      // parsed 域事件已删除：解析信息不发射任何 knowledge:tasks 事件（仅 converted/failed 两类）
      expect(bus).toEqual([]);
      BatchRunner.abort();
      child.emit('close', 0);
      await p;
