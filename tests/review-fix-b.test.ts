...
describe('批 B-3：触控热区收编 core .bz-touch-target', () => {
  ...
  it('跳过项守护：attach（padding 抬档形态）维持域内 pointer:coarse 块；cinema 已在批 C 收编 .bz-touch-target', () => {
    ...
    expect(repo('src/cinema/layouts/midnight/render.ts')).not.toMatch(/bz-touch-target/); // cinema 风格化落域（issue 236 / ADR-0103）：脱离共享面板骨架，域内自绘（先例收藏本 ADR-0101）；ADR-0104 markup 迁布局层后反扫随之
