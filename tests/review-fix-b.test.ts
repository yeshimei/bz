...
describe('批 B-3：触控热区收编 core .bz-touch-target', () => {
  ...
  it('收编域挂类与档位映射（外扩量与原域内 inset 一一对应）', () => {
    ...
    expect(repo('src/core/ui/setlist.ts')).toContain("remove.className = 'bz-setlist-remove bz-touch-target--xl'");
  ...
  it('跳过项守护：attach（padding 抬档形态）维持域内 pointer:coarse 块；cinema 已在批 C 收编 .bz-touch-target', () => {
    ...
    // 2026-09-20 深审 cinema 批B 起反向收编：设计手册 §8.2 绝对下限 40px 对风格化域同约束，
    // 热区外扩不可见不涉视觉自治（原 ADR-0103 豁免理由不覆盖），头行钮/chips 挂共享类（diary/favorites 先例）
    expect(repo('src/cinema/layouts/midnight/render.ts')).toMatch(/bz-touch-target/);
    expect(repo('src/cinema/shared.ts')).not.toMatch(/bz-touch-target/);
...
describe('批 B-5：图标单一事实源尾差', () => {
  ...
  it('home 入口 icon 全量迁移：iconOf() 引 DOMAIN_ICONS，无残留字面量（issue 232b 收敛后 13 条；issue 250 补密码本 14 条；issue 251 补第二大脑 15 条；ADR-0115 回忆墙磁贴并入日记本 → 14 条；issue 368 补游戏架 → 15 条）', () => {
