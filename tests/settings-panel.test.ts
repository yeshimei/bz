...
describe('设置面板（settings-panel）', () => {
  ...
  it('桌面端：构建侧栏工作台（整宽头行仅标题 + 搜索 + 域导航 + 右侧面板）', async () => {
    ...
    expect(badges[3]).toBe('7'); // AI 页（issue 330 拆三组但行数不变）：服务商+模型名称+上下文+最大输出 + B站 Cookie/ApiZero Key/豆瓣 Cookie（密钥行门控隐藏）
  ...
  it('桌面端：默认渲染通用域（数据存储路径），点 AI 域渲染 AI 服务商设置', async () => {
    ...
    expect(groups.length).toBe(3); // issue 330：AI 页拆三组（服务商/模型配置/数据源凭据）
  ...
  it('桌面端：AI 域数据源凭据组——Cookie 行渲染为 textarea，B站行带「从 CLI 导入」按钮（issue 330）', async () => {
  ...
  it('桌面端：分组卡图标为 lucide（schema 图标名 → setIcon data-icon）', async () => {
    ...
    // AI 域分组：服务商/模型配置/数据源凭据（issue 330 拆三组）
    const aiItem = Array.from(popup.querySelectorAll('.bz-sp-nav-item')).find(
      (el) => el.textContent?.includes('AI')
    ) as HTMLElement;
    aiItem.click();
