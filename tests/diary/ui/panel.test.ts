...
describe('设置读取（ticket 09 前置）', () => {
  ...
  it('⚙️ 设置弹窗：分组卡片 + 目录/批量/标签计数/文件日期', async () => {
    ...
    // 分组卡片结构：5 组可见（外观/目录/显示/默认视图/维护；issue 246 补外观组；移动端组挂 bz-setting-hidden
    // 整组隐藏——ticket 131），原生图标 + 徽标回填项数
    ...
    expect(heads.map((el) => (el as HTMLElement).textContent!.trim())).toEqual(['外观2 项', '目录4 项', '显示5 项', '默认视图3 项', '维护0 项']);
    expect(heads.map((el) => el.querySelector('.bz-settings-group-icon')!.getAttribute('data-icon'))).toEqual(['palette', 'folder-open', 'eye', 'monitor', 'wrench']);
