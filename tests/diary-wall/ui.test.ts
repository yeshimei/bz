...
describe('回忆墙 UI', () => {
  ...
  it('头部按钮序：编辑、搜索、按年月跳转（增强 #10）、设置（issue 201）、关闭；图标 lucide 化（增强 #4）', async () => {
    ...
    // 编辑 → 搜索 → 年月跳转 → 设置 → 关闭（issue 201 头行对齐备忘录：+⚙设置直达）
  ...
  it('issue 210：胶卷条只收图片/视频条目，文字/纯音频不产格，每月最多 5 格', async () => {
    ...
    expect(rail.querySelector('.bz-diary-wall-month-thumb--a')).toBeNull();
    rail.querySelectorAll('.bz-diary-wall-month').forEach((m) => {
      expect(m.querySelectorAll('.bz-diary-wall-month-thumb').length).toBeLessThanOrEqual(5);
    });
  ...
  it('issue 210：章节栏视频缩略懒加载——无 IO 直挂 src + preload=metadata，格内留播放角标', async () => {
    ...
    await openAndWait();
    const rail = document.querySelector('.bz-diary-wall-desk .bz-diary-wall-rail') as HTMLElement;
    const vt = rail.querySelectorAll('.bz-diary-wall-month-thumb--v');
