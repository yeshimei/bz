...
describe('obsidian-adapter', () => {
  ...
  it('create：md 文件恒发通用兜底事件，命中日记目录另发语义事件且附带 date', () => {
    ...
    vault.fire('create', { path: '我的/日记/2608231232.md', extension: 'md' });
    expect(rec.payloadsOf('vault:md-created')).toEqual([{ path: '我的/日记/2608231232.md' }]);
    expect(rec.payloadsOf('diary:file-created')).toEqual([
      { path: '我的/日记/2608231232.md', date: '2026-08-23' },
    ]);
  ...
  it('rename 到日记目录：语义事件附带新路径日期', () => {
    ...
    attachObsidianAdapter({ vault }, vi.fn());
    vault.fire('rename', { path: '我的/日记/2601030930.md', extension: 'md' }, '我的/日记/2601022105.md');
    expect(rec.payloadsOf('diary:file-renamed')).toEqual([
      { oldPath: '我的/日记/2601022105.md', newPath: '我的/日记/2601030930.md', movedOut: false, date: '2026-01-03' },
    ]);
    rec.stop();
  });
