...
/**
 * memo 域 D2 可靠写契约回归（试点域写路径迁移）：memo.json 双视角同源（与旧 memo 共写）。
...
const BASE_SETTINGS = { storagePath: 'CONFIG/STORAGE', memoFilePath: 'CONFIG/STORAGE', cinemaFolderPath: '我的/影视' };
const MEMO_PATH = 'CONFIG/STORAGE/memo.json';

const seedItem = (id: string, patch: Record<string, any> = {}) => ({
  id, title: '备忘录' + id, scene: '工作', priority: 'minor', created: '2025-06-14 10:00:00', completed: null, ...patch,
});

describe('memo（memo.json 双视角）D2 可靠写契约', () => {
  let vault: MockVault;
  beforeEach(() => {
    vault = new MockVault();
  ...
  it('①批量迁移场景与单条完成并发：双方改动都落盘', async () => {
    ...
    await Promise.all([
      MemoData.updateSceneBulk('工作', '学习'),
      MemoData.completeItem('b'),
    ]);
  ...
  it('②解析坏文件 → 留档 CONFIG/.CORRUPT + 降级初始化后域功能可用', async () => {
    ...
    expect(await MemoData.updateSceneBulk('工作', '学习')).toBe(0);
    ...
    // 降级后域功能可用：新增 + 批量迁移正常
    await MemoData.addItem(seedItem('t') as any);
    expect(await MemoData.updateSceneBulk('工作', '学习')).toBe(1);
    expect((await MemoData.loadItems())[0].scene).toBe('学习');
  });
});
