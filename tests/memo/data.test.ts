...
/**
 * 备忘录（memo）数据层测试：memo.json 同源读写（与旧 memo 共用）、14 字段零迁移、
 * 场景解析、条目 CRUD、公开课笔记检索。
 */
import { describe, it, expect, beforeEach } from 'vitest';
import { MemoData, parseScenarios, DEFAULT_SCENARIOS } from '../../src/memo/data';
...
describe('MemoData 构建', () => {
  ...
  it('init 拼出 memo.json 路径（storagePath / 尾部斜杠去除）', () => {
    MemoData.init({ ...BASE_SETTINGS, storagePath: 'CONFIG/STORAGE/' });
    expect(MemoData.memoFilePath).toBe('CONFIG/STORAGE/memo.json');
    MemoData.init({ ...BASE_SETTINGS, storagePath: 'DATA/' });
    expect(MemoData.memoFilePath).toBe('DATA/memo.json');
  });

  it('场景从设置解析（逗号分隔去空去重；空则默认 6 场景，与旧 memo 一致）', () => {
    MemoData.init({ ...BASE_SETTINGS, memoScenarios: '工作,学习,工作, ' });
    expect(MemoData.getScenarios()).toEqual(['工作', '学习']);
    MemoData.init({ ...BASE_SETTINGS, memoScenarios: '' });
    expect(MemoData.getScenarios()).toEqual(DEFAULT_SCENARIOS);
  });
  ...
  it('读取旧 memo 域写入的数据（14 字段完整条目零迁移）', async () => {
    ...
    const legacy = {
      ...
      linkedNote: '归档/网页剪藏/文章.md', url: 'https://example.com/a', recur: null, checklist: null,
    };
...
describe('MemoData CRUD', () => {
  ...
  it('addItem 头部插入 + 读取往返', async () => {
    await MemoData.addItem({ id: 'a', title: 'A', scene: '工作' } as any);
    await MemoData.addItem({ id: 'b', title: 'B', scene: '学习' } as any);
    const items = await MemoData.read();
    expect(items.map((i: any) => i.id)).toEqual(['b', 'a']);
  });

  it('updateItem：合并字段、保留 id/created、title 自动提取 url', async () => {
    await MemoData.addItem({ id: 'a', title: '旧标题', scene: '工作', created: '2025-01-01 00:00:00' } as any);
    await MemoData.updateItem('a', { title: 'https://example.com 新标题', scene: '剪藏' } as any);
    const items = await MemoData.read();
  ...
  it('updateItem 不存在 → 抛「条目不存在」', async () => {
    await expect(MemoData.updateItem('nope', { title: 'x' } as any)).rejects.toThrow('条目不存在');
  });

  it('completeItem：写入 completed 时间戳', async () => {
    await MemoData.addItem({ id: 'a', title: 'A', scene: '工作' } as any);
    await MemoData.completeItem('a');
    const items = await MemoData.read();
  ...
  it('deleteItem：删除指定 id，返回被删条目原索引', async () => {
    await MemoData.addItem({ id: 'a', title: 'A', scene: '工作' } as any);
    await MemoData.addItem({ id: 'b', title: 'B', scene: '工作' } as any);
    const idx = await MemoData.deleteItem('a');
    const items = await MemoData.read();
  ...
  it('deleteItem：删除不存在 id 返回 -1', async () => {
    const idx = await MemoData.deleteItem('nope');
    expect(idx).toBe(-1);
  });
  ...
  it('restoreItem：撤销删除——条目插回删除前原索引（增强包：删除接撤销）', async () => {
    await MemoData.addItem({ id: 'a', title: 'A', scene: '工作' } as any);
    await MemoData.addItem({ id: 'b', title: 'B', scene: '学习' } as any);
    await MemoData.addItem({ id: 'c', title: 'C', scene: '生活' } as any);
    // 顺序 [c, b, a]：删中间的 b（idx=1），撤销后必须回到 idx=1 而不是头部
    const idx = await MemoData.deleteItem('b');
    expect(await MemoData.read()).toEqual([
      expect.objectContaining({ id: 'c' }),
      expect.objectContaining({ id: 'a' }),
    ]);
    await MemoData.restoreItem({ id: 'b', title: 'B', scene: '学习' } as any, idx);
    expect((await MemoData.read()).map((i: any) => i.id)).toEqual(['c', 'b', 'a']);
  });

  it('restoreItem：未传索引/越界回退头部插入（对齐 addItem 语义）', async () => {
    await MemoData.addItem({ id: 'a', title: 'A', scene: '工作' } as any);
    await MemoData.addItem({ id: 'b', title: 'B', scene: '工作' } as any);
    await MemoData.deleteItem('a');
    await MemoData.restoreItem({ id: 'a', title: 'A', scene: '工作' } as any, 99);
    expect((await MemoData.read()).map((i: any) => i.id)).toEqual(['a', 'b']);
    await MemoData.deleteItem('b');
    await MemoData.restoreItem({ id: 'b', title: 'B', scene: '工作' } as any);
    expect((await MemoData.read()).map((i: any) => i.id)).toEqual(['b', 'a']);
  });

  it('updateSceneBulk：批量迁移条目场景（场景重命名/删除的数据底座），返回迁移条数', async () => {
    await MemoData.addItem({ id: 'a', title: 'A', scene: '工作' } as any);
    await MemoData.addItem({ id: 'b', title: 'B', scene: '工作' } as any);
    await MemoData.addItem({ id: 'c', title: 'C', scene: '学习' } as any);
    const moved = await MemoData.updateSceneBulk('工作', '生活');
    expect(moved).toBe(2);
    const items = await MemoData.read();
    ...
    // 无命中：不写盘返回 0
    expect(await MemoData.updateSceneBulk('不存在', '学习')).toBe(0);
  });
