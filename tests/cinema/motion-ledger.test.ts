...
/** 功能性循环时长（加载/指示类；改这些值时随手补进白名单即可，改交互时长则不行）。
 *  现存：1.1s 骨架 / 1.3s 表单扫描 / 1.4s 海报脉冲 / .8s 转圈 / 1s 打字机光标闪烁
 *  （《观影志》影评幕的 `.yb-caret`——光标闪是文本指示，不入交互台账）；
 *  旧观影分析的灯牌/探照灯/表盘循环已随该稿删除，白名单同步收干净。 */
const FUNCTIONAL_LOOPS = ['1.1s', '1.3s', '1.4s', '.7s', '.8s', '1s'];
