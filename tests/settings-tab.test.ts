...
describe('设置页 BzSettingTab（ADR-0009 单页）', () => {
  ...
  it('单页平铺：无 tab；分组卡片（带 icon）：AI + 采样参数（issue 187）+ 数据存储路径', () => {
    ...
    expect(groupNames).toEqual(['AI', '采样参数', '数据存储路径']);
    const groupIcons = [...tab.containerEl.querySelectorAll('.bz-settings-group-icon')].map((i) => i.getAttribute('data-icon'));
    expect(groupIcons).toEqual(['sparkles', 'sliders-horizontal', 'folder-open']);
  });
