/* 源指纹 b1e236b84a9c8e47 · 仓内输入 41 个（校验见 tests/preview-freshness.test.ts） */
...
var BZW_knowledge = (() => {
  ...
  function resolveModelLimits(model) {
    const key2 = normalizeModelId(model || "");
    if (!key2) return null;
    let best = null;
    for (const entry of MODEL_LIMITS) {
      for (const k of [entry.id, ...entry.aliases || []]) {
        if (key2 === k) return { maxOutput: entry.maxOutput, contextWindow: entry.contextWindow };
        if (key2.includes(k) && (!best || k.length > best.len)) best = { entry, len: k.length };
      }
    }
  ...
  function outboundTargetsViaCache(ctx, file) {
    ...
    for (const key2 of ["related", "mounted"]) {
      for (const item of fmValueList(fm == null ? void 0 : fm[key2])) {
  ...
  function valueStartOf(prefix, key2) {
    const marker = `"${key2}"`;
    let from = 0;
    for (; ; ) {
  ...
  function partialStringField(prefix, key2) {
    const start = valueStartOf(prefix, key2);
    ...
    while (i < prefix.length) {
      ...
      if (ch === "\\") {
        ...
        out += esc2 in ESCAPE_CHARS ? ESCAPE_CHARS[esc2] : esc2;
        i += 2;
        continue;
      }
      if (ch === '"') break;
...
${sample}`
  ...
  function routeEdges(boxes, edges, opts) {
    ...
    for (const j of jobs) {
      ...
      let dirty = !exitClean || !j.clean || stubDirty(E2, E1, rects) || stubDirty(T1, T, rects);
      const items = [S, E2, E1];
      if (raw) {
...
${String(blockText != null ? blockText : "").trim()}`);
  ...
  var VIDEO_EXIT = "bz-kb-video-exit";
  ...
  var treeShownIds = /* @__PURE__ */ new Set();
...
`;
  ...
  var UIManager = class {
    ...
    cardRowHtml(c) {
      ...
      <div class="bz-kb-hw"><span class="bz-kb-w">${esc(c.title)}</span>${orphan ? '<span class="bz-kb-orphan" title="既无入链也无挂载">孤 儿</span>' : ""}<span class="bz-kb-dom">${esc(
        c.domain
      )}</span></div>
      <div class="bz-kb-tail"><span>${c.review ? "复习中 · 到期由闹钟安排" : "未入复习"}</span>${n > 0 ? `<span class="bz-kb-refbadge" title="${esc(refBadgeTitle(n))}">被引 ${n}</span>` : ""}<button class="bz-kb-mt-openbtn" data-kb-act="mount-tree" data-path="${esc(c.path)}" title="以这张卡为主卡打开挂载树">看挂载树</button></div>
    </div>`;
    }
    /**
    ...
    entryRelRefresh() {
      ...
      if (st === "done") {
        ...
        el.innerHTML = items.map(
          (it, i) => `<span class="bz-lit-rel-chip"><span>${esc(it.title)}</span><button type="button" data-rel-drop="${i}" title="这条不写入">✕</button></span>`
        ).join("");
        motionRelChips(el);
        return;
      }
