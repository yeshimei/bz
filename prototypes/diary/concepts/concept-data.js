/* 构建产物（勿手改）：node scripts/build-diary-concepts.mjs — concepts/concept-data.ts（概念稿共享数据包，复用真解析链） */
var BZ_CONCEPT = (() => {
  ...
  // prototypes/diary/concepts/concept-data.ts
  var concept_data_exports = {};
  __export(concept_data_exports, {
    boot: () => boot,
    model: () => model,
    tagHue: () => tagHue,
    url: () => url
  ...
  var MEDIA_EXT_RE = /\.(png|jpe?g|gif|webp|avif|bmp|svg|mp4|m4v|webm|mov|ogv|mp3|m4a|aac|wav|flac|ogg|oga)$/i;
...
![[${String(cover).trim()}]]`;
  ...
  async function boot() {
    ...
    const tagCounts = [...tagMap.entries()].map(([tag, count]) => ({ tag, count })).sort((a, b) => b.count - a.count);
    ...
    const days = [...dayMap.entries()].map(([date, list]) => ({ date, entries: list })).sort((a, b) => b.date.localeCompare(a.date));
    ...
    _model = {
      ...
      mediaUrl: (name, sourcePath) => mediaSrc(app, name, sourcePath),
  ...
  return __toCommonJS(concept_data_exports);
})();
