/**
 * 脸谱行为单源 · 公共假 obsidian（issue 447，范式随 memo）
 *
 ...
 *   - setIcon / IconName       → DOM 内联 SVG（表 = window.BZW_PEOPLE_ICONS，prototype-icons.js）
 ...
 *   - requestUrl              → AI 生成在壳内不演示（无罐头），调用走抛错降级
 */
// ==================== 视口判定 ====================
...
// ==================== 图标（表 = prototype-icons.js 的 window.BZW_PEOPLE_ICONS） ====================

declare global {
	interface Window {
		BZW_PEOPLE_ICONS?: Record<string, string>;
	}
}

/** 渲染 lucide 内联 SVG（未知 id 静默忽略——与 Obsidian setIcon 同语义） */
export function setIcon(container: HTMLElement, iconId: string): void {
	const d = (typeof window !== 'undefined' && window.BZW_PEOPLE_ICONS?.[iconId]) || '';
	if (!d) return;
