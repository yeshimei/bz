...
window.BZ_SP_ICONS = {
  ...
  // 首页组头图标（issue 288 拆组新增：时间线/内容过滤/预告栏）——lucide-static v0.544.0 真值
  ...
  // ===== 2026-09-23 补：收藏本「标签管理」custom 行的图标（域内自绘列表）=====
  //  · 9 类默认标签图标（DEFAULT_TAGS 的 ic：github/app-window/globe（原表已有，不重复）/
  //    brain-circuit/keyboard/bot/beer/waypoints）——缺了整列空白（fake setIcon 查不到即静默忽略）
  //  · 三个操作钮 chevron-up/chevron-down/pencil（trash-2 原表已有）
  //  片段取自收藏本/归物本/影院壳同类键（各壳实现一致）；chevron-up 各壳都没收，
  //  按 lucide v0.544.0 原型手写（收藏本壳的「上移」钮同样空着，那边一并补）
