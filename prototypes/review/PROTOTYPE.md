...
| 文件 | 作用 |
|---|---|
| `prototype.html` | 评审壳。组件样式零内联，按构建同序链接 core 三件套（`../../src/core/…`）+ `../../src/review/styles.css`；`<head>` 内联演示种子（`window.RVW.SEED`：真实 vault 复习条目扩写 + 真实 quiz 题库节选，日期相对「当下」）；内联脚本只剩徽牌（重置数据/移动端开关）与 `?selftest=1` 自检 |
