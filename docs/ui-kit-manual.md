...
| 触控热区 | `.bz-touch-target` | `--sm/--lg/--xl`；内联 `--bz-touch-outset` 覆写 | 仅触屏（pointer:coarse）生效：`::after` 外扩命中区至 ≥44px、视觉不变；档位按元素原尺寸——默认 -6px（32px 档钮）、`--sm` -4px（36/40px 档）、`--lg` -8px（26/28px 档）、`--xl` -12px（20/22px 档）；**修饰类可独立使用**（R2：自带热区，不挂基类 `::after` 照常生成）；热区外扩会盖住相邻元素命中，行距紧凑的列表行**勿用**（改 padding 抬档，先例 attach 清单行） |
...
7px 竖条，`cursor: col-resize`，平时透明、hover/拖拽 `.is-drag` 点亮 `--bz-brand-soft`；`pointer:coarse` 隐藏。配合 §4 `uiVSplitter` 使用。
...
| `uiMainHead` | `{title, count?, action?: {label, icon?, onClick?}}` — **推荐：render 纯层直接用共享类手写 markup（`.bz-panel-head` 等）；工厂适用于行为层动态构建**（现状：零 TS 消费者，实际消费全走纯层手写路径） | `{el, setTitle, setCount}` |
| `uiRail` | `{groups: [{label?, items}], activeId, onSelect?, foot?}` — RailItem=`{id, name, icon?, boxedIcon?, emoji?, badge?: {t, label, tint?}, dot?, count?, pill?, unread?, children?}`（issue 201 加 emoji 槽，优先级 icon > emoji > badge）。**推荐：render 纯层直接用共享类（`.bz-rail` 等）手写 markup；工厂适用于行为层动态构建**（现状：clipbook/memo/diary 均手写，零 TS 消费者） | `{el, setActive}` |
| `uiMobStrip` | `{items: [{id, label, dot?}], value, onChange?}` — **推荐：render 纯层直接用共享类（`.bz-mobstrip` 等）手写 markup；工厂适用于行为层动态构建**（现状：belongings/memo 均手写，零 TS 消费者） | `{el, setValue}` |
