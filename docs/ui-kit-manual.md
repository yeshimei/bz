...
| `uiPopover` | `{anchor, options?: [{id, label, icon?}], value?, emptyText?, onPick?}` — anchor 点击开合模式；anchor 须在 position:relative 容器内，浮层挂其父元素。**输入锚定**（issue 198）：改传 `{input, getOptions?(q), emptyCloses?, keyboard?, onOpenChange?}` | `{open, close, setValue, setOptions, detach}` |
