...
- **域皮肤档豁免（拍板注记）**：`.bz-<域>-skin-*` 域皮肤体系（ADR-0095 备忘录双肤等）经拍板可自持字体字重/边框/阴影档位——如备忘录皮肤段的 `font-weight: 900`、`border: 2px solid` 与硬偏移阴影系拍板豁免而非违规；本节与 §5 的字重/边框/阴影约束针对常规域内样式，自检清单对皮肤段豁免。
...
> **新 UI（新体系）请优先走 `docs/ui-kit-manual.md`**：样式库 `src/core/ui/*.css` + 组件库 `src/core/ui/*.ts` 已实现本手册的取值，域直接用共享类/工厂，不再域内自造。
