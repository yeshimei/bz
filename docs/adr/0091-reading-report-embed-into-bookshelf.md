...
2. **面板内视图**：bookshelf 状态增加 `M.view: 'shelf' | 'report'`，主面板内容区为两互斥视图容器。左栏「阅读分析报告」入口与移动头行报告钮面板内互切；报告视图左栏入口变「‹ 返回书架」（桌面返回路径，对齐面板范式），报告视图内关闭钮仅移动端显示。**（2026-09-20 深审批终态改写）**：左栏已随书脊墙换血退役，返回/关闭钮合并为报告视图右上角恒可见「返回书库」钮（桌面+移动同钮，aria-label 齐，bz-touch-target--lg）——原「关闭钮仅移动端」表述自此作废。
...
- （2026-09-20 深审补认可）bookshelf⇄reading-report 双向回指系本 ADR 宿主关系的实现常态：reading-report 的 `stats.ts`/`index.ts` 回指 `bookshelf/data`（getAllBookNotes 口径同源）属宿主供数，非逆向依赖；后续架构审查以此句为准，不再记档。
