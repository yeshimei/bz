...
## 2026-08-25 修订（用户拍板）：三态观察 → 仅保存观察

上线后暴露两个问题：① 停留时长按 `openedAt` 墙钟起算且关闭面板不暂停——面板挂机/跨时使时长离谱，又叠加「跳过 ≥2 分钟升阅读」规则，大量「跳过」被误记成「读了 N 分钟」（N 离谱）的「阅读」；② 用户重新评估观察价值：跳过/阅读两态记忆价值低，进入观察流的噪音大于信息。修订：

- **观察口径收敛为仅「保存」**：`markAsRead('saved')` 才产观察（立即形态），`notifyNewsSaved` 补全链不变；「下一篇/跳过」不再调用 `notifyNewsRead`（聚合讯域统计 news-stats.json 照记）。「跳过 ≥2 分钟升阅读」规则废除。
- **阅读时长 = 累计可视时间**：`render` 按 `url|title` 键判断文章切换，切换才清零累计；`hide`（关闭/遮罩/ESC）暂停并把已视时长并入 `accumMs`；重开同篇续算；保存结算取整分钟 ≥1（维持「打开起算/关闭暂停/下一篇重置」的用户语义，实得值不再离谱）。
- **兼容**：`NewsReadState` 三态类型、`buildNewsReadText` 三态文案、`notifyNewsRead` 函数契约保留（潜在扩展点），仅生产调用点（reader）收敛为 saved——数据零改动、旧记忆直接可读。