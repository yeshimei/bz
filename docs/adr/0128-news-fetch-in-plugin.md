...
- **news.json 新增段 `lastFetchAt`**（epoch ms 数字，插件写守护退役后插件唯一写）：自动触发按「now − lastFetchAt ≥ 间隔」判定，替代守护的定时轮询；`writeNewsDataMerged` 段清单与 `parseNewsFileContent` 同步扩展（容错解析，缺失 → 0）。避免用 articles 最新 fetchedAt 判定——零新增轮会反复触发。
