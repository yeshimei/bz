...
2b. **字幕可信度必须先校验再采纳**（实现期实测新增）：命中字幕轨后仍过两道闸——① 字幕时间轴末端不得超出视频时长 1.25 倍加 5 秒；② 字速（字/秒）不得超过 12。任一不过 → 判「字幕存疑」并**回退音频转写**，同时把原因记在条目 `subtitleRejected` 上（可见、可追溯）。理由是 B 站对部分稿件返回错配字幕，盲信会产出与视频无关的假要点。
