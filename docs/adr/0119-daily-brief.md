...
6. **阅读区形态**：要点分节（`## 小节` + `- 要点`）+ 顶部「打开原视频」与视频元信息（时长/发布时间）+ **可展开的完整转录稿**（自 bili-dl 缓存读，需要细节时看这里）。
7. **产出形态 = 按小节分组的要点列表，每条要点一句话**（**2026-09-10 两轮实读后定稿**：初版每条要点句子偏长、信息堆叠，用户判为过长；中途一度改为「整篇一句话」，被用户澄清否定——「我说的是每个要点一句话」。故最终形态 = 保留 `## 小节` 分组结构，**每条要点只写一句话、30 字以内**）。明确不要分段时间轴，区别于既有 47 篇结构。
...
- **整篇一句话总结**：2026-09-10 中途试行的形态（曾误判用户意图），用户澄清「我说的是每个要点一句话」——否，回退为要点列表 + 每条要点一句话
