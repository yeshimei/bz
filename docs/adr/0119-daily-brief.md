# 每日简报（B 站 UP 视频 字幕/转写 → AI 一句话总结）

背景：剪藏本需要一个「每日简报」源——盯住指定的 B 站 UP 主，每天把其新投稿变成一句话总结供快速扫读。首个对象为**黑鸦Heya**（mid `3706929260006322`，5.2 万粉，AI 日报类短视频，实测时长 28 秒 – 2 分 53 秒，一天多条）。
...
- 既有 `归档/BILIBILI AI 视频总结/` 的 47 篇外部工具产物，其 frontmatter（category/title/desc/summary/url/bvid/cid/mid/pic/pubdate/duration/up_name/up_face）与分段时间轴格式**不作为本功能的产出格式**（用户选定「短总结」且「不产 .md」）。

### 实现期实测修订（2026-09-10，部分推翻上文一条结论）

实现时带登录 Cookie 复测，**上文「字幕全部为空」是评估缺陷**——那次调用未带 Cookie，B 站对匿名请求不返回字幕轨。带 Cookie 抽样 12 条的真实分布：

| 结果 | 条数 | 特征 |
|---|---|---|
| 字幕可信可采纳 | 3 | 时间轴与视频时长吻合、字速合理（6–7 字/秒） |
| 字幕存在但**内容错配** | 6 | 时间轴远超视频时长（如 39 秒视频配 756 秒字幕，内容是别的视频） |
| 无字幕轨 | 3 | — |

同一 `cid` 两次调用甚至可能拿到**不同**字幕（`BV1ZSb36zE9G`：一次 138 行讲「重生/小娘子」、一次 365 行讲「贵阳炒菜」，而该视频实为 39 秒 AI 日报）。该 UP 简介自称「营销号&标题党，不对信息内容负责」，稿件内容与标题本身也常不匹配。

**结论修正：字幕必须过校验才可采用，转写是主力路径（9/12）而非例外**——这反而强化了「转写兜底」的必要性。校验规则见拍板要点 §2b；Cookie 由此成为**全链路硬前置**（无 Cookie 既拿不到投稿列表、也拿不到字幕）。
...
2. **字幕优先，无字幕转写**。字幕链路为将来别的 UP 留余量；对黑鸦Heya 实测 3/12 可采纳、9/12 落到转写。
2b. **字幕可信度必须先校验再采纳**（实现期实测新增）：命中字幕轨后仍过两道闸——① 字幕时间轴末端不得超出视频时长 1.25 倍加 5 秒；② 字速（字/秒）不得超过 12。任一不过 → 判「字幕存疑」并**回退音频转写**，同时把原因记在条目 `subtitleRejected` 上（可见、可追溯）。理由是 B 站对部分稿件返回错配字幕，盲信会产出与视频无关的假总结。
3. **数据进 news.json，新增两段**：`briefUps`（每日简报名单，uid 数组，插件侧写，与 `bilibiliUps` 并列）与 `briefs`（简报条目，一条视频一条记录）。**转写稿不进 news.json，只放总结**。
...
6. **阅读区形态**：一句话总结 + 顶部「打开原视频」与视频元信息（时长/发布时间）+ **可展开的完整转录稿**（自 bili-dl 缓存读，需要细节时看这里）。
7. **结论形态 = 一句话总结**（**2026-09-10 用户实读后修订**：初版按小节分组的「要点列表」被判过长，改为一句话；`body` 恒为单行自然句，60 字以内，阅读面另留全文转录稿折叠区兜细节）。明确不要分段时间轴，区别于既有 47 篇结构。
...
- **复用 `归档/BILIBILI AI 视频总结/` 产 .md 并对齐其格式（含分段时间轴）**：用户明确「不进 obsidian 仓库、只存数据文件」且选定短总结——否
- **要点列表（按小节分组）**：初版形态，用户实读后判为过长——否，改为一句话总结（细节交给转录稿折叠区）
