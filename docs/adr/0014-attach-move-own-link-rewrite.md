...
4. 其余不变：文件夹选择弹窗、记忆 `attachLastFolder`、不删空目录、结果 toast、主页磁贴自动播种（desktop+mobile 各 placeAtEnd 末尾）。**（2026-09-20 深审批终态注记）**：决策 3 的 warning 文案已并结果 toast；「无预览确认」已被预览确认弹窗取代（可勾选清单+脏关闭拦截+中止+重试，见 CONTEXT 词条）；「主页磁贴自动播种」随 home 磁贴机制演进由 DOMAIN_MENU 快捷动作承接。
