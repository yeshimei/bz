...
4. **设置按钮置关闭正前 + 关闭钮再小 2px + 非全屏隐藏关闭**（用户反馈）：① 各窗口 ⚙️ 一律排在 ❌ 正前（favorites/encrypt/clipping 换序，余窗原本合规）；② 关闭按钮 22×26/14px → **20×24/12px**（新类 `.bz-win-close` 挂全部自定义关闭钮，createIconBtn 系用既有 `--close`，memo/news 用 `.todo-btn-close`/`.news-close-btn`）；③ **移动端 ≤768 且非真全屏（常规卡）时隐藏关闭按钮**——卡片态靠点遮罩关闭、真全屏靠 ❌ 关闭；桌面端不受影响（`@media (max-width:768px)` 内 `.bz-win-mfs` 限定显示）。