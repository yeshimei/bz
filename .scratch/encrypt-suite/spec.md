...
## 三、数据落地（用户拍板，Q10=A + 细化）

- 新增设置项 `encryptRoot`：加密清单与附件密文镜像的根目录，可配置（默认 `CONFIG/ENCRYPT`，可改库外绝对路径）。
- **加密清单**：`<encryptRoot>/safe.enc`——整库唯一清单，AES-GCM 加密（用共享主密钥包裹）。
- **附件镜像**：`<encryptRoot>/附件/` 内按原相对路径镜像存放密文 blob。例如原 `我的/影视/x.png` → `<encryptRoot>/附件/我的/影视/x.png`（密文）。
- 主配置文件（safe.enc）放 encryptRoot 根；图片/视频再新建「附件」子文件夹镜像存放（用户明确要求）。
...
> blob 存储：大文件（尤其视频）**不直接 Base64 内嵌进 JSON**（会膨胀），而是镜像为独立 `.enc` 文件放 `<encryptRoot>/附件/<镜像相对路径>`，清单记镜像相对路径 + 文件大小 + 指纹。小文件可内嵌 Base64。本期实现：**附件一律独立镜像文件**（用户明确要"放到对应文件夹，配置文件记录路径"），正文/摘要/预览 大文本也斟酌（正文直接内嵌，正文一般不大）。
...
- 解锁后：主面板（备忘录样式）标题"加密保险箱"，顶部按钮：🔐 锁定、预感视图开关、⚙️ 设置、关闭。
- 列表：按 createdAt 倒序列出加密笔记卡片（标题 + 状态徽标[已入库/已还原] + 相对时间 + 附件数）；点卡片 → **真还原该篇**（确认弹窗，说明会把原文+原质量附件还原到原路径）；卡片上另有「预览」按钮 → 打开预览窗。
- 已还原未入库的卡片：红点/徽标提示，一键收走（加锁该篇）。
...
| bz-encrypt-lock | 加密当前笔记（入保险箱） | 把当前打开的笔记 + 其双链附件移入保险箱 |
| bz-encrypt-lock-all | 收回全部（加锁） | 把全部已还原未入库的重新入箱 |

> 真还原按篇在面板内操作（不含独立 unlock 命令，避免与 open 重复）；已还原未入库在卡片以「已还原·待收回」徽标提示，可一键收走（lock-all）。
...
```
encryptRoot: string          // 'CONFIG/ENCRYPT'
...
encryptAutoCollectOnHide: boolean // false（可选：窗口隐藏自动收）
```
...
- `data.test.ts`（@vitest-environment node，真实 crypto.subtle + MockVault）：清单加密往返、SafeNote 加锁/还原状态机、崩溃幂等（指纹冲突跳过）、附件镜像路径、指纹校验。
- `ui.test.ts`（jsdom + MockVault）：解锁弹窗（首设/解锁）、主面板列表渲染、预览窗打开、真还原确认、加锁收回、安全模式自动收。
- smoke.test.ts：EXPECTED_COMMAND_IDS 追加 bz-encrypt-open/lock/lock-all。
