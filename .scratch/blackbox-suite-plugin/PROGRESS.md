...
## ✅ 2026-08 · v3 六票全部完成（1108 测试全绿，git: fde5800→ee19261）

01 数据层笔记化 / 02 概念录入改造 / 03 摘抄想法录入 / 04 主面板微调 / 05 实时同步 / 06 原位注入 —— 全部 done。详见各 ticket 段。
...
## 2026-08 · ticket 06 原位注入完成（黑匣子 v3 六票全部落地，1108 测试全绿）

- **`src/blackbox/inject.ts`**：computeInjection 纯函数（行/列 → 偏移替换，恒别名形式 `[[目标|原文字]]`，显示不变）+ injectIntoSourceNote（vault 写来源笔记，铁律 #1 唯一豁免）。
- **四重守卫**：frontmatter / 代码块（```/~~~，含未闭合栅栏）/ 数学块（$$）重叠、选区已是 `[[…]]` 包裹（含 `[[名|别名` 形式）→ 跳过 + toast「选区位于代码块/元数据内，未插入链接」；无选区/来源笔记不存在/越界（文件已变）→ 不注入正常保存（永不拒收）。
