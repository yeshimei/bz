...
1. **单击整卡打开文章**（替换双击；jumpToArticle 语义不变：openLinkText + 关主面板；反链📌 stopPropagation 不受影响）。
...
- Q2/Q7：**单击整卡打开文章，长按打开抽屉**（Q7a：整卡任意处单击即开，移动端同样，反链📌除外）。
...
- `tests/clipping/view.test.ts`：重写双击/长按日期两测；新增 单击打开+关面板、桌面右键菜单（defaultPrevented + 四动作 + 单击仍开）、移动端抽屉全流程（头部两行+动作顺序+删除确认）、复制双链写剪贴板+通知、复制原文链接（sub=域名+写剪贴板）、反链直点（不触发整卡打开、抽屉无反链动作）。
