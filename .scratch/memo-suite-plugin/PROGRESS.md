...
## 2026-09-16 整体评审修复批：剪藏本×知识盒七项（issue 333）
**状态：双评审代理扫 issue 329 四批 + 332 全量改动；明确项 7 修复合并部署，全量 5205 测试（326 文件）全绿**

- ✅ P1：refreshAfterAction prevId 空不误标读（撤销恢复承诺）；P2：删除清侧写、图版描述 ]] 清洗、关联失败不吞 onCreated
- ✅ P3：内部链别名 ]] 清洗（linkAliasText）、本地图不出保存项、直达预览独立宿主 ESC 可关
- 📄 记档未修（设计权衡项）：review-329-final.md（已存覆盖重写语义 / 保留策略侧写清理 / B站分流动线豁免等 8 项）
