...
26. 作为用户，我希望日记「未解析行」不再在启动时弹数字 toast，而是在日记⚙️设置弹窗点「检测日记解析」主动检测：打开面板 → 进度条逐文件解析 → 可修项（头行补空格/时间补零，预览修改前后）确认后一键批量修复、正文归位不改写；不可修项（如时间越界 24:99）列清单点击跳转并定位到行手工改，以便我知道到底哪些日记哪里出错并能修好。（2026-08-27 用户拍板，ticket 121，ADR-0054：手动驱动修复、只修不合规头行、不额外备份、规则白名单 R1 补空格/R2 补零/R3 正文归位；**ADR-0130 起重新定义**——条目文件化后无头行可修，体检只读上报 unparsable/legacy/name-mismatch）

27. 作为用户，我希望日记从「一天一文件多条目」迁移为**一目一文件**：每条日记独立成 `我的/日记/YYYY-MM-DD HH-MM.md` 笔记（同刻冲突加 `-2/-3` 后缀），frontmatter 携带 `日期`（`YYYY-MM-DD HH:mm`，英文冒号）与 `类型`（多值列表，标签名不带 emoji），正文去一级标题外原样保留（#人名/#书名 行留正文、无 标签/星期/来源 属性）；原 522 个日期文件归档 `归档/日记/`，`我的/其他/` 非日期笔记不动；日记墙/小橘/复盘/首页等消费方经 `core/diary-format.ts` 契约单源继续工作，小橘记忆流 2476 条引用同步重写路径段。（2026-09-13 用户拍板，issue 304，ADR-0130：NTFS 禁英文冒号故文件名用 `-`、属性值保留 `:`；解析层不做旧格式兼容；frontmatter 损坏从文件名优雅降级并保留类型；体检保持严格口径；SafeNote 块 v2 用标签名——库内加密日记 0 条零数据迁移）
...
22. 作为用户，我希望密码本从统一保险库拆回独立域（金色三栏工作台 v1 成型版 UI 原样恢复），命令 `bz-password-vault-open`「密码本」+ 设置面板/首页磁贴独立入口，与保险库共锁同库双向同步，以便两域各自演进。（2026-09-09，issue 250，ADR-0109；生成/安全设置键归位密码本 schema，encrypt 面板密码资产暂存待后续摘除）
23. 作为用户，我希望密码域接入原型 × 插件三层单源（样式/渲染/行为与其他域一样一份源码两端跑）：markup 出自 render.ts，评审壳双 iframe 跑真 ui.ts（演示库主密码 demo），自检 26 断言，以便迭代一处生效无同步轮。（2026-09-09，issue 251，ADR-0110；行为单源第八域）
...
26. 作为用户，我希望已读文章在目录中保持原位（打开/显式标读/收藏都不重排，只变色与更新计数），且「打开即已读」在桌面端同样生效，直到重开剪藏本面板才把已读收进「已读」段、已剪藏收进「已收」段，以便顺读时不被重排打断。（2026-09-08 拍板，issue 249，ADR-0108：会话冻结序；桌面源目录无，未读时默认展开已收/已读兜底不空场）
...
- smartcat 心情门控（ticket 095，ADR-0042，086 v4 方向四「限范围修：输出维度换」）：平静期不降搭话频率只换表达——门控输入 = analyzeEmotionTrend 的 EMA valence（趋势漂移，非瞬时 PAD）；窗口采样器固定挂既有 60s PAD 衰减循环（MoodSystem.onDecayTick 钩子）+ 30 分钟趋势心跳（不新建循环，采样最小间隔 10 分钟去重），判定 = 窗口内多数采样低于阈值（进 ≥3 样本且最近 ≤5 条低值严格多数 / 出最近 3 条非低 ≥2，防抖替代 v3 hysteresis）；quietMode={on,since} 持久化 + 静默超时 48h 自动退出兜底；平静期 Bandit 选中臂映射温和话术子集（模板与 LLM styleHint 双路，任意臂都落子集，选臂与 reward 口径零改动）+ 主动间隔 2 天→3~4 天（默认 3.5，晨起可调常量）；每日 1 次温和问候豁免（纯本地语料按本地日历日去重：不计 proactive 计数、不标 pendingArm 不领 reward，与 Bandit 主动共享间隔/作息闸门并刷新 lastAt 占槽顺延——体验原则 1 打扰总量守恒每周触点 ≤2）；loadMoodState 接线（新鲜 <24h 合并持久化 PAD / 陈旧或无数据归中性基线防重启假情绪）；兼容冻结仅加 editingData.quietMode/gentleGreeting 可选字段，设置面板零新项

- smartcat 多路召回联想检索（ticket 096，ADR-0043，086 v4 方向一裁决 + H3 前置重建）：**H3 前置**——EMOTION_VAD 补 curious/sleepy/playful/focused/upset 五类（'upset' 共振差量=0 现网 bug 解除）、emotionAffinity/vadAffinity VAD 连续距离评分（余弦 [-1,1]，'相反'=负距离，废 8 标签硬匹配）、reflect 证据池 LLM 情绪追标 emotionBackfilledAt（evidenceTop 窗口内无 emotion 观察一次批量追标；只补不覆盖/失败裁剪不整轮失败/独立退避 5min→30min 与反思退避分离/H4 边界继承）、emotionDensityStats 密度指标只汇报不阻断（「未达标不宣称三路」）；**方向一主体（槽位保留制）**——retrieve() topN=10 与三处调用点冻结、GA 公式权重不动，≤6 收缩只落 formatMemoriesForPrompt 可选 maxEntries 参数（聊天 retrieveMemories/主动关心两注入点传 PROMPT_SLOTS.maxEntries=6，不传保持全量行为），selectSlotMemories 纯函数：语义 ≤4 席 + 情绪 ≥1（|vadAffinity(记忆emotion, 当前PAD-VAD)| rerank 修饰非硬过滤，同向反向皆可）+ 时间 ≥1（周年 score2>星期几 score1 两类强锚点，小时粒度砍掉——与 recency/作息画像冗余），剩余名额 GA 序回填、保序去重；空 query 显式退化 recency+importance 现行为（代码注释 + ADR §O5）；ADR-0043 写入三路权重归一化公式 S_final=(w_sem·GA+w_emo·|aff|+w_time·anchor)/(w_sem+w_emo+w_time)（默认 0.70/0.20/0.10 晨起可调）与路由权重上限（w_emo≤0.35 且 w_time≤0.25——非语义两路合计不过半）；兼容冻结仅加 MemoryStreamEntry.emotionBackfilledAt 可选字段
- **复习/做题门面三改（ticket 099，grilling 定稿 2026-08-26）**：① **多选标签删除**——`renderModal` 的 `.quiz-multi-badge`「多选」徽标与 `.quiz-multi-hint`「本题为多选题，可多选」提示条整体移除（含 styles.css 与 tests/quiz/ui.test.ts「ticket 098：多选 UI」断言同步反转为「无徽标/无提示条」）；多选交互（切换选中+提交答案）与计数逻辑（ADR-0044 解冻点）零改动；② **待重做 split 崩溃修复（Q7 丙双保险）**——根因：`regenerateQuestions`（review/app.ts）返回题缺 `notePath`，`renderModal` 首题 `q.notePath!.split('/')`（quiz/ui.ts:314）报 `Cannot read properties of undefined (reading 'split')`；修法：`regenerateQuestions` 与 `batchGenerateQuestions` 对齐补 `notePath/_index`（甲·治本）+ renderModal 判空降级（乙·防御）；③ **重命名/移动自动更新**——`onVaultRename` 删除 confirm 弹窗改自动 `updateFilePath`（Q10，「不更新 → 排除名单」分支删除）；④ **监听文件夹交互改造（Q11）**——设置页「＋添加监听文件夹」改打开文件夹选择弹窗（review 域仿 attach `FolderSelectModal`，`bz-review-*` 类名）→ 选择后立即弹「N 篇未加入」确认（`confirmBatchAddForFolder`：确认 → 批量加入+添加 chip；**取消=什么都不做，不写排除名单**）→ 文件夹行改 chip+关闭标签；**打开复习面板时的批量收编确认（promptBatchAddAll/promptBatchAddForFolder/batchPrompted）整体移除**；增量自动加入 toast「已自动加入复习计划：X」保留（Q12）；⑤ 数据格式/命令 id/ADR-0044 重做语义零改动。**Q9 撤销项**：用户拍板「待重做自动抓题」整体不做（只修 split bug）。
- smartcat 数据面板展示层升级（ticket 097，纯展示层+口径统一，零数据写入改动）：成长轨迹行归因徽标「LLM 归因/词法推断」（091 attribution 只读消费；llm 且 quote 非空才显引用原文截 30 字，lexical 一律无解释文案——不产伪解释）+ 总览英雄区安静陪伴 chip（quietMode.on 才渲染不留占位）+ 情绪页标注覆盖率小字（复用 emotionDensityStats；观察样本 <5 条只报条数不显百分比）+ 感情卡依恋切 lazyAttachment 读侧分离衰减视图与总览 computeDashboardStats 口径一致（hint 注明读侧视图不写盘）+ 洞察行主题 chip（theme 受限枚举校验）/supersededBy 非空整行降透明度加删除线徽标「已被推翻」/pinned「已固定」并存时优先显示（固定·废弃按钮行为不变）+ 手动刷新按钮删除（smartcat-dash-refresh id 留档移除）改 vault modify 命中 smartcat.json/memo.json 防抖 3s 静默自动刷新（保持当前页签零 toast、失败保旧画面、close 全清监听与防抖计时器、幂等重开无泄漏）；新样式 bz-sc-dash-badge-attrib-llm/-lexical/tl-quote/chip-quiet/badge-theme/memory--superseded 收敛域内 styles.css
...
### 影院交互回归原型 + 全域筛选「再点回全部」统一（issue 208，2026-09-06）

> 用户截图反馈：影院类型树三角/再点收起、表单观影日期、豆瓣按钮、在看快捷面板；并拍板全域筛选统一为待办范式——「全部」保留加图标，再点选中项跳回全部。

- **影院类型树**：去组行尾三角（回归原型「二级展开无三角」）；再点选中组 = 取消回全部 + 二级收起；类型/状态「全部」行头补图标；移动 chips 同步（含再点取消）。
- **影院表单/日期**：表单删观影日期字段（禁改）；创建/状态变更自动刷当前时间（仅状态流转刷，改评分/影评不刷；右键标记同步）。
- **影院详情/卡片**：详情弹窗去「豆瓣页面」按钮（链接行保留，打开走右键「在豆瓣打开」）；想看/在看灰字改纯展示，快速状态窗退役——状态流转唯一入口 = 右键菜单 + 编辑表单。
- **网格列数设置**：cinemaGridColumns 默认 5 / bookshelfGridColumns 默认 6（string 键数字，钳 2~12），CSS 变量消费，移动端媒体查询列数不受影响。
- **全域统一**：书库状态/分类行再点取消；剪藏本源行再点回全部未读；文献盒「全部」按钮加图标；待办/收藏本/归物本/回忆墙已达标不动。
- **core 清理**：components.css 的 .bz-rail-caret/.sub-open 死规则随唯一使用点退役。

### 待办面板皮肤三选一 + 设置页顶部卡片选择（issue 210，2026-09-06）

> 5 套风格原型选 2（纸感手账/编辑部）+ 默认，做成待办设置页最顶部的视觉卡片选择（无编号无描述）。

- **机制**：面板根挂 `bz-todo-skin-*`，作用域覆盖 `--bz-*` token + 局部结构覆盖；设置键 `todoSkin`。
...
### 首页入口按端独立排序/隐藏 + 域快捷菜单 + 设置面板侧栏七组（issue 267，2026-09-10）

> 用户四项点名（首页域外观组置顶 / 拖拽组下移 / 去掉「已隐藏」改排最下面 / 侧栏分得更细）+ 追加一项（移动端域设置页去右上角关闭钮），快速原型模式逐轮确认后走「同步」收尾。
> **§1 入口顺序与显隐 = home.json v3**：`{version,desk[],mob[],hiddenDesk[],hiddenMob[]}` —— 顺序与隐藏都按端各一份，桌面删的域不会从移动端消失（v2 单一 `hidden` 两端共用是 bug，读取时两端各继承一份迁移；写盘提交整份、本端编辑不动另一端）。**修 `reorderTo` 下标口径**：新增第 4 参 `hidden`，下标只对可见域计数（旧实现按完整序列解释，有隐藏项时拖拽落点整体错位），落盘 = 新可见序列 + hidden 追尾。
> **§2 入口编辑器浮层弹窗 → 面板内联**（新增 `src/home/entry-editor.ts`）：设置面板「首页 → 入口」组 custom 行挂载，无弹窗、无「编辑入口…」按钮、无「命令面板」说明行；拖动排序（触屏先按住 ~200ms 再拖）+ 点 × 移除，**移除的域不分区、无「已隐藏」标题**，就地排到同一列表最下面（`.bz-home-ent-row--off` 弱化 + 拖柄 `visibility:hidden` 不参与排序 + 点 + 加回），`.bz-home-ent-sec` 全退役。
> **§3 首页域拆两组**：组[0]「外观」（issue 246 范式：布局「活动河」+ 主题「米白」`layoutKey homeLayout` 联动，占位键 `homeLayout`/`homeSkin` 默认 `default`/`cream`）置顶、组[1]「入口」在下。没有 custom 行时本域会被 `listableDomains` 滤掉（button 行不计入项数），别再换成 button 行。
> **§4 域右键菜单 / 长按抽屉**（同分支早先落地）：走 `core/item-actions` 的 `DOMAIN_MENU`，只放域自己的快捷动作（不放「打开 X」——入口本身就是打开；不放「整理顺序」——排序改直接拖拽），**本表没有条目的域不挂浮层**；抽屉盒头上排 = 彩色图标 + 域名同行、下排灰字，作用域 `.bz-home-menu`（经 `sheetClass` 通道传入）；番茄钟文案随专注态动态。
> **§5 设置面板**：`NAV_SECS` 重分七组（基础 `global/appearance/home` · 智能 `ai/secondbrain` · 记录 `diary/memo/belongings` · 收集 `clipping/favorites` · 媒体与阅读 `cinema/bookshelf/review/knowledge` · 工具 `pomodoro/smartcat` · 安全 `encrypt/password-vault`），18 可见域全在表内、无「其他」尾组，桌面导航与移动列表同源同序（共用 `groupDomains`）；**移动端域设置页去掉右上角关闭钮**（域页已有一枚返回钮弹回首页，再叠一枚会并排、误触率高），全 DOM 只剩首页页一枚。
> **§6 原型自检修复**（本轮主要工作量）：设置壳自检此前从 48 项处 `ERR ... reading 'querySelector'` 崩掉、**移动端整段从未跑过** —— 根因三个独立问题叠在 dir-picker 那一节：① 首页域那节之后没切回日记本域（面板停在首页页 → 永远找不到「日记目录」行）；② `bz-sp-path-btn` 只在**空态**渲染（有值走 chip；种子已给 `diaryDirectory` → master 上就崩在这里）；③ 种子缺 `homeLayout`（`layoutKey` 联动把主题单卡滤空）。顺带修既有陈旧段：`.bz-sp-mob-modal` 全表已不存在（M1 早改成**推入成页**，标记 `bz-sp-mob-pushed`）、`diarySkinTheme` 种子 `ivory`→`gallery`（ADR-0115 正名）、AI 徽标断言 9→5（采样参数组 4 行 2026-09-08 退役）。结果设置壳 **82/82**、首页壳 **70/70** 全绿。
> **§7 行尾根治**：`.gitattributes` 此前只固定 `prototypes/**/*.js`，**未覆盖原型侧 `.ts` 种子**（`fake-sim.ts` / `fake/*.ts`）—— 这批文件被内联进产物，而**源指纹按磁盘字节算**，`autocrlf=true` 下 worktree 检出 CRLF、主仓 LF → 换个位置重出就刷一遍指纹行（6 个壳受影响，「构建位置无关」的承诺对这批文件失效）。补 `prototypes/**/*.ts text eol=lf` 并把 26 个种子的工作区重整为 LF，实测产物连续两次重出逐字节一致、指纹与构建位置无关。
> 门禁：vitest 257 文件 **4153 用例全绿** + tsc 0 错 + 双壳自检（82/82 · 70/70）+ 新鲜度守卫 24 例 + `pnpm run build` 部署完成。无新 ADR（可逆域内改动 + 沿用既有范式，沿 issue 254/262 先例）。
...
### 知识盒四修：预览双份 + 标题陈述句 + 滚动条收敛 + 录入自动回填（issue 275–278 / ADR-0122，2026-09-11）

> grill-with-docs 十七问拍板（用户口语「第二大脑域」经核实实指**知识盒 knowledge 域**；第二大脑域只有检索/对话/自动双链，无文献预览、无出题——域归属纠偏写入 ADR-0122）。四票代码均**暂不动**，先出文档：issues/275–278 + ADR-0122 + CONTEXT 三词条 + 手册滚动条节重写。
> ① **文献预览正文重复两遍（275）**：真因 = 弹层骨架先把正文以纯文本段落预填进 `#bz-kb-preview-body`（ui.ts:553），随后同一容器跑 `MarkdownRenderer.render`（:563）——真机 render 是**追加**语义不清空容器，故纯文本副本（含字面 `![[…mp4]]`）+ 真渲染副本共存；:566 兜底判定因预填必然有子节点而恒假（死代码）。修法：容器初始为空 → 渲染前清空 → 仅抛错/无产出才回退纯文本（空正文「（无正文）」分支显式保留），笔记内容与嵌入位置一律不动。**测试防回归为同源必修**：共用 mock（mock-obsidian-entry.ts:48 `el.textContent = md`）与原型 fake 渲染器（fake-obsidian.ts:138 `innerHTML = …`）都是覆盖语义、双双掩盖真机行为 → 一律改追加语义 + 知识盒预览加「正文只出现一次」断言；知识盒壳补最小 `?selftest=1`（十二壳有、知识盒无）。
> ② **标题去新闻问句（276）**：用户所示「没有脑子的立方水母为何拥有六只相机眼」是文献笔记 frontmatter 的 AI title（note-gen.ts:120 prompt 明文允许「陈述句或疑问句」，B 站原题多为悬念问句）。**只改 prompt 措辞**：改「完整陈述句」+ 明列禁疑问语气（为何/为什么/怎么/如何/吗/呢），其余禁项原样；auto-summary/processor.ts:38 剪藏标题同源同病同步改。不加后置校验、不加 AI 改写兜底、不本地剥问号；存量标题不迁移。做题家选择题题干不在本次范围。
...
### 日记一目一文件迁移：条目独立成笔记 + core 契约单源（issue 304 / ADR-0130，2026-09-13）

> grill-with-docs 多轮拍板：522 个日期文件（2023-04~2026-08，1241 条）拆为 1241 个条目笔记，frontmatter 双属性 `日期`（YYYY-MM-DD HH:mm）+ `类型`（多值标签名列表），正文去一级标题原样保留（#人名行留正文、无 标签/星期/来源 属性）；文件名同刻冲突 `-2/-3` 后缀（库内 3 例），属性值保留英文冒号而文件名用 `-`（NTFS 禁 `:` 实测 ENOENT）。新增 `src/core/diary-format.ts` 契约单源（正则/命名/序列化/解析，消费方 diary/smartcat/recap/home/encrypt/path-classify），解析层不做旧格式兼容，frontmatter 损坏从文件名优雅降级（体检保持严格）。diary 写层重写为条目文件粒度（每路径串行队列 + 队内撞名复检），加密 SafeNote 块 v2 改标签名头（库内加密日记 0 条零数据迁移），小橘记忆流 ref 路径段迁移重写（定位符 `#HH:mm` 保留），recap 摘要写回改 addEntry+旧摘要清理，首页 streak 按日文件名前缀判定。迁移脚本 `scripts/diary-split.mjs` dry-run 默认，原日期文件归档 `归档/日记/`；须关 Obsidian 执行（防插件运行时竞写 smartcat-memory.json），先迁移后部署（解析层无旧格式兼容）。
...
### 知识盒录入四入口体验补齐：段落/图版快捷命令 + 关闭二次确认 + 生成后开笔记（issue 326，2026-09-15）

> 用户诉求四条：① 段落与图版一直没有快捷命令（影像/名词有）；② 段落命令与名词同构——编辑器选区自动进
> 输入框、当前笔记自动作来源（ADR-0116 模式），差异点是段落不自动生成（大段文字进面板让用户确认再生成）；
> ③ 四个入口界面（名词/段落/图版同壳三态 + 影像录入弹窗）有内容时关闭要二次确认，弹窗走 ADR-0125 风格化壳
> + 知识盒域皮（`confirmDiscard` + `kb bz-kb-flow-dialog`）；④ 名词/段落/图版确认写入落盘后关掉录入面板并
> 直接打开生成的笔记（openNote 自带收起主面板与队列面板）。脏判定两条路线：同壳三态按**值**判定（输入非空 /
> 有预览 / 生成中 / 图版有内存图；来源行不算脏——命令入口本来就预填来源，一开就关就弹是骚扰）；影像录入弹窗按
> **用户事件打标**（打开即自动重抓会程序化改写 url 与区间，数值比对必假阳；打标点 = 链接输入 / 区间条拖拽 /
> 时间框提交 / 分P / 档位 / 整片重置，RangeBar.set 只重绘不回调天然免疫）。影像批处理在后台异步完成，保持
> 手动打开笔记（行点击 / 动作按钮），批量连跑逐个弹笔记是灾难，刻意不做自动开。

### 知识盒录入关联行三改：确认写入不回退 / 分析不锁按钮 / 中断与后台分支（issue 327，2026-09-15）

> 用户拍板：① 确认写入**全程不动关联行**——写入阶段此前复用「分析中」loading 视觉造成「又在重新分析」的
> 误导（实际只是本地写 related）；② 关联分析期间不再锁定 重新生成 / 总结 / 确认写入（setEntryRelBusy 整套
> 退役；生成/总结自身的忙态锁保留）；③ 分析中点 重新生成/总结 = **真中断**在途预演——AbortSignal 从
> LinkBridge.preview 一路穿到 AI.ask，点下即断、行归「—」，新内容回来再起新一轮；④ 分析中点确认写入 =
> 面板照常关、笔记照常开，作废面板绑定的一次（省 token），后台重起 预演→apply，`notify` 同键动态通知
> （progress「后台分析中…」→ 结果原地更新）。预演 failed 的兜底 now 同样转后台，行全程不被确认写入触碰。

### 知识盒名词录入防重名：输入实时提醒 + 确认硬拦截（issue 328 / ADR-0143，2026-09-15）

> 用户最初诉求「四个入口都防止标题重复」，grill 后自我收窄：只有名词的标题是用户手输的——段落 / 图版 /
> 影像标题皆 AI 生成，「重复了重新生成即可」，刻意不防（重名静默 `_2` 并列维持现状）。名词侧两道防线：
> ① 输入框下方内联实时提醒（input 防抖查重，命中即显、改名即消，不阻断不锁按钮）；② 确认写入硬拦截
> （`onTermConfirm` 非空校验后查重，命中 error 通知「已存在同名文献笔记：…」+ 不写入，预览保留改名重试，
> 仿 cinema 同名拒绝先例）。口径 = **只查文件名**：`sanitizeMdTitle(term)` 与文献目录 md basename 比对，
> 与 `writeUniqueNote` 撞名判定同源——被拦的正是「不拦就会变 `_2`」的情况；frontmatter title 重复但
> 文件名不同（手动改过名）不拦，刻意接受。`writeUniqueNote` 永不覆盖契约不动，拦截全在调用方。

### 剪藏本划选工具框 × 知识盒直达 + 图版描述（issue 329 / ADR-0144/0145，2026-09-15）

> 剪藏本阅读器式划选工具框（桌面/移动同套）：文字选区浮「复制 Markdown / 存为名词 / 存为段落」，
> 单击图片浮「保存图片 / 存为图版」，动作直达知识盒录入（选区文字/图片/原文 url+标题预填，
> 生成后不自动开笔记）。划词锚定双链（名词/段落同机制）：原文字变 `[[笔记|原文字]]` 下划线链接，
> 剪藏本内点击直达知识盒文献预览（新 API 按 path）；不用块 id。落盘两路：已保存 md 直写；
> 未保存条目渲染层内存替换 + 侧写标记，保存为剪藏时物化进 md（此前保存图片的本地图同场换链）。
> 名词/段落/图版 source 两态：未保存=外链，物化时升级 `[[剪藏路径|标题]]`（新 API 幂等回写）。
...
### 设置面板 AI 页重新分组：三卡拆分 + Cookie 行换多行文本框（issue 331，2026-09-16）
