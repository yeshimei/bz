...
12. 作为用户，我希望打开 epub 书阅读时自动开始**独立读书计时**（与主番茄钟状态机解耦）：快照并挂起主番茄钟（endTime 冻结），另起端到端累计阅读时长；endTime 基准累计，Obsidian 退后台被节流/重启也不漏时，以便阅读计时可靠。
13. 作为用户，我希望关闭 epub 书（或换书）时结算读书累计、单独以 target.type=book 条目入读书历史，并**恢复进入读书前的主番茄钟状态**（跑中的专注/休息原 endTime 继续、时间不流逝），以便不打断原有番茄钟节奏。
14. 作为用户，我希望打开书时按场景决策：主番茄钟空闲/暂停/未运行 → 直接开始读书计时（免确认）；休息中/他处专注运行中 → 弹确认（选否保持原样、本次打开不再提示）；读书计时中换书直接切新书（旧书累计入账），以免误伤其他专注。
15. 作为用户，我希望读书统计显示「📚 读书 X 个 🍅」（今日结束、target 为书的读书会话数），以便统计读书量。
