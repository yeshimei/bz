...
**1.2.1/1.2.2 修订（2026-08-25）**：Q24 两次追加修订——「生成文献笔记」改底部常驻**快捷命令**并逐步扩为一键全流水：1.2.1 修复 `fmtPrec` 秒位显示 bug（`pad(ss%60)`）、按钮移底部；1.2.2 前置改为仅「已下载」，点击自动执行 **剪切/压缩 → 转文字（自动补跑）→ 交付 → AI → 写笔记**。更新已同步 spec.md / 工具 CONTEXT.md / 本 issue。
