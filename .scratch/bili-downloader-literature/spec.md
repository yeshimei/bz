...
- **状态**：五轮 grilling 共识（Q1–Q25，2026-08-25）已达成；**实现已完成**（工具版本 1.2.0，`npm test` 60 全绿，含缓存命中/文献笔记端到端）。本 spec 是唯一事实源——后续改动先改 spec 再改码。
...
| # | 决策 |
|---|---|
| Q24 | 「生成文献笔记」按钮在「完成」交付后启用；未交付先点 toast 提示「请先点完成」 |
| Q25 | 历史形态 = 交付条目追加可选 `note` 字段；不另增历史条目 |

## 4. 域模型（对齐 CONTEXT.md）

- **视频缓存 (Video Cache)**：下载原件的跨任务持久缓存；键 = {BV, cid, 清晰度}；超期清理；剪辑/压缩件不进缓存。
- **快速流程 (Quick Flow)**：转文字之后的一键后续——AI 生成元数据 + 轻润色正文，落文献笔记；视频走既有交付。
...
- 入口：交付结果区「生成文献笔记」按钮，`T.phase === done` 才可点；转录缺失或未交付先点 → toast 提示对应前置。
...
- 不做「转文字后一键全自动交付」。（Q24 拍板：按钮在交付后，交付本身仍手动）