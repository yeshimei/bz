...
- **状态**：五轮 grilling 共识（Q1–Q25，2026-08-25）已达成；**实现已完成并发布**（先 1.2.0：`npm test` 60 全绿；后 **1.2.1 跟进**：Q24 改快捷命令 + 修复 `fmtPrec` 秒位溢出显示 bug）。本 spec 是唯一事实源——后续改动先改 spec 再改码。
...
| # | 决策 |
|---|---|
| Q24 | 「生成文献笔记」按钮在「完成」交付后启用；未交付先点 toast 提示「请先点完成」🔁 **2026-08-25 用户追加修订**：改为**快捷命令**——按钮常驻底部（转文字旁），未交付时点击自动先执行交付（复用「完成」逻辑），已交付则跳过重复交付；前置仅「已下载 + 已转文字」 |
| Q25 | 历史形态 = 交付条目追加可选 `note` 字段；不另增历史条目 |

## 4. 域模型（对齐 CONTEXT.md）

- **视频缓存 (Video Cache)**：下载原件的跨任务持久缓存；键 = {BV, cid, 清晰度}；超期清理；剪辑/压缩件不进缓存。
- **快速流程 (Quick Flow)**：快捷命令——点击后自动执行 交付（未交付时）→ AI 生成元数据 + 轻润色正文 → 落文献笔记；已交付则跳过交付直走 AI。
...
- 入口：底部快捷命令「📄 生成文献笔记」（转文字旁），恒可见；`S.dur>0 && S.transcript` 才可点（前端 `updateGenNote`）。未交付时点击：前端先调 `POST /api/done`（复用交付参数 segments/mode/crf）自动交付 → 再调 `POST /api/note`；本任务已交付（`S.delivered`）则跳过交付直走 note。`/api/note` 服务端仍要求 `T.phase === done` + 转录存在（前置防线不变）。
...
- 不自动转文字（faster-whisper 慢且依赖模型，转录仍为「转文字」按钮手动触发；快捷命令只自动交付 + AI + 写笔记）。