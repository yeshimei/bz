...
## 用户故事 18：加密分类（ticket 67，ADR-0017，真·密文）

写日记弹窗**不提供**「加密」分类；仅在改类型（标签选择器）时提供。选「加密」→ 若保险箱未解锁先弹主密码 → 因正文将离笔记二次确认 → 确认后把该条（含 `# emoji HH:mm` 标题行与正文）整体移出 md，作为 `SafeNote(kind='diary-entry')` 存入保险箱 `safe.enc`（复用 `SafeManager.lockNote`，附件一并入库；不新增独立 .enc 文件）。

- 加密条目未解锁**完全不可见**（Q21-a）；筛选栏「加密」标签上锁态显示 🔒 无计数，点击弹主密码后恢复普通态（🔐 + 计数）。
- 解锁后解密成标准 `DiaryEntry` 混排进列表（🔐 角标），点击只读预览弹窗，**不跳 md、不提供面板内编辑**（Q17-b）。
- 已加密条目改类型 = 自动降级（Q20-a）：解密写回原日期 md（按 date+time 重插块，md 已删则新建），密文**取出即删**；附件随还原写回原路径，`![[原路径]]` 引用文本不变。
- 加密条目删除 = 永久销毁密文（removeNote）。
- 关面板即上锁（固定行为）；保险箱面板过滤 `kind==='diary-entry'` 不显示日记加密。