...
1. v1 候选范围限定**文献盒内部**互链（跨盒留待后续阶段，管线不变仅换候选过滤条件）；
...
- 监听 `文献盒/*.md` 的创建（v1 不做正文大改重跑；兜底靠手动命令）；
- 防抖聚合约 1 分钟内的批次；
...
- 取新笔记向量在**文献盒范围内**做近邻检索，Top-K = `linkAgentTopK`（默认 8）。
...
- ⚙️ 弹窗内 `linkAgentEnabled` 为**明细设置的显隐开关**：开启时下方展开明细行（TopK / 上限 / 完成通知 / 自动清理）；关闭时明细整体隐藏；
- 显隐即时生效（toggle onChange 重渲染该区块），各键独立持久化，重开弹窗按当前状态还原；
- 显隐属功能性显隐，样式走既有 settings group 组件，无新 CSS。

另为实施前提（用户配置变更，非代码）：`secondBrainAllowPaths` 加入 `文献盒`，插件在开关开启但范围缺失时给一次性引导提示，不代改用户配置。
...
- `bz-secondbrain-rebuild-links`：对当前笔记重跑一次关联（大改后的兜底入口）。
...
跨盒互链（纳入卡片盒）｜晋升流程｜复习计划联动｜书库/词语纳入向量库｜正文大改自动重跑。
