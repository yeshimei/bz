...
状态：已评审定稿（2026-08，六轮设计交流 + 用户逐项拍板）；v1.1 增量（ticket 115：启动存量补链 + 批量补链命令）含内
归属域：secondbrain
...
- 目标清单：`linkAgentScopes` 范围内全部 `.md` 存量笔记，剔除 ①已含 `related`（进度检查点，下次启动自动跳过）②encrypt 锁定文件 ③队列内待重试条目（避免重复算力）；
...
- 范围外不处理：候选仍按 `linkAgentScopes` 过滤，与 v1 一致；`related` 已有者靠 `bz-secondbrain-rebuild-links` 逐篇重跑。
...
- `bz-secondbrain-link-all`（v1.1/ticket 115）：对关联范围内所有缺 `related` 的存量笔记批量补链——启动自动补链的显式兜底。与启动路径同实现（`backfillMissingLinks`），仅增加结果通知：完成（含新建条数）/ 无待补链 / embedding 不可达 / 开关已关闭。
...
- [ ] 数据层 + UI/通知层 Vitest 覆盖（队列 CRUD、幂等、清理逻辑、开关行为、补链目标清单与串行锁），smoke 同步；
