...
状态：已评审定稿（2026-08，六轮设计交流 + 用户逐项拍板）；v1.1 增量（ticket 115：启动存量补链 + 批量补链命令）+ v1.2 语义修订（ticket 116：范围只管目标侧 / 候选来源 = 白名单索引库 / 两目录字段默认空、空=什么也不录）含内
归属域：secondbrain
...
- 监听 `linkAgentScopes` 各目录下 `.md` 的创建（v1 不做正文大改重跑；兜底靠手动命令）；空范围 = 不触发任何监听（什么也不录）
- 防抖聚合约 1 分钟内的批次；
