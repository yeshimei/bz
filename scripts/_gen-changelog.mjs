...
/* ☰ 版本抽屉开关：仅移动端出现（桌面版本栏常驻，不需要开关） */
.cg-menu{display:none;flex:none;align-items:center;justify-content:center;
width:40px;height:40px;margin-right:2px;border:0;background:none;color:var(--sp-ink-2);
cursor:pointer}
.cg-menu:active{color:var(--sp-ink)}
.cg-spacer{flex:1}
...
/* 标准移动端文档布局（用户 2026-09-26 拍板）：顶栏 = ☰ + 标题 + ✕ 三格同行；
   版本栏收成左侧抽屉，点 ☰ 滑出、点遮罩/版本收回；正文独占整宽。
   ✕ 由外壳给，定在顶栏中心线上（top:24px），故顶栏只需右侧留出它的宽度。 */
.cg-bar{padding:0 48px 0 8px}
.cg-menu{display:flex}
.cg-body{position:relative}
.cg-side{position:absolute;inset:0 auto 0 0;z-index:4;width:min(76vw,280px);
flex:none;flex-direction:column;overflow-y:auto;gap:2px;padding:12px 8px 24px;
border-right:1px solid var(--sp-line);box-shadow:8px 0 24px rgba(0,0,0,.16);
transform:translateX(-102%);transition:transform .2s ease}
body.toc-open .cg-side{transform:none}
.cg-scrim{position:absolute;inset:0;z-index:3;background:rgba(0,0,0,.28);
opacity:0;pointer-events:none;transition:opacity .2s ease}
body.toc-open .cg-scrim{opacity:1;pointer-events:auto}
.cg-nav{width:100%;padding:9px 10px;font-size:13px}
...
<div class="cg">
  <div class="cg-bar">
    <button class="cg-menu" id="menu" type="button" aria-label="版本列表">
      <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"
        stroke-linecap="round"><path d="M3 6h18"/><path d="M3 12h18"/><path d="M3 18h18"/></svg></button>
  ...
  <div class="cg-body">
    <div class="cg-scrim" id="scrim"></div>
...
function renderRail(){
  ...
  side.innerHTML = h;
  side.querySelectorAll('[data-v]').forEach(b => b.onclick = () => {
    cur = b.dataset.v;
    setDrawer(false); // 选完版本就收回抽屉，让正文立刻可见
    render();
  });
}
/* 版本抽屉（移动端）：点 ☰ 开、点遮罩关；桌面上这些类名与 .cg-menu 都不生效 */
function setDrawer(open){ document.body.classList.toggle('toc-open', !!open); }
function renderPage(){
  const r = BY_VER[cur];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('menu').onclick = () => setDrawer(!document.body.classList.contains('toc-open'));
