...
| smartcat 小橘（桌面宠物猫 + 笔记 AI 陪伴：气泡/聊天/心情/记忆流（ADR-0021，单层 GA 三因子）/动画/语音/跟随/书评；AI 走 bz core/ai，数据单 json（向量豁免 .vec），样式域内 styles.css） | smartcat.json（config/mood/personalityGrowth/emotionalMemory/timeEmotion/editingData/memory 记忆流）+ smartcat-memory-vectors.vec |
