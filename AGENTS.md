...
4. 样式写 `src/<域>/styles.css`，构建聚合至根 `styles.css`；**滚动条不自造**——bz 界面级单源隐藏（core 通杀，ADR-0122），域内禁 `scrollbar-width: thin/auto` 与自绘 thumb。
   **唯一例外（ADR-0199 皮肤包）**：远端皮肤包的 CSS 只能在 `src/core/skin-pack.ts` 的**单一注入点**生效。其余任何位置、任何用途的运行时注入 `<style>` 与内联视觉样式**依旧禁止**；例外不得扩散（有静态守卫测试），要用就得另开 ADR。
   （本条历史编号为「铁律 9」，ADR-0020 落笔时改写过编号，出处 ticket 60。）
