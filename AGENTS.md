...
5. 原型是该域 UI 的唯一真理源（详见 `docs/prototype-first.md`），差异即缺陷，修复只能改原型再让域追赶。域与原型共用样式、渲染与行为实现（行为经公共假层 `fake/`，ADR-0106，试点 belongings），内容一致性由自动化测试强制；迭代严格遵循“改原型→评审→构建”单向流程，禁止反向适配或私改。
