...
## encrypt（保险库）2026-09-19 体验线

> UI 决策制第二域执行：UI/交互类 8 项分流 `review-deep-ui-pending.md` encrypt 节；姊妹域（password-vault）共锁问题 6 条记 pv 域轮不动其文件。

### 归修复（对齐既定拍板 / 缺陷修复必要组成）
- **P1 加密日记资产入口恢复**（对齐 ADR-0158 拍板两资产）：`b86a20c4` 属无人值守越权拍板且 ADR-0158 未追认；存量老加密日记唯一救回路径已断，恢复入口=权威拍板回潮非新 UI 决策。
- **P1 移动端 @media 恢复**（issue 346 回归修复）。
- **搜索 ESC 清词+✕**（效率定稿范式对齐；壳收编 uiSearch 另行待拍板）。
- **列表滚位记忆**（clipbook 已落范式对齐）。
- **销毁防护对齐**：日记销毁升级重输主密码（同域笔记侧先例更严，取安全侧；「永久销毁不可逆」防护双档属缺陷）。
- **体检锁定态如实禁扫**（上锁被汇报为全绿属信息正确性缺陷）。
- **密码错误通知单条化 encrypt 侧**（双 toast 互相顶走）。

### 分流待拍板（8 项）
列表多选批量（eff S1）、设置入口可发现性（eff S2）、软删除撤销对齐（eff S3，不可逆密文销毁现双保险合规）、搜索壳收编 uiSearch（cons 新-17）、图标机制统一 iconSpan/mountIcons（cons 新-18）、pv 首设流程统一 core 三态（cons 新-7，随 pv 域轮）、工作台键盘可达性体系（eff 新-10，↑↓/Enter roving tabindex——全站「工作台类面板」范式未定，建议 ui-kit 立条后统一落）、旧建议 ux#8（自动上锁可调）/ux#9（忘记密码出口）/ux#10（双命令文案）/ux#11（解锁自动轻体检）重呈。

### 姊妹域边界拍板
encrypt 域轮**不修 password-vault 文件**（cons 新-6/7/8/9、N18①、eff 新-8 pv 侧、desc 对齐共 6 条记 pv 域轮）——避免域轮越界触碰姊妹域（b86a20c4 先例教训）；pv 域轮紧随其后，届时以上一轮审查结论直接作为输入。
