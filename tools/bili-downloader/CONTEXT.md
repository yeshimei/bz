...
**快速流程 (Quick Flow)**:
快捷命令——点击后自动执行 交付（未交付时先自动「完成」）+ AI 生成标题/标签/一句话简介并轻度润色转录正文，落一篇「文献笔记」；已交付则跳过交付直走 AI。笔记嵌入交付文件。
_Avoid_: 一键流程、AI 后处理
...
- **文献笔记一键直达**：「📄 生成文献笔记」为底部常驻快捷命令——未交付时点击自动先交付（复用「完成」逻辑），已交付跳过重复交付；前置仅「已下载 + 已转文字」；`标题.md` 重名加序号、永不覆盖；AI 失败即中止可重试，不落半成品。
