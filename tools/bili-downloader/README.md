...
- 🎙 转文字（faster-whisper）：**逐段实时输出 + 模型加载/进度计数/已用计时**（长视频不再静默等待），转录完成**自动复制到剪贴板**
- ✨ **长步骤进度反馈**：AI 润色有进度条 + 阶段文案 + 计时；页面**刷新后任务自动恢复**（解析卡/段落/转录/已交付结果，`GET /api/state`）
- 🪟 交付后可一键**「打开所在文件夹」**（资源管理器定位）；取消任务有**确认弹窗**防误触
- 🔁 **重复启动实例复用**：`~/.bilibili-dl-port` 探测到存活旧实例时只开浏览器、不重复起服务
