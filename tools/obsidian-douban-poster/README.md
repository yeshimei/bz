...
- 👁️ 监听影视文件夹的新建与改动，自动扫描待补的笔记（缺海报或缺豆瓣链接）
- 🕒 每 15 秒串行处理一个，避免豆瓣接口限流
- 🖼️ 下载高清海报（`l_ratio_poster`，约 200-400KB）
- 📋 补全豆瓣信息：评分、导演、编剧、主演、类型、地区、上映日期、片长、语言、又名、IMDb、简介
- 🔁 失败自动重试（指数退避，最多 3 次）
- ⏭️ 海报和豆瓣信息都齐全的笔记自动跳过，不覆盖手动编辑的数据
- 🩹 已有海报、缺豆瓣链接的笔记走补全分支：只补信息字段，不重下海报（避免孤儿海报文件与重复 embed）
...
**跳过条件**：笔记已有 `海报` 和 `豆瓣链接` 字段时跳过（watcher 扫描与 fetch 均适用，2.3.0 起扫描口径扩为「缺海报或缺豆瓣链接」）。仅有海报、缺豆瓣链接时仍会重跑，但走补全分支：只补豆瓣信息字段，不重新下载海报。
