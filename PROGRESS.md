...
**状态：已交付**（主仓库全量 4802 测试绿 + tsc 0 错 + 原型 CDP 自检 42/42；2026-09-14 合并并构建部署）

- [x] issue 309：录入入口收四枚名词（名词 / 段落 / 图版 / 影像，图版在影像前——复核改序）；
      段落录入与名词同壳（AI 自动标题）；属性区「关联」行 = AI 出内容即起跑第二大脑预演
      （preview/apply/now 三段显式通道 `core/link-now.ts`），删除 link agent 对 knowledge:tasks 的被动订阅
- [x] issue 310：影像入口直达录入界面（解析后才展开下半表单）；处理面板 660×480 不遮挡主窗、
      头部左计数/右图标组、历史并入面板内切换；emoji 全部退役改 lucide 图标；
      数据层 `read()` 非数组形状兜底（实测 `data.push is not a function` 事故）
- [x] issue 311：`core/ai` 多模态通道（`AIInput = string | {text, images}`，OpenAI content 数组 +
      base64 data URL；DeepSeek V4.1-Flash 原生读图；带图空闲超时 180s；纯文本报文零变化）
- [x] issue 312/313：图版 = 第四类录入（拖入 / 点选 / Ctrl+V 粘贴，**多图上限 9**，缩略图网格可删），
      读图成文 → 预览 → 确认写入（图先在内存、写入才落盘）；笔记正文**文字在上、图片在下**；
      图片目录设置键 `knowledgeImageFolder`（留空回落 `<文献目录>/assets`，设置行带 fallbackValue）

## Issue 307 — b23.tv 短链解析：落地页补全元信息 + 写回规范链接（ADR-0134）

**状态：已交付**（主仓库全量 4772 测试绿 + tsc 0 错 + 真网络端到端核对 + 原型 CDP 自检 16/16；2026-09-14 合并并构建部署）
...
- [x] 独立 review（子代理）：P2×1（非视频 B 站页 bvid 误提取会静默改写 URL，已修：og:url 只认 /video/ 路径段、
      state bvid 只从 videoData/viewInfo 取、needsBvidRepair 收口 backfill 与落库）+ P3×6 逐条收口
      （bvid-only 成功判据 / backfill 域与状态收口 / 切 P 档位查询补序列号判据 / state 边界与请求顺序用例 /
      写回保护与 meta.bvid 兜底隔离 / 短链超时上界写进 ADR）
- [x] 文档：ADR-0134、CONTEXT（录入元信息补短链路径 + 新增「短链解析」词条）、spec、issues/307
- [x] 合并回主仓库（99820b1b）→ 产物重出（bc2ecf63）→ 构建部署（主 main.js 08:31）
