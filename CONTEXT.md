...
**概念与实体 (Concept/Entity)**: 黑匣子的第一类输入——用户给出名词（如「提喻法」），AI 生成知识卡片（一句话或一段话说明它是什么），卡片间关联由 AI 自动构建，用户仅确认卡片是否合理、决定是否录入。**纯知识卡片，不带感触外壳**（无情绪标签/涉及的人等维度）。构成意识体的知识背景。
_Avoid_: 术语卡、名词解释

**文献笔记 (Literature Note)**: 黑匣子的第二类输入——用户从别处摘抄的信息片段，带来源；录入前 AI 分析其中可提取的概念/实体，与已有概念建立联系，给出名词表由用户勾选关联。
_Avoid_: 剪藏（剪藏本存整篇，文献笔记存精选片段 + 概念链接 + 个人感受）

**核心知识 (Core Knowledge)**: 黑匣子的第三类输入——用户自己的想法，可能在录入文献笔记时带出，也可能独立录入。三类输入中的核心层，第一、二类（概念与实体、文献笔记）是它的知识补充与背景。
_Avoid_: 思考笔记、随笔
