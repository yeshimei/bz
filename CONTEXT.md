...
**书库观察（ticket 081，ADR-0034）**：weave-data.json 数据文件监听（**数据文件监听先例**——阅读数据由外部 weave-epub-reader 落盘，bz 书库 UI 纯只读）——`DOMAIN_FILES.library = { file: 'CONFIG/STORAGE/weave-data.json', extract: libraryWeaveExtract }`（`src/smartcat/library-source.ts` 纯函数 diff，**extract 返回类型升级 `string | string[] | null`**）；五类事件（按书迭代、单次保存可产多条数组）：开始读（percent 首次 >0，进度本身不观察）/ 读完了（stats.completedTime 首次出现）/ 划重点（highlights 增 n 计数）/ 写想法（excerpts 增 n 计数）/ 阅读时长（sessions 新增 durationSeconds 求和向上取整分钟，最小 1）；prev 按 bookId 记账 `lib:<id>:started/done/hl/ex/sess`，标题取 meta.title，无标题的书跳过；首次快照只记状态不产出；`onVaultActivity` 对 kind==='reading' 短路（书库 md 通道停用，context-source reading 分支保留不触发，防双记录）。
