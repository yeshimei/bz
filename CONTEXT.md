...
**剪藏本移动目录 (Clipbook Mob Toc, ADR-0107)**: 剪藏本移动端第一屏目录化（issue 248）——章 = site（aggregateSites 桌面 rail 同源口径），未读/在读常显、「已收 N 篇」折叠成点线行可展开（arch.hidden 属性驱动，检索态命中全平铺不折叠）；源横滑条（mobstrip）退役（源级「全部标为已读」迁章头长按）；推翻 issue 214「移动端标准皮肤不动」——移动双屏同族编辑部皮（--clip-* 纸墨）。**m3 逐字覆盖修订**：顶栏刊名 + 「搜索/关闭」小字动作、章头不吸顶、无 44px 触控档（动作钮全衬线文字）、条目单行 flex（点·标题·[在读]·时间）、详情屏2 逐字（文字返回/「站名·目录」/文字保存钮/期次行/纯字状态章/读下一则脚）；打开笔记入口走条目长按抽屉。折叠态模块级记忆（详情往返不丢，面板重开复位）；刘海安全区保留。
