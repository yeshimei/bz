...
**影院 (Cinema, ADR-0087 接管影视)**: `src/cinema/` 域，命令 `bz-cinema-open`/`bz-cinema-add`/`bz-cinema-analysis`（影视分析报告 = 覆盖影院面板的一层《观影分析》26 幕，ADR-0174 / ADR-0175；层框 = 面板矩形、点遮罩 / ESC 关闭、桌面无关闭按钮、移动端保留；ADR-0090 的内嵌分析页与 issue 405 的滚动放映室两稿均已作废删除）/`bz-cinema-random-pick`「随机抽一部」（想看池空回落全量）。目录 = cinemaFolderPath（显式配置）→ 回落「我的/影视」；frontmatter 契约（tags 类型 / 评分 -1 想看 0 在看 >0 已看 / 观影日期 / 影评 / 海报 / 豆瓣字段 / 片长 / 季集）与旧 movie 域及外部 douban-poster watcher 完全一致。**ADR-0087 承接**：找同类 AI（详情弹窗按钮 → 页内结果）、补发 `movie:` 域事件（created/status/rated/review/deleted，smartcat 行为流观察依赖，载荷对齐 MovieActionEvent 契约）。AI 页标题固定「AI 荐片」（找同类结果同页呈现，M.aiTitle 字段已随六域单源清理退役）。分析页统计卡「已放映」=「已看」条数的午夜场风味同义词，口径同一。设置收敛设置面板「影院」tab。**未打类型 tag 的条目自动归类**（`type-decide.ts`，issue 393 / 395；回落口径见 ADR-0181）——Jev choice 从闭合词表（`constants.ts` 的 TYPE_GROUPS）选一个，弃权（哨兵 / 置信度不足）即留空由用户手点、**取值越界算畸形走回落**；**Jev 不可用一律回落单次 LLM JSON**（闭集校验），两道都不可用不阻断解析（字段已到手）。**剧集按季合并（issue 376、387 / ADR-0168）**：设置面板「影院 → 显示 → 剧集按季合并」（`cinemaMergeSeasons`，**默认开**，即时生效）——同一部剧的各季在网格里合成一张卡（海报**左下角一排季圆点**：金实=已看 / 橙实=在看 / 空心=未看，一个圆点=一季，**不出注释文字**、不占卡片高度），鼠标移入圆点即**换脸预览该季**（海报 + 名字/meta/星级，离开复原；issue 377）。**特别篇并入**（387）：片名 = `<剧名>` + 分隔符 `[\s:：·\-—－]+` + 副标题的条目（电影版/特别篇/外传，如「神探夏洛克：可恶的新娘」）并入该剧合并卡，**只按前缀认**、**「之」不作分隔符**；不占季号不进圆点、不再单独出卡，评分与聚合状态含它们。合并卡：**左键**开各季明细弹窗（季在前、特别篇顺其后，无分隔标题，头部计数按并入条目类型写「共 2 季 · 2 部电影」）；**桌面右键 / 移动长按**出跟手菜单/底部抽屉，只有一条「查看全部」；弹窗内**每行**右键/长按出该行动作（先收弹窗再执行）。合并判定单源 = `src/cinema/seasons.ts`（名称剥掉「第X季 / Season N」后同名 + 剧集/动漫组 + 库内 ≥2 季；`S1`/`第N部` 不认），**纯渲染层分组、零存储零写盘**，关掉即逐季一卡；rail 与头行计数按**卡片数**，分析页仍按笔记条数。
**观影分析 (Yearbook, ADR-0174 / ADR-0175)**: 影院域观影分析的实现形态——**覆盖影院面板矩形**的一层长片（命令 `bz-cinema-analysis`「影视分析报告」；层外遮罩点关、ESC 同义，桌面无关闭按钮，移动端面板满屏故保留按钮；基字号随层框派生 `clamp(12, 12 × min(w/900, h/620), 19)`，面板固定尺寸不跟窗口跑），26 幕、一幕一屏；数据只认笔记 frontmatter 里真有的 14 个字段（片长/季集/热门短评在内），演示数据由 `scripts/_gen-cinema-demo.mjs` 从真库全量导出；翻幕 = `scrollTop = 幕号 × 屏高`（换幕走过片遮片，合上那一帧换，issue 407 批），表演按时间轴自演（进场编排 → 常驻呼吸 → 定格），一次滚轮手势只翻一幕；配色纸/墨/朱/靛四色随 Obsidian 明暗。**指针层（issue 408）**：引擎把指针交给**当前这一幕**（`move/down/up`，客户端坐标 + 归一化 -1..1），命中判定走 `elementFromPoint`（不逐帧量 rect），浮签全片共用一个节点、挂在整屏根，翻幕/离场统一收；26 幕分别做跟手（日晷/河流/散点/领奖台/板墙）、悬停读数（日历/串珠/分箱/印章/海报/散点/短评/矩阵）、联动（类型条↔星图、双峰同档、压暗其余）与动词（尺子读数/按住天平/弧线站出来/扇面摊开/卡片倾斜/翻页钟倾斜/弹幕减速）。实现 `src/cinema/yearbook/`（data/scenes/motions/engine/kits）。_Avoid_: 观影志（上屏文案已改名「观影分析」，代码域名词仍留 yearbook）、分析页（ADR-0090 的内嵌统计页已退役）、放映室（issue 405 的滚动抽帧稿已作废）
**游戏库 (Gameshelf)**: `src/gameshelf/` 域（显示名 2026-09-17 由「游戏架」改为「游戏库」；域键/命令/设置键仍是 gameshelf）（issue 368 / ADR-0161，2026-09-17 建域；UI v3 见 issue 370；入口落位与首页快捷命令见 issue 371），命令 `bz-gameshelf-open`「游戏库」/`bz-gameshelf-sync`「同步游戏库」/`bz-gameshelf-stats`「游戏库数据统计」——Steam 直连自动拉库的游戏收藏墙：用户只填 SteamID64 + Web API 密钥（设置面板「游戏库」页），库存/时长/最后游玩全部自动同步，零手动登记；数据 = `我的/游戏/*.md` 一作一笔记（影院范式，frontmatter 管辖字段 `AppID/游玩分钟/最后游玩/封面/封面源/图标/图标源/截图/截图源/中文名/Windows分钟/SteamDeck分钟/Mac分钟/Linux分钟/有成就/成就/成就已解/成就总数/稀有成就/成就更新/截图更新/详情时间/同步时间/已下架`，AppID 为笔记身份，正文用户自由）；同步对账 upsert（用户数据零覆盖、库中消失标 `已下架: true` 保留不删）。**面板两视图**：游戏墙（门面 hero + 档位筛选/排序/搜索 + 封面网格，档位口径单源 = `report.ts` 的 `BUCKETS`）与数据统计（统计卡/时长排行/档位分布/最后游玩年份分布/平台分项/最近玩过/口径注记）；**详情弹窗**展示能拿到的全部数据（我的游玩数据 + 成就逐条明细含全球解锁率 + 商店资料 + 截图灯箱）；**详情弹窗属性优先、零网络**——全部数据都在笔记属性里（ADR-0166，取代 ADR-0163 的两级缓存）：`成就` 每行 **6 段**（显示名｜描述｜已解锁｜解锁日期｜全球率｜apiname，分隔符 `|` 且入库前净化；日期 / 全球率未知写 `-`——行尾空段会被 trim 吃掉；存量 8 段行的尾两段被解析忽略，见 ADR-0176）、`截图源`（远端，**始终写**，无截图写空数组作「查过、真没有」标记）+`截图`（本地路径）**同序同长**且下载失败位留空串、`成就更新`/`截图更新`/`详情时间` 作新鲜度与幂等标记；打开某款详情时属性过期则后台静默刷新该项（不做全库定时刷新）。媒体键分工见 ADR-0164：`封面源`/`图标源`/`截图源` 归**同步管辖**，`封面`/`图标`/`截图` 是**本地 vault 路径**由媒体队列写（同步绝不碰）；本地图三类 = 封面 `<appid>.jpg` + 库内小图标 `<appid>-icon.jpg` + 截图 `<appid>-shot-<n>.jpg`（取原图前 8 张，全库约 460 MB，见 ADR-0166 体量口径）；**成就图标不本地化**（issue 410 / ADR-0176）——它按成就逐条生成、每个成就两色，实测占 8,118 个文件，改由界面直取 Steam 远端 URL（不下载、不存路径，无地址画占位块），`成就` 行因此恒 6 段。口径诚实：Steam 无逐日游玩史，「最近玩过」按最后游玩日期取前 8（`GetRecentlyPlayedGames` 实测恒空）。国内网络 `api.steampowered.com` 需系统代理（requestUrl 跟随，报错文案分级指引），`store.steampowered.com` 直连可达，封面 CDN 独立可达。设置面板「游戏库」页组顺序 = 外观（布局「海报墙」/主题「墨黑」占位，issue 246 范式）→ 目录 → Steam；**首页入口菜单**（右键 / 长按抽屉，`home/shared.ts::DOMAIN_MENU`）= 立即同步（不开面板、keepHome，成功后补本地媒体、中文名与详情回填三队列）+ 数据统计（直开面板统计页）+ 末尾统一「设置」直达游戏库设置页（全域同款尾部项，见「内容首页」词条，issue 388）。**移动端两处差异（issue 372）**：窄屏不渲染门面（`.bz-gs-hero` display:none，门面版式只属宽屏），且搜索行是**结构上的固定**而非 sticky——窄屏把滚动权交给网格宿主 `#bz-gs-gridhost`（`#bz-gs-body[data-view="shelf"]` 打标才套，统计页仍整页滚动），工具行为独立固定层（头行已退役，issue 378）故滚动时纹丝不动；两层「网格」注意区分：宿主 `#bz-gs-grid`（.bz-gs-gridhost）与卡片网格 `.bz-gs-grid`（shelfHtml 渲染）。**头行退役·海报头顶格·悬浮预览（issue 378，2026-09-18）**：标题/N款整条去掉，大海报升级 frame 级常驻直接顶到面板顶（数据统计视图共用，面板恒有「脸」，高度 `clamp(180px, 28vh, 264px)`），常驻操作 = 海报右上角三枚图标钮（统计↔游戏墙 / 立即同步 / 移动端关闭，桌面关闭仍走点遮罩+ESC）；**悬浮卡片双预览**：门面换脸（背景+名字/原名/副行跟随悬浮款，口径标签隐去，`peekedHeroAppid` 同款防抖——mouseover 在卡内子元素边界连环触发不重写正脸——频闪病根之一；静息态 `heroRestHtml` 快照复原不重算）+ 卡片海报商店截图快轮播（0.3s 一张、只轮已解码就绪图不亮空框、远端 600×338 缩略图 404 回退原图、本地文件优先、零网络只读属性、触屏不挂，`shotReelHtml` markup 单源；`.bz-gs-reel` z-index:1 显式提层——封面 hover scale 变换层会按 DOM 序盖住 reel，桌面端轮播不可见的病根）；卡底条时长口径退役改**成就进度条**（宽度 = 已解/总成就，无成就页整条不渲染，数据经 `opts.achOf` 注入保 markup 纯函数），全成就小海报右上角挂裸金杯+深投影（`--bz-warning` 金档，下架章同位让位下移）；评审壳种子条数开关 `fake-sim.ts::SEED_LIMIT`（砍在数据源级——sync 罐头回放同读 `GAMESHELF_DATA` 且经 globalOf 回落父窗口，只砍种子会被自动同步灌回整库）。
...
**候选相似度下限 (Min Score, issue 330/ADR-0146)**: 自动关联的候选粗筛线（设置 `linkAgentMinScore`，默认 **0.30**，0 = 不过滤）——`findCandidates` 在范围过滤后、TopK 截断前把向量分数低于下限的候选直接剔除、不送 AI 裁判。分数落在**原始余弦 [0,1]** 这把唯一阈值尺上（issue 425/ADR-0185 起不再做 `score^0.35` 锐化，旧值经 `migrateLinkMinScoreScale` 一次性换算：`0.65^(1/0.35) ≈ 0.29` → 新默认 0.30）；**阈值只读余弦**——参考面板百分比自 issue 429 起对重排过的条目改显示重排分（`hit.rerankScore`），与阈值判定不同尺（重排是纯换序层，ADR-0186 §1）。三条触发路径与录入预演同源生效。_Avoid_: 高分直链（下限只拦低分，高分候选仍一律过裁判）、查询截断（那是 LINK_QUERY_MAX_CHARS 输入侧）、锐化分（旧尺，ADR-0185 已退役）
...
**凭据 (Credentials)**: 设置面板「数据源凭据」组（ADR-0133 收编为「AI 与凭据」组，issue 331 拆组定名）——集中管理非 AI 服务商的第三方数据源凭据：影院 ApiZero Key / 豆瓣 Cookie（原影院「数据抓取」组挪入）+ 知识盒 B站 Cookie；AI 侧另列「LLM」（服务商下拉 + 各家密钥 + 模型配置，issue 422/ADR-0182 合组）/「Embedding」（Ollama 本地 URL + 向量化模型——模型行 issue 422/ADR-0182、地址行 issue 423/ADR-0183 自第二大脑迁入；「移动端远程地址」行 issue 424/ADR-0184 删除，改由桌面端启动自动跟随本机 IP）/「JEV」（服务商 / 密钥 / 模型三行，issue 424/ADR-0184 收口：常开无开关、端点由服务商决定、超时固定十秒）三组。组内键各自归属消费域，仅陈列位置统一。**三行统一单行掩码**（issue 413/ADR-0180，取代 ADR-0147 决策 2 的 Cookie textarea）：一律 `type:'secret'`（password + 眼睛切明文），行序 Key → B站 → 豆瓣；多行掩码（textarea + `-webkit-text-security`）档位已随批退役。_Avoid_: 密钥组、外部配置（泛称）、多行加密框（已退役形态）
...
**第二大脑 (Second Brain)**: 笔记向量库的管理与检索功能（ticket 103 正名，前名「闪念」——QuickAdd《闪念.js》完整原型）：主面板统一入口（统计总览/来源分布/趋势/最近向量化/AI 一键概括）· 右侧窄窗（吸附缩起/悬停展开/参考卡拖出浮卡）· 向量检索增强（Ollama 嵌入，默认 bge-m3、可换 Qwen3-Embedding——**模型配置自 issue 422/ADR-0182 起迁入 AI 面板「Embedding」组**（含行内「获取模型」按 embedding 能力过滤，Qwen3-Embedding 走官方查询指令前缀）；**「Ollama 本地 URL」「移动端远程地址」两行亦自 issue 423/ADR-0183 随迁同组**（键不变）；**issue 424/ADR-0184 起远程地址全自动**——`ensureRemoteOllamaUrl` 自动跟随本机 IP（插件写下的值随 DHCP 漂移刷新，`secondBrainRemoteOllamaAuto` 记上次自动值做人机归属判定：等于当前值 = 插件管、跟着刷；人填的指向他机地址一律不动），「移动端远程地址」输入行与「本机局域网 IP」自查行（原挂「填入远程 URL」一键刷新）、移动端提示行随批全删——本域设置页「服务」组只剩「额外检索目录」（同批「检索」组四行删：段落最小长度 / 上下文限制不再限制，防抖 300ms 与轮询 500ms 固化为常量，四键退役）；**检索层自 issue 425/ADR-0185 收敛为精确全扫余弦**——VP-Tree 退役（近似召回漏最近邻、且 `cos=1−d²/2` 的距离换算只对单位向量成立）、分数即**原始余弦 [0,1]** 不再 `^0.35` 锐化、归一化矩阵按来源数组身份缓存（3192×4096 库全扫 ~27ms < 旧 VP ~100ms）；**交叉编码重排自 issue 427/ADR-0186 接入**——**列表整列重排（≤ 50 条，`RERANK_MAX_DOCS`）**——上限 = 「参考结果数 TopK」「对话参考结果数」两行的设置上限 1–50（`panel-settings-rows` 不变式测试钉住「两条 max ≤ 该上限」），故面板任何合法 TopK 都整列同尺、不出现「已重排头部 + 只有余弦分的尾部」混排；**超长列表（建链候选池等后台链路，可达 150）整轮跳过重排**——那些链路自己按分重排/取 max，重排不改其产出、只付 GPU 耗时，且半重排会让「整列同尺」失守（issue 429 review 收口）交 Qwen3-Reranker-4B（`RERANK_MODEL`）经 Ollama `/api/chat` 的 yes/no logprob 通道逐对串行打分（Ollama 无原生 rerank 端点；`think:false` 必带——社区转换版是思考模板，不关则首 token 恒为 thinking 标记、永远取不到 yes/no）；**纯换序层**——`hit.score` 仍是原始余弦（阈值判定不动；面板显示尺见下）；AI 面板「Embedding」组「启用重排」总闸（默认开；**issue 431/ADR-0189 起常显**——8B 门收窄给本地通道）+ 同组「重排走 Jev」开关（键 `secondBrainRerankJev`，默认关）构成**重排通道二选一**，运行期判定单源 `rerankChannel(): 'off'|'local'|'jev'`（设置行可见性与检索分流共用，替代原 `rerankActive()` 双头布尔；「行隐藏即不生效」原则以通道为单位各自成立）：总闸开 + Jev 关 = 本地通道（仍绑「嵌入 = Qwen3-Embedding-8B」门，ADR-0186 语义零改动）；总闸开 + Jev 开 = **Jev 通道**（issue 431/ADR-0189——`core/jev.ts` **noul 每候选一题**，state = 查询 + 编号候选清单、摘录截断、批上限沿用 `RERANK_MAX_DOCS`，`answers[key].noul` 直接记 `hit.rerankScore`；**不绑 8B 门、任何嵌入模型可用**，与 `secondBrainRerankModel` 无关——此时「重排模型」行隐藏；失败（未配密钥 / 超时 / 畸形 / **缺题键**）一律整轮回余弦序——经 `judgeOrFallback`、fallback 槽位 = 维持余弦序**而非 LLM**（ADR-0181 纪律下首个非 LLM 回落接入点），缺题不做单条计 0（与 link-agent 决策 9 有意分叉：计 0 会把相关条目沉底，正是要避免的半排观感）；取消直抛不回落（ADR-0187））；两开关 desc 静态写明各自门槛（本地需 8B 嵌入 / Jev 需 JEV 组密钥、未填自动回余弦），不做动态警示；**同组第四行「重排模型」（issue 429）**——行内「获取模型」走同款选择器（`pickRerankModels`：名字含 `rerank` 者优先，社区转换版没有 `rerank` 能力标签故**不按 capabilities 过滤**，一条都没有则全量返回让用户自辨），选中写 `secondBrainRerankModel`（默认 `''` = 回落内置 `RERANK_MODEL` 4B；**换重排模型不动向量索引**——重排是纯换序层，改完下一次检索即生效），可见性 = 8B 嵌入 ∧ 总闸非关 ∧ Jev 开关关（issue 431/ADR-0189 起 Jev 通道开启时本行隐藏——通道二选一；总闸关掉时一起收起，不留暗态设置）；失败（模型未装 / 无 logprobs / 超时 / 整轮预算 6s 用尽）**静默回退余弦序 + `console.warn`**，绝不打断检索链路；**取消例外（issue 428/ADR-0187）**——调用方取消（`AbortError`）直抛、不回退余弦序（回填旧序会盖掉新查询的列表）；**检索取消语义（issue 428/ADR-0187）**——参考面板与移动面板各持在途 `AbortController`，新查询发起或面板/抽屉关闭即**真中断**上一轮（signal 一路透穿 `store.search/searchMobile → vectorSearch → getEmbedding/rerankScores → Ollama fetch`），**列表态恒归最新一轮**；取消走 `AbortError` 静默收口：不落「检索失败」提示、不追加降级说明、不降级文本检索（取消与超时靠 `timedOut` 标记分开报，`core/abort.ts` 为 `abortError/isAbortError/throwIfAborted/linkAbort` 单源；全扫同步段每 64 篇设检查点、重排每对前设检查点）；**空行不再触发查询**（`getCurrentContext` 整行空白即返回空串，删除旧「回退上一行尾 300 字」语义）——桌面面板保持现状、移动端清空参考列表；**参考面板百分比与名次同尺（issue 429）**——重排过的条目另记 `hit.rerankScore`（P(yes)），列表卡与浮卡预览优先显示它，未重排/回退的条目回落余弦 `score`；桌面参考面板与移动端参考列表**共用同一函数**（`vector-store` 导出的 `relevancePct`，issue 429 review 收口——同一「灵感参考」功能的两端，尺不能各用各的）；**阈值仍只读 `hit.score` 余弦**，ADR-0186 §1 的单尺不破；对话面板百分比仍按余弦（本轮不动）；**取 4B 而非 8B**（12GB 显存下 8B 重排与 8B 嵌入无法共驻，每次检索多付 ~8s 换入换出，20 条重排 ≈9.7s 撞 SEARCH_TIMEOUT_MS 10s → 面板降级纯文本，比不重排更差），`num_ctx 2048` + 查询 1000 字 + 候选段 600 字封顶，与嵌入共驻 11.05GB/12GB（热态单对 p50 ~70ms）；**坏向量双重防线**：入库侧拒收零向量/非有限分量与条数维度不齐（整批失败交逐条回退、坏段不登记下轮重试），检索侧整行跳过无效行（存量库带病不阻塞，面板「重新索引」即清除）——2026-08-25 一次瞬时写入故障留下的 5 条全零行曾让「任何查询都命中同一批 78%」；库内 meta 记 `_model`，与当前配置不符即整库重建——维度不同（bge-m3 1024 / qwen3-embedding:8b 4096）不可混存；meta v9 段 + secondbrain.vec；ticket 110 起切块剥离 frontmatter、标题并入首块；ticket 120 起数据整合为**两文件**：`secondbrain.json`（meta/panel/link 三段 JSON）+ `secondbrain.vec`（向量二进制，原 secondbrain_vectors.vec 改名））· AI 对话（经主设置页 core AI 服务商，ticket 108 起统一；不再回退 Ollama 对话模型；issue 359 起域内残留 DeepSeek 单例 getDeepseekAI 已退役，createAI 不锁模型、跟随 AI 设置的 provider.model）。常驻监听光标移动与笔记变更。**引导态**（ticket 107）：本地无向量数据时三命令统一进主面板，首次向量化须用户点击按钮触发；**增量索引**（ticket 108）：打开面板时如有待处理变更，先以进度视图展示索引推进再进统计；**重新索引**（ticket 108）：设置弹窗确认后清空全库重嵌（区别于增量索引的 mtime 差异刷新）。**自动双链 link agent**（ticket 111 + 115 + 116 + 118 + 120；**ADR-0141 起正名「自动关联」并迁入知识盒域**——设置组与两条建链命令归知识盒、范围恒为三个盒子，引擎与向量索引仍在本域，详见「自动关联」词条）：**范围固定为三个盒子（`linkAgentScopes` 已退役），被处理端（落盘监听 + 存量补链 + 死链清理）与候选端（向量近邻召回）都限三盒，手动命令与知识盒显式通道亦不豁免**（ADR-0141 §2；ticket 116「候选取自白名单全库」口径作废）；建链检索**查询端用笔记全文嵌入**（ticket 118：剥 frontmatter 去空白，超长 8000 字安全截尾）→ 本地语义近邻召回候选 → 在线 AI 裁判择优（"只链实质关联，存疑不链"）→ 单侧幂等写 `related`（Obsidian 图谱双向呈现）；待处理队列与基准哈希并入 `secondbrain.json` 的 link 段（queue/state，原 secondbrain_link_queue.json / secondbrain_link_state.json 已由 store-file 一次性迁移合并，ticket 120）跨设备自动消费、死链自动清理；**存量补链**（ticket 115）：每次启动自动对三盒内缺 `related` 的存量笔记批量建链（`related` 即进度检查点；`link.state` 另记「已尝试且零条」者不再重试，ADR-0141 §6），命令 `bz-knowledge-link-all` 手动兜底，批次与监听共用串行锁；**正文大改自动重跑**（ticket 119/v1.4）：每次成功建链后把全文内容哈希记入 link.state 基准，范围内笔记被修改时按基准哈希过滤——**内容实质变化才重跑该篇建链**（Obsidian 高频保存/自写 related 触发 → 哈希相同 → 不空转；无基准的升级前存量首次修改视为变化重跑并重建基准）。**知识盒录入即跑**（issue 298 → 309 改显式通道）：知识盒录入面板在「AI 出内容之后、确认写入之前」经 `core/link-now.ts` 的 LinkBridge 调本域管线三段——preview（草稿未落盘也能算：近邻检索 + AI 裁判）/ apply（落盘后写预演结果）/ now（兜底单篇管线），**不等约 60 秒批次防抖**；先等索引装载完成再跑，执行经同一串行锁与批次 / 补链互斥；结果由面板就地呈现（通道侧通知静默）。原「订阅 `knowledge:tasks` 生成即跑」的被动路径已删（它不回报进度、与调用方写入结果无法对齐），原「目录不在白名单内一次性引导提示」亦随三盒恒含退役（ADR-0141 §3）。**ADR-0141 起范围口径**：索引语料 = **三个盒子恒含**（无需指定）∪ `secondBrainAllowPaths`「额外目录」（键保留、语义降级，值里的三盒条目 onload 自动剔除）——「白名单空 = 什么也不录」现仅约束额外目录；`.canvas` 亦按三盒抽取进索引（只作候选来源）。**每周知识动态**（issue 360）：每周（滚动 7 天周界，lastRunAt 与 knownPaths 快照存 `secondbrain.json` 的 weekly 段）静默聚合一次新增笔记 / 新增关联 / 主题撞车提示（向量**原始余弦** ≥0.63（旧锐化尺 0.85，issue 425/ADR-0185 换算）、降级 TF-IDF 覆盖率 ≥0.7 判「高度重合」），有实质内容才弹一条通知（挂「查看详情」）；详情弹层另由主面板入口卡（标「最近一份」区间）与头行图标钮回放，命令 `bz-secondbrain-weekly` 随时强制重聚；首轮只立基线不产出（knownPaths 快照差分判新增，空索引延迟立基线），空轮保留上一份摘要零打扰（存量摘要随盘留存、其**向量通道**分数是旧锐化尺——新摘要自带 `scale: 'cos'` 标记，详情弹层显示前对无标记者换算回原始余弦，TF-IDF 覆盖率本就绝对标尺不换算）。**Syncthing 冲突自愈**（ticket 152）：多设备各自 refresh 索引不同新笔记 → 两端真实分叉，Syncthing 必然保留 `secondbrain.sync-conflict-*` 副本（写前比对止血后仍发生）；store-file **每次读取时**扫描并自动收敛——JSON 段级 union（meta.notes 键并集取 mtime 大者 / panel 取 generatedAt 大者 / queue-state-chatHistory 并集去重）写回主文件、.vec 按合并后 meta 键序行级重排（行序不变式 = 键序 × chunks 数，meta 未变则主 .vec 直接复用），随后删除冲突文件；无同批 meta/维度不符/行不足 → 删向量走既有 indexIncomplete 全量重建（ticket 107 兜底，元数据仍在数据不丢）；损坏冲突 JSON 保留待人工处置。
_Avoid_: 闪念（旧功能名，仅存于「闪念笔记」文档类型语义）、AI 补全（ai_completion 时代旧称）

**远程 Ollama URL (Remote Ollama URL)**: 移动端连回桌面 Ollama 的局域网地址（设置键 `secondBrainRemoteOllamaUrl`，**默认空**——enh-sweep-a 起不再写死内网 IP）。移动端在无向量引导初始化与检索时优先用它（`initMobile` 探活成功后 searchMode='remote'；embedBase 移动端优先取它），不可达则降级 TF-IDF/文本。它是**桌面机当前的局域网 IP**，DHCP 漂移后需同步更新；非代码缺陷。**issue 423/ADR-0183 起**桌面端 onload 走 `ensureRemoteOllamaUrl` 自动写入、设置行迁 AI 面板「Embedding」组；**issue 424/ADR-0184 起升级为自动跟随本机 IP**——面板上「移动端远程地址」输入行与「本机局域网 IP」自查行（原挂「填入远程 URL」一键刷新）全删，改由插件按人机归属判定自愈：新键 `secondBrainRemoteOllamaAuto` 记上次自动写入值，当前值等于它即跟随新 IP；值为空、或空记录 + `http://<IPv4>:11434` 形态（存量插件值）亦接管；**人填的地址（含指向他机者）一律不动**。手机端恒不写、只读同步值。
...
**备忘录面板皮肤 (Memo Skin, ADR-0095)**: 备忘录面板两选一外观（设置键 `memoSkin`：paper 纸感手账/editorial 编辑部，issue 210 四轮默认风格下线），备忘录设置「外观」组（面板布局行之下）以 `choiceCards` 视觉卡片行选择（issue 210，预览+名称无编号无描述）。机制 = 面板根挂 `.bz-memo-skin-*` 作用域就近覆盖 `--bz-*` token + 少量结构覆盖；皮肤固定明度不随 Obsidian 明暗；面板与二级浮层（uiModal 弹窗/flow-dialog 流程框/右键菜单/长按抽屉）经 `skinClass()` 同挂 `.bz-memo-skin-*`（issue 291/ADR-0125 皮肤通道）；`applyMemoSkin` 支持已开面板热切换。组件库 `uiCardChoice`（.bz-cardpick）供其他域视觉选择复用。
_Avoid_: 域内复制组件库类族、新域自绘面板壳/头行、域内另起按钮/输入基线、手写 emoji 图标（一律 lucide 经 setIcon）

**布局 (Layout) 与 主题 (Theme)**: 面板外观的**两条独立轴**（ADR-0105 分层；issue 246 范式的设置面板外观组 = 上「面板布局」行 + 下「面板主题」行，均为 choiceCards 单选卡）。**布局** = 结构/版式变体；**主题** = 配色/明暗档。**皮肤一律指主题轴**（ADR-0199）。键名在代码里并不统一，读改前必查：布局用 `XxxLayout`（bookshelf/memo/home）或 `XxxSkin`（diary/clipbook/encrypt/favorites/knowledge/password-vault/review/secondbrain/cinema 的布局语义），主题用 `XxxSkin`（**bookshelf/memo 把主题放在这个键上**）或 `XxxSkinTheme`（其余域）。权威形状见 `belongings/ui.ts:150` 的选项卡 `{ value, label, layout, prevClass }`——`layout` 字段明示该选项挂在哪条布局轴下。布局轴现全为 `'default'` 单卡占位（`memo/settings.ts` 注释「布局维度待皮肤设计时接入」）；**主题轴单值域**（diary/clipbook/favorites/review/secondbrain/knowledge/encrypt/gameshelf/home/cinema/password-vault/belongings 等各 1 个值）= 内置视觉常量，**不构成「多套皮肤」**。
_Avoid_: 把 `XxxSkin` 一律当皮肤读、按键名推断属哪条轴、把「几个域有好几套皮肤」当成全域现状

**皮肤包 (Skin Pack)**: 皮肤（主题轴）的云端分发单元（ADR-0199，issue 475）——清单 `manual/skins/index.json` + 每皮肤一个 `.css`，条目含 `id / domain / name / 预览色值 / 版本区间 / sha256`；落 `<configDir>/plugins/bz/skins/`（复用 `assetVaultPath`），经 `src/core/skin-pack.ts` 的**唯一运行时注入点**生效（ADR-0020 的注入否决在此开例外，边界不得扩散，有静态守卫测试）。**只分发主题轴 ≥2 值的域**（smartcat 13 / pomodoro 10 / bookshelf 5 / memo 2，共搬 26 套），每域**首套恒内置**作兜底；单值主题域与设置面板自身皮肤**永不分发**。启动时静默拉取（**串在自更新检查之后**——自更新会改插件版本号，而版本区间校验用它；无 24h 节流），失败静默、等下次启动。未下载成功 / 已下架 / 版本区间外的皮肤**一律不进选择卡**；下架 = 清单移除 + 删本地文件，已选中者回落首套。
_Avoid_: 把内置首套也搬走、往单值主题域塞云端皮肤、在 `skin-pack.ts` 之外注入样式、往选择卡里放不生效的皮肤
...
**AIService / createAI**: Q3 的 AI 服务抽象——provider 由 **AI 提供商注册表**描述：`core/ai.ts` `AI_PROVIDER_REGISTRY`（**issue 411/ADR-0179 起只留三条在册通道**：`deepseek`（官方，缺省 `DEFAULT_AI_PROVIDER`）/ `zhipu-plan`（智谱 Coding 套餐）/ `ollama`（本地）；每家含 endpoint/model/密钥键/密钥行文案/默认 maxTokens/思考档位表），`getAIProvider` 查表解析（未知名回退缺省通道描述），新增提供商只需注册表加一行（设置页密钥行与思考档位自动生成）。其余 14 家（opencode-go / openai / anthropic / google / moonshot / zhipu / dashscope / siliconflow / openrouter / xai / groq / mistral / together）与 `custom`（自定义 OpenAI 兼容端点）连同密钥键、设置行、`aiCustomEndpoint/aiCustomModel/aiCustomApiKey` 一并退役——**用到了再按注册表加一行**；脚本内指定第三方端点仍走 override 对象（endpoint/apiKey/model/extraHeaders，不经注册表故无思考档位）。deepseek 保留 QuickAdd data.json 兜底；ollama 本地服务无鉴权空密钥放行；CORS 不友好的端点由「fetch 失败 → requestUrl 兜底」兜着（issue 412 起 desc 层无 noCors/extraHeaders 字段；对象 override 仍可自带 extraHeaders）。key 存于插件设置（`aiProvider`、`deepseekApiKey`、`zhipuPlanApiKey`、`ollamaApiKey`）。**per-provider 配置（ticket 172）**：`aiModelOverrides` / `aiMaxTokensOverrides` / `aiThinkingOverrides` 三 map（键 = provider id，空 = 回落默认；原 `aiContextOverrides` 随「上下文窗口」行删除退役）；设置页「模型名称 / 最大输出 token / 思考」三行默认按当前 provider（模型档位按当前模型名查 `core/model-limits.ts`：设置覆盖 > 模型查表 > 注册表默认，deepseek 系 384K 输出、智谱 glm-5.3 系 128K）、切换联动、清空回落。**输出上限护栏（issue 457/ADR-0193）**：上限是「模型」属性不是全局常量（智谱 glm-5.3-flash 131072 ≠ DeepSeek 393216），基准单源 `maxOutputCapOf()`（① 当前生效模型在 model-limits 的官方最大输出；② 查不到取注册表 `maxOutputCap`——deepseek 393216 / zhipu-plan 131072 显式声明，本地 Ollama 有意留空；③ 皆无 = 不设围栏）——面板「最大输出 token」行的输入上界按它动态取（`NumberRow.max` 可收函数，`resolveNumberBound` 为唯一求值口，两个渲染器共用、随 provider/模型名刷新），`getAIProvider` 出口再封顶（填了覆盖 → `min(覆盖值, 基准)`；未填 → 基准优先、无基准回落兜底档；0/负数/非数按未填）。注意 `maxOutputCap`（墙）与 `defaultMaxTokens`（缺省档，可被覆盖越过）是语义之分，别拿兜底档当上限——旧全局硬编码上界 200000 已废（对智谱越界、直送即服务端 400/1210 `max_tokens参数非法`，且 `prompt` 每次请求都带它，失败面是全域 AI 而非单次）。`prompt` 的 max_tokens **面板独裁（issue 334/ADR-0148）**：唯一权威 = provider 解析链（但受上述上限封顶），`modelOptions.max_tokens` 与 createAI 注入均已废除（传入即忽略）——调用点不再持有任何输出上限私有值；`AIInput` 另收 `{messages}` 多轮报文（smartcat 行为流）。主设置页 🤖 AI 区块：服务商下拉（注册表驱动）+ 每家提供商一行密钥（visibleWhen 互斥）+ per-provider 三行。_Avoid_: 逐家写死的默认 provider 字面量（统一 `DEFAULT_AI_PROVIDER`）、把注册表当「服务商全集」（只留在用通道）
_Avoid_: getAIProvider 内新增 if-else 分支、各调用点硬编码 max_tokens、新增提供商时改 schema/解析函数、把输出上限写成全局常量（如面板 max: 200000）、面板与解析各算一套上限

**Jev 决策通道 (Jev Decision Channel)**: 与「AIService / createAI」（**生成通道**）并存的**判定通道**（ADR-0173，2026-09-21）——TypeSafe AI 的 System One 模型 Jev 的接入封装：state + 类型化问题（Choice 从清单选一个 / Score 按量表打分 / Noul 判是或否）→ 类型化答案 + 校准概率，**不生成任何文本**，故不存在 JSON 解析失败。便于后来者检索——本术语在代码中的落点是 `src/core/jev.ts`（通道）+ `src/core/jev-fallback.ts`（判定编排，ADR-0181），键为 `jev*`。**知识盒内容生成不接入**——`note-gen.ts` 的 `domain` 与 `title`/`summary` 是同一次流式调用的产物，拆开会让属性区「分析中…」失去增量语义（ADR-0173 §8）。优先级 **Jev 优先、凡不可用一律当次回落 LLM**（未启用/未配置 → 不造材料直接回落；请求失败 / 答案畸形（题型不符、取值越界）→ 回落；回落对用户不可见，降级不通知不落盘标记）——编排单源 = `core/jev-fallback.ts` 的 `judgeOrFallback({request,parse,fallback,signal?,config?})`，**新接入点一律消费它**（ADR-0181：影院类型归类 `decideCinemaType` 即走此路，Jev 不可用回落单次 LLM JSON（接调用方 signal），回执按**闭集校验**顶替置信度门槛；issue 395 的影院「不回落」已作废）。**Jev 正常返回但弃权（哨兵 / 置信度不足）是有效判定，不触发回落**。两通道都不可用按域降级（关联入队重试 / 影院分类留空由用户手点），不引第三层兜底。**关联预演（录入面板「关联」行）与后台自动关联同通道、不拆参数**——`previewLinks` 与 `processNote` 一并走 Jev，仅预演的 `AbortSignal` 需透传（取消不算失败、不回落；**预取消时连 state/材料都不造**）。判定口径以 Jev 为准（关联变稀是接受的代价），**存量 related 不重跑**（有意接受的口径混血；影院分类同性质——不落盘标记产出方，复盘要靠 `console.debug`）。**issue 424/ADR-0184 起常开**：无总开关（`jevEnabled` 退役，`isJevConfigured` 只看密钥齐备——填了即接管判定，清空即回落 LLM），端点是**服务商属性**（`JEV_PROVIDER_REGISTRY`，issue 430/ADR-0188 起两家：Typesafe + **博查**——`jev.bochaai.com`，报文/模型列表与 Typesafe 同构、国内直连 ~0.2s、测试期限免；「Jev 端点」设置行退役），超时固定十秒（「Jev 超时」退役），模型默认**按服务商各配**（issue 430/ADR-0188：typesafe → `jev-latest`，博查 → `bocha-jev-v1`；行内「获取模型」拉当前服务商 `/v1/models` 自选，onload 迁移把旧缺省 `jev-1.13.0` 改写为新缺省——ADR-0173 §5「钉版本」的旧口径作废）。配置一组三行（服务商 / 密钥 / 模型）落在设置面板 AI 分区，接入点不设独立开关。**密钥与模型按服务商分存（issue 433/ADR-0190）**：`jevApiKeys` / `jevModels` 两张 map（键 = 服务商 id），密钥/模型行走三函数绑定 + refreshKey——切服务商两行显示值原地换槽、互不覆盖（旧全局键 `jevApiKey`/`jevModel` 已一次性迁入 typesafe 槽位后退役）；`isJevConfigured` 只看当前服务商那格，另一家配了不算。密钥行行内「测试」按钮（同 LLM 密钥行）发一次**真实**极小调用验证连通——成功 = 链路无抛错且回复非空，通知带耗时；**所配即所测**（先落盘防抖值再调）。在册门槛 = 与 SystemOne 报文同构，OpenAI chat 面的服务商（如硅基流动）先加适配层、不许直接进表。密钥行「测试」钮为三态反馈（issue 434：点击转圈、成功绿框显示已连通、失败红框显示简短原因如密钥无效；不弹通知，约两秒复原）。**第二大脑 Jev 重排（issue 431/ADR-0189）**是首个 fallback 槽位不填 LLM 的接入点——回落 = 维持余弦序（重排是增强层，失败只该损失增强；50 条逐条 LLM 在检索预算内不可能）；它带独立开关 `secondBrainRerankJev`——重排是检索链路环节，不受判定通道「常开」哲学约束，开关在 AI 面板「Embedding」组与「启用重排」总闸构成通道二选一。
_Avoid_: Jev 模型（泛指时）、AI 兜底通道（指备用 LLM 时）、Jev 备用通道
...
**取消语义 (Abort Semantics)**: 跨域统一的「主动取消」口径（`src/core/abort.ts`，issue 428/ADR-0187）——`abortError()` 造 `name === 'AbortError'` 的错误、`isAbortError()` 判定（禁字符串比较）、`throwIfAborted(signal)` 检查点、`linkAbort(outer, controller)` 把调用方 signal 链到内层超时 controller（**返回解绑函数，必须在 finally 里调**，否则长生命周期 signal 上泄漏监听器）。两条铁律：**取消 ≠ 失败**——被取消的链路一律直抛 AbortError、由发起方静默收口，不走任何既有降级/提示分支（不降级文本检索、不落失败 UI、不回退默认序）；**取消与超时分开报**——内层 controller 被两者共用，实现须以自己的 `timedOut` 标记区分（只看 `controller.signal.aborted` 会把取消误报成「无响应」）。首个消费方 = 第二大脑参考面板/移动面板「只查最新」（新查询发起或面板关闭即真中断上一轮，列表态恒归最新一轮）。
_Avoid_: 软取消（只丢结果不砍请求）、把取消当失败提示、abort 判定散落各模块各写一份
