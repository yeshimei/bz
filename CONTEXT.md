...
**每日简报 (Daily Brief)**: 剪藏本第三个内容源（ADR-0119/issue 263）——盯住「每日简报名单」内 UP 主的新投稿，把视频内容（字幕优先、无字幕走音频转写）交 AI 总结成**要点列表（按小节分组，每条要点一句话）**，目录内按天分节供扫读。与聚合讯流**分开成目**；条目参与剪藏本状态机（unread/reading/read/saved），右栏阅读区 = 要点分节（小节 + bullets）+「打开原视频」与视频元信息 + 可展开转录稿（需要细节时看折叠区全文）；保存到剪藏本写**专属目录**（默认 `归档/每日简报/`），不与 `归档/网页剪藏/` 混。**自动产出不写 vault 笔记**，纯数据驱动 UI。_Avoid_: 聚合讯（旧称）、AI 视频总结（指既有 47 篇外部工具产物时）、一句话总结（2026-09-10 中途试行的旧形态）
