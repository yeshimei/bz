...
**Jev 决策通道 (Jev Decision Channel)**: 与「AIService / createAI」（**生成通道**）并存的**判定通道**（ADR-0173，2026-09-21）——TypeSafe AI 的 System One 模型 Jev 的接入封装：state + 类型化问题（Choice 从清单选一个 / Score 按量表打分 / Noul 判是或否）→ 类型化答案 + 校准概率，**不生成任何文本**，故不存在 JSON 解析失败。便于后来者检索——本术语在代码中的落点是 `src/core/jev.ts`，键为 `jev*`。**知识盒内容生成不接入**——`note-gen.ts` 的 `domain` 与 `title`/`summary` 是同一次流式调用的产物，拆开会让属性区「分析中…」失去增量语义（ADR-0173 §8）。优先级 **Jev 优先、失败即当次回落 LLM**（回落对用户不可见，降级不通知不落盘标记）；两个通道都不可用则入队重试，不引第三层兜底。**关联预演（录入面板「关联」行）与后台自动关联同通道、不拆参数**——`previewLinks` 与 `processNote` 一并走 Jev，仅预演的 `AbortSignal` 需透传（取消不算失败、不回落）。判定口径以 Jev 为准（关联变稀是接受的代价），**存量 related 不重跑**（有意接受的口径混血）。模型默认固定 `jev-1.13.0`，不用 `jev-latest`。配置独立一组落在设置面板 AI 分区（启用 / 端点 / 密钥 / 模型 / 超时 + **一个全局总开关**），接入点不设独立开关。
