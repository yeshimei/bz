...
**黑匣子 (Black Box)**: bz 的一个新域——长期累积「感触」的容器，AI 作为其意识体（伴侣向）。输入以平行新流（感触流）为主，未来可纳入次级内容（ADR-0014）；当前与闪念/剪藏/日记并列，不做元层聚合。**v3 起三类条目笔记化落盘**（`黑匣子/概念|摘抄|想法/*.md`，笔记即事实源，ADR-0015）；`CONFIG/STORAGE/blackbox.json`（v3）只存派生层（画像/事件/复盘/对话/词表）+ id→路径索引。录入走命令弹窗（`bz-blackbox-capture` 及 `bz-blackbox-capture-concept/-literature/-thought` 三直达命令），对话走中央弹窗（`bz-blackbox-open`），不新增常驻窄窗（避免与闪念窄窗冲突）。
_Avoid_: 盒子、灵魂容器

**来源笔记 (Source Note)**: 捕获概念/摘抄时选中文字所在的笔记——摘抄笔记经 `[[来源]]` 双链关联；捕获时把选区内原文替换为 `[[目标笔记|原文字]]`（显示不变，铁律 #1 的显式豁免，带四重守卫：frontmatter/代码块/数学块/已是链接的选区跳过）。
_Avoid_: 出处、原文笔记
...
**概念与实体 (Concept/Entity)**: 黑匣子的第一类输入——用户给出名词（如「提喻法」），AI 生成知识卡片（一句话或一段话说明它是什么），卡片间关联由 AI 自动构建，用户仅确认卡片是否合理、决定是否录入。**v3 落盘为 `黑匣子/概念/<概念名>.md`**（正文=定义，底部 `- 关联：[[…]]` 双链；纯知识卡片，无感触外壳）。构成意识体的知识背景。
_Avoid_: 术语卡、名词解释

**文献笔记 (Literature Note)**: 黑匣子的第二类输入——用户从别处摘抄的信息片段，带来源；录入前 AI 分析其中可提取的概念/实体，与已有概念建立联系，给出名词表由用户勾选关联。**v3 落盘为 `黑匣子/摘抄/<AI 标题>.md`**（正文=摘抄文本 + 底部 `来源：[[…]]`、`关联概念：[[…]]`；标题由 AI 生成，失败降级正文前 20 字）。
