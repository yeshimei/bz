...
### 黑匣子域（v3 笔记化，ADR-0015）

**黑匣子 (Black Box)**: bz 的一个新域——长期累积「感触」的容器，AI 作为其意识体（伴侣向）。输入以平行新流（感触流）为主，未来可纳入次级内容（ADR-0014）；当前与闪念/剪藏/日记并列，不做元层聚合。**v3 起三类条目笔记化落盘**（`黑匣子/概念|摘抄|想法/*.md`，笔记即事实源，ADR-0015）；`CONFIG/STORAGE/blackbox.json`（v3）只存派生层（画像/事件/复盘/对话/词表）+ id→路径索引。录入走命令弹窗（`bz-blackbox-capture` 及 `bz-blackbox-capture-concept/-literature/-thought` 三直达命令）+ **书内选区录入**（EPUB 阅读器选区工具栏，ADR-0016），对话走中央弹窗（`bz-blackbox-open`），不新增常驻窄窗（避免与闪念窄窗冲突）。
_Avoid_: 盒子、灵魂容器

**来源笔记 (Source Note)**: 捕获概念/摘抄时选中文字所在的**笔记形态来源**——摘抄笔记经 `[[来源]]` 双链关联；捕获时把选区内原文替换为 `[[目标笔记|原文字]]`（显示不变，铁律 #1 的显式豁免，带四重守卫：frontmatter/代码块/数学块/已是链接的选区跳过）。书内形态见「书内来源」。
_Avoid_: 出处、原文笔记

**书内来源 (In-book Source)**: 捕获概念/摘抄时选中文字所在的 **EPUB 阅读器选区**（ADR-0016）——来源形态为阅读器双链 `[[书路径#位置|书名]]`（`#` 后为 weave-cfi 定位符，指向书内原文位置）；摘抄存于 source 字段，概念存于 links 字段（**单值约定**，与摘抄对称）；黑匣子列表点击该来源经阅读器公开 API 跳回书内对应位置。来源笔记与书内来源并列：笔记选区录入 → 来源笔记，书内选区录入 → 书内来源，无选区 → 无来源。
_Avoid_: 出处链接、epub 引用
...
**情绪标签 (Emotion Tag)**: 感触条目上的多维情绪标注——20+ 固定词表，可多选（最多 3 个），不加强度；因数据格式稳定铁律，词表与维度在第一版即定全，后续只加语义不改字段。
_Avoid_: 心情、mood
...
**概念与实体 (Concept/Entity)**: 黑匣子的第一类输入——用户给出名词（如「提喻法」），AI 生成知识卡片（一句话或一段话说明它是什么），卡片间关联由 AI 自动构建，用户仅确认卡片是否合理、决定是否录入。**v3 落盘为 `黑匣子/概念/<概念名>.md`**（正文=定义，底部 `- 关联：[[…]]` 双链、其下 `来源：[[…]]` 行；纯知识卡片，无感触外壳）。**v3 起持有来源（单值，与文献笔记一致；笔记/书内两种形态）**。构成意识体的知识背景。
_Avoid_: 术语卡、名词解释

**文献笔记 (Literature Note)**: 黑匣子的第二类输入——用户从别处摘抄的信息片段，带来源（笔记/书内/URL 三形态）；录入前 AI 分析其中可提取的概念/实体，与已有概念建立联系，给出名词表由用户勾选关联。**v3 落盘为 `黑匣子/摘抄/<AI 标题>.md`**（正文=摘抄文本 + 底部 `来源：[[…]]`、`关联概念：[[…]]`；标题由 AI 生成，失败降级正文前 20 字）。**想法由用户手输**（ticket 50：不 AI 提炼，非空则另落一条「核心知识」笔记并 `来自：` 指向摘抄）；**摘抄不分类**，一律存 `黑匣子/摘抄/` 根目录（ticket 50）。
_Avoid_: 剪藏（剪藏本存整篇，文献笔记存精选片段 + 概念链接 + 个人感受）

**新概念流转 (New Concept Flow)**: 文献笔记录入时名词表勾选的「✦新概念」的处理流程（ticket 50）——摘抄存入**不落空定义概念笔记**，同弹窗依次流转概念录入（概念名可编辑、来源继承摘抄来源、AI 生成定义后确认）；全部完成后再把新建概念**回填进摘抄的关联概念**（terms）。中途跳过/关窗 = 该概念不创建、摘抄不加（已确认的仍回填）；确认时改名与既有概念同名 → 不新建、摘抄直接关联既有概念。
_Avoid_: 概念排队、二次录入

**核心知识 (Core Knowledge)**: 黑匣子的第三类输入——用户自己的想法，可能在录入文献笔记时带出，也可能独立录入。**v3 落盘为 `黑匣子/想法/<AI 标题>.md`**（标题由 AI 生成，失败降级正文前 20 字）。三类输入中的核心层，第一、二类（概念与实体、文献笔记）是它的知识补充与背景。
_Avoid_: 思考笔记、随笔

**知识背景 (Knowledge Background)**: 由概念与实体、文献笔记构成的支撑层——服务于核心知识（想法），也是意识体对话时可引用的知识面。

**人物画像 (Character Profile)**: 黑匣子中关于某个现实人物（前女友、家人、朋友）的派生档案——AI 从已有条目中自动提炼维护，用户可调整修改；与条目的「涉及的人」维度双向关联。构成意识体对用户人际世界的认知。含用户印象区（字段级锁，用户改过 AI 不再覆盖）与 AI 观察区（AI 持续写入，可采纳进印象区）；对话检索时以概要级（印象一句话 + 最近 3 个事件标题）进入包仔上下文。冷启动双路径：录入时可主动新建，不建则只存名字，AI 复盘时提示。
_Avoid_: 人设、名片、profile

**事件 (Event)**: 从三类条目中提炼出的「发生了什么」——有具体行动/变化/时刻；由 AI 全自动提炼整理，带置信度标记，用户可事后纠正（编辑/删除/合并，改过的 AI 不再碰）。单份存储，人物画像时间线与全局时间线均为其投影（按人/按时间）。
_Avoid_: 大事记、timeline

**推测事件 (Speculative Event)**: 置信度低的事件（意图、计划、梦境等非事实内容）——以推测样式入时间线（虚线 + ❓），用户可确认或忽略。

**AI 观察 (AI Observation)**: 人物画像中与用户印象区并存的 AI 视角区——AI 持续写入新认知（虚线框），用户认可可「采纳」进印象区（采纳后一并锁定）。

**名词表 (Term List)**: 录入文献笔记时 AI 提取并匹配出的候选概念列表，由用户勾选以建立条目与概念的关联。
