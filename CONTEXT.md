...
**文件引用同步 (File Reference Sync)**: 笔记 rename/delete 后自动维护「引用了该笔记的数据条目」的家族机制，共三员——memo.json（待办）、favorites.json、review.json：各数据属主订阅域事件总线通用通道 `'vault:md-*'` 就地更新自身字段（memo/favorites 的 linkedNote/notePath/标题，review 的计划内笔记路径）。todo/favorites 持本地纯函数私有副本（ADR-0048 自 ai-agent 拆回，语义逐行等价、勿跨域 import；ADR-0092 起 memo 一员由 todo 域承担）；review 一期已订同一通道。
_Avoid_: 引用同步单独成域（AI Agent 已解散）；跨域 import 他域 sync 副本 **ADR-0088 起 todo/favorites 两路无条件常驻**（原 aiAgentEnabled 门控删除，rename/delete 同步是数据完整性功能不设开关）；监听范围固定 SYNC_WATCHED_FOLDERS（core/settings-common，卡片盒+归档/网页剪藏）。
