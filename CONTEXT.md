...
**影视分析报告 (movie-report, 旧域，已退役删除 ADR-0090)**: 原 `src/movie-report/` 独立观影数据分析弹窗（ADR-0048 自 `src/movie/analysis.ts` 迁出）。ADR-0090 报告内嵌化：19 板块统计全部并入影院内嵌分析页（src/cinema/analysis.ts，`M.view='stat'`；类型分布环形图改水平条形行——「圆形统计被否」拍板），原 `bz-movie-report` 命令 id 退役换 `bz-cinema-analysis`（名称「影视分析报告」保留用户习惯），独立窗与 ESC 层 `movie-analysis` 随域删除；面板存续期间 vault 变更自动刷新分析页（cinema registerAutoRefresh 既有链路）。数据只读 `我的/影视/*.md` 由 cinema 同源承担，「片长」「季集」字段并入 CinemaItem（片长画像/追剧深度两板块的数据源）。
_Avoid_: 观影数据分析弹窗 / 独立报告窗（已随 ADR-0090 退役）；影视数据分析弹窗（更旧词条口径）；由影视.js 调用 / 共享 __MOVIE_FOLDER_PATH（已解除）
...
**移动端默认全屏 (Mobile Default Fullscreen)**: bz 的跨域设置（ticket 68，ADR-0019）——12 个有主窗口的域各一项布尔开关（键 `<域前缀>MobileDefaultFullscreen`，落 data.json），**仅移动端（`Platform.isMobile`）显示与生效**，桌面端不显示不受影响。语义：≤768px 时 **开=真全屏**（主窗口覆盖整个视口 100vw×100vh、去圆角、头部避让安全区、底部 env(safe-area-inset-bottom)，统一类 `.bz-win-mfs`），**关=常规卡**（95%/90vh 圆角卡）；只决定每次打开的**初始形态**，窗口内无手动切换按钮。多窗口域（影视主面板+影视分析（ADR-0090 起报告窗退役）、书库主面板+读书笔记+阅读报告（ADR-0091 起阅读报告随书架墙面板同控））一并对控制，筛选/批注等小弹窗不纳入。**聚合讯跟随剪藏本键、阅读报告跟随书库键（2026-08 用户拍板：两域不设独立开关、窗口无 ⚙️ 设置入口；ADR-0091 起独立阅读报告窗退役，该键仍由书架墙面板与读书笔记弹窗消费，报告随面板同控）**。默认值=行为保持（原移动端即全屏的域默认开——日记/归物本/剪藏本/密码本/收藏本/书库/影视/复习/保险箱；原居中卡的 3 域默认关——备忘录/番茄钟/文献盒）。做题家、入口页不设此开关（用户拍板）。
