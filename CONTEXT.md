...
**Jev 决策通道 (Jev Decision Channel)**: 与「AIService / createAI」（**生成通道**）并存的**判定通道**（ADR-0173，2026-09-21）——TypeSafe AI 的 System One 模型 Jev 的接入封装：state + 类型化问题（Choice 从清单选一个 / Score 按量表打分 / Noul 判是或否）→ 类型化答案 + 校准概率，**不生成任何文本**，故不存在 JSON 解析失败。便于后来者检索——本术语在代码中的落点是 `src/core/jev.ts`，键为 `jev*`。**知识盒内容生成不接入**——`note-gen.ts` 的 `domain` 与 `title`/`summary` 是同一次流式调用的产物，拆开会让属性区「分析中…」失去增量语义（ADR-0173 §8）。优先级 **Jev 优先、失败即当次回落 LLM**（回落对用户不可见，降级不通知不落盘标记）；两个通道都不可用则入队重试，不引第三层兜底。**关联预演（录入面板「关联」行）与后台自动关联同通道、不拆参数**——`previewLinks` 与 `processNote` 一并走 Jev，仅预演的 `AbortSignal` 需透传（取消不算失败、不回落）。判定口径以 Jev 为准（关联变稀是接受的代价），**存量 related 不重跑**（有意接受的口径混血）。模型默认固定 `jev-1.13.0`，不用 `jev-latest`。配置独立一组落在设置面板 AI 分区（启用 / 端点 / 密钥 / 模型 / 超时 + **一个全局总开关**），两处接入点不设独立开关。
_Avoid_: Jev 模型（泛指时）、AI 兜底通道（指备用 LLM 时）、Jev 备用通道

**受限分类 (Constrained Classification)**: 让模型从**受限候选清单**里选一个的**做法**（ADR-0174）——起因是 Jev 的 Choice 必须预先给定候选（不像 LLM 能自由写），而让模型自由写类目名会漂移（实测本库 30 个 `domain` 值里 `心理`/`心理学`、`生物`/`生物学`、`物理`/`物理学` 同义分裂）。**三条口径**：候选清单、问题文本、state 全部由调用方注入；「以上都不是」是**显式哨兵候选**（Jev 一定会选一个，这个出口必须显式给出）+ 置信度兜底；**不实现逃逸**——当前无消费者（领域维持 LLM 生成、影院词表闭合）。**它不是一个 core 层模块**：原设计的 `core/decide.ts` 通用原语因消费者只剩影院一处而**取消**（ADR-0174 §1），逻辑内联在 `src/cinema/`，出现第二个消费者再上提。已知调用方：**影院 typeTag 判定**（不允许逃逸，哨兵被选中或置信度不足则该片保持未分类、交人工）。**开放的词表容得下生成值，闭合的词表容不下。**
_Avoid_: 分类助手、通用 AI 工具（泛指时）、`core/decide.ts`（已取消，别再引用）
