...
**文献盒 (Literature Box Panel / Bilibili Task Queue)**: bili-downloader 域的面板+队列能力（ADR-0065 起建，ADR-0066 正名「文献盒」）——条目 = 视频链接 + 起止时间（HH:MM:SS(.S)，留空=整片）+ 状态（待处理/处理中/成功/失败）+ 可选备注 + notePath/videoPath，数据 `CONFIG/STORAGE/bili-tasks.json`；**移动端仅暂存录入**（bili 服务桌面端专属，无处理/下载按钮），**桌面端批量处理**：按列表顺序**串行逐部** spawn 工具无头批处理命令（`cli.js --batch <json>`）跑既有「快速流程」，产出「文献笔记」落「文献盒」；**行内详细进度（ADR-0066）**：✓步骤时间线 → 当前步骤 + 阶段百分比（`[bz-p] {"phase","pct"}` 行，下载/剪辑/转写/AI 分块，null=不确定）+ 进度条 + ⌛耗时，步骤文案同步落库（重开面板可读）；支持「中止整批」（杀死当前子进程、未开始项保持待处理）、失败项单独重试与「遇错即停」（设置项，失败后中断整批）；全部成功才视为批次完成。**ADR-0066 设置 ⚙️ 弹窗五项**：详细进度提示 / 保留视频原件（关=只出笔记不落视频）/ 下载清晰度（最高·1080P·720P，低档命中独立缓存键）/ 遇错即停 / 输出目录（空=跟随工具配置），键直绑落 data.json。**域事件分发（ADR-0066）**：`'bili-tasks'` 通道（added/edited/converted/failed），小橘只收 added+converted → 行为流（来源「文献盒」，不向量化不气泡）。命令 `bz-bili-tasks-open`（名称「文献盒」）。
