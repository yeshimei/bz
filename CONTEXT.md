...
**海报抓取 (Poster Fetch)**: **插件直调**（ADR-0113，PM2 守护进程退役——其 2026-09 静默僵死 4 天无报警是转向导火索）：插件内存队列 → 串行 spawn `douban-poster fetch <笔记>`（桌面端 child_process；15s 间隔防限流，3 分钟硬超时杀进程）→ spawn 退出后字段验证定完成/失败；**失败才聚合提示**（一条错误通知含片名清单），进度零通知、卡片海报遮罩 spinner 反馈。队列口径 = 缺海报或缺豆瓣链接（「海报」是抓取模式开关：有海报=只补信息不动海报，完成标志是「豆瓣链接」）；入队时机 = 面板打开扫描 + 新建落盘，同会话内存去重，每次 Obsidian 会话首轮打开补抓一次未齐条目。移动端无 child_process → 队列禁用，新片同步到 PC 后打开面板补抓。全局 npm 包保留（手动 CLI `douban-poster fetch` 仍可用）；脚本源码在 `tools/obsidian-douban-poster/`（npm 包 `@jwbz/obsidian-douban-poster`）。
