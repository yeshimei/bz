...
**文献笔记 (Literature Note)**: 黑匣子的第二类输入——用户从别处摘抄的信息片段，带来源（笔记/书内/URL 三形态）；录入前 AI 分析其中可提取的概念/实体，与已有概念建立联系，给出名词表由用户勾选关联。**v3 落盘为 `黑匣子/摘抄/<AI 标题>.md`**（正文=摘抄文本 + 底部 `来源：[[…]]`、`关联概念：[[…]]`；标题由 AI 生成，失败降级正文前 20 字）。
