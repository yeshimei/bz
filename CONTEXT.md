...
**日记条目 (DiaryEntry)**: 日记本中展示的最小单位，由 `# emoji序列 HH:mm` 标题 + 正文构成，属于某个日期文件。
_Avoid_: 日记、记录、post
...
**ADR-0119 扩职责**：守护兼作「每日简报」的**调度器**——在现有 30 分钟抓取轮里把 `briefUps` 名单 UP 一并抓（同一份动态请求，条目**不入 articles**），发现新 bvid 则 spawn bili-downloader 的 brief 模式（单轮上限 3 条）；**AI 总结与落盘仍归插件**（守住 ADR-0011「工具不调 AI」）。_Avoid_: 新闻抓取、新闻爬虫、news watcher 进程
...
**每日简报 (Daily Brief)**: 剪藏本第三个内容源（ADR-0119/issue 263，UI 精修见 issue 264）——盯住「每日简报名单」内 UP 主的新投稿，把视频内容（字幕优先、无字幕走音频转写）交 AI 总结成**要点列表（按小节分组，每条要点一句话）**，目录内按天分节供扫读。与聚合讯流**分开成目**；条目参与剪藏本状态机（unread/reading/read/saved），右栏阅读区 = 要点分节（小节 + bullets）+「打开原视频」与视频元信息。保存到剪藏本写**专属目录**（默认 `归档/每日简报/`），不与 `归档/网页剪藏/` 混。**自动产出不写 vault 笔记**，纯数据驱动 UI。**2026-09-10 用户拍板退役**：阅读面「完整转录稿」可展开段与目录行首「字幕/转写」小标均已移除（数据层 `src`/`transcriptPath` 字段保留，仅不上屏）；`readBriefTranscript()` 保留——`retryBrief()` 仍靠它判断缓存是否在。_Avoid_: 聚合讯（旧称）、AI 视频总结（指既有 47 篇外部工具产物时）、一句话总结（2026-09-10 中途试行的旧形态）

**每日简报名单 (Brief UP List)**: news.json `briefUps` 段（ADR-0119）——uid 数组，决定哪些 UP 走深度总结链路；首次启用回溯最近 10 条，此后只跑新 bvid，已总结的永久不重跑。插件侧写（复用「UP 主名单管理」弹窗的「每日简报」子列表 + 逐条开关），数据源守护只读用于调度；**与 `bilibiliUps` 互斥**（同一 UP 不同时出现在两个名单，同一视频不在两处重复出现）。_Avoid_: 深度总结名单、UP 主名单（指普通名单时）

**简报条目 (Brief Item)**: news.json `briefs` 段的一条记录，**一条视频一条**（不按天聚合）——要点正文（`body`，`## 小节` 分组 + `- ` 每条一句话）+ 原视频链接 + 视频元信息 + 处理状态。参与剪藏本状态机与保留策略（未读永不清理；已读/已保存骨架超 `newsRetentionUnsavedDays` 默认 30 天删除，与 articles 段同口径）；失败（风控 -352/412 / 转写失败 / 模型加载失败）以**可见错误条目**留档，含原因与重跑入口。_Avoid_: 每日资讯（旧称）、简报文章（指聚合讯条目时）

**转录稿 (Transcript)**: bili-downloader 转写产出的 UTF-8 全文——**不进 news.json**，落工具缓存（`cacheDir` + `cacheRetentionDays`，默认 7 天）自然过期；插件按需读取渲染「可展开转录稿」区，过期后该区隐藏。_Avoid_: 字幕（B 站侧产物，属「字幕优先」链路另一来源）、全文文稿
...
**海报抓取 (Poster Fetch)**: **插件直调**（ADR-0113，PM2 守护进程退役——其 2026-09 静默僵死 4 天无报警是转向导火索）：插件内存队列 → 串行 spawn `douban-poster fetch <笔记绝对路径>`（桌面端 child_process 调**系统 Node**——Electron runAsNode fuse 被禁不可用（issue 256）；15s 间隔防限流，3 分钟硬超时杀进程）→ spawn 退出后字段验证定完成/失败；**失败才聚合提示**（一条错误通知含片名清单），进度零通知、卡片海报遮罩 spinner 反馈（入队即渲染，完成后立即+延迟双重建渲染上卡）。队列口径 = 缺海报或缺豆瓣链接（「海报」是抓取模式开关：有海报=只补信息不动海报，完成标志是「豆瓣链接」）；入队时机 = 面板打开扫描 + 新建落盘，同会话内存去重，每次 Obsidian 会话首轮打开补抓一次未齐条目。移动端无 child_process → 队列禁用，新片同步到 PC 后打开面板补抓。全局 npm 包保留（手动 CLI `douban-poster fetch` 仍可用，2.3.0 起带补全分支）；脚本源码在 `tools/obsidian-douban-poster/`（npm 包 `@jwbz/obsidian-douban-poster`）。
_Avoid_: 抓海报、豆瓣补全、poster fetch；把「海报」当成抓取完成的标志——它是抓取模式开关（有海报=只补信息），完成标志是「豆瓣链接」；「豆瓣检查」字段（ADR-0111 触碰协议已退役，存量休眠无害）

**桌面端专属能力 (Desktop-only Capability)**: 依赖 Node.js 外部进程（child_process）、移动端（Capacitor）不可用的功能。门禁：`window.require('child_process')` 为 null 即非桌面端；移动端不注册事件监听，设置项置灰标注「仅桌面端可用」，不静默降级。（当前实例：文献盒批量处理、影院海报抓取直调 spawn 等外部工具调用）
...
**知识盒 (Knowledge Box, ADR-0112/issue 255)**: knowledge 域三部枢纽（ticket 136/ADR-0072 自 bili-downloader 迁出；ADR-0112 重构更名）——部壹·文献：文献目录词典列表+全文预览+提炼成卡+两种录入（文字·术语/视频·任务队列）；部贰·卡片：卡片目录只读扫描（领域读序 domain→category→未分类，存量零迁移）；部叁·主题：主题笔记仅展示（写作与检索归 Obsidian+第二大脑，关联机制探索中）。提炼成卡铁律=连一张旧卡+一句为什么，related 关联链（Obsidian 原生双链）+领域自动带、源文献自动互链；原领域筛选/搜索/双击/抽屉/面板设置钮按原型移除。——主面板像剪藏本列出「文献目录」文件夹里的文献笔记（.md，扫描 + metadataCache 解析 frontmatter，**不从数据文件派生**）：顶部**领域筛选**（各领域带数量标签）+ **类型过滤**（全部/视频/术语）+ 搜索，滚动触底懒加载，域事件四通道自动刷新，**双击打开** + 统一抽屉（打开/复制双链/复制原文链接/删除，删除视频笔记同步清理任务记录）。右上角三按钮：**文字录入**（术语生成流程，见「术语文献」）/ **视频录入**（视频转文献任务队列：添加/处理/历史；**移动端仅添加+历史**；无 ⚙️ 无 ⬇️，设置全并入主面板设置面板）/ **设置**。数据 `CONFIG/STORAGE/literature.json` 单一文件（视频任务结构沿用 bili-tasks.json，**术语不留任务**；旧文件不迁移不做兼容）。命令 `bz-knowledge-open` / `bz-knowledge-note-term`。批处理 = spawn `cli.js --batch`（**ticket 147 起 JSON 改 base64 `b64:` 前缀传输**——.cmd shim 需 shell:true，JSON 引号/空格会被 shell 对消，报「不是合法 JSON：Expected property name at position 1」），**AI（元数据+润色+type/domain）与笔记落盘在插件侧**（CLI 去 AI，产出转录临时文件）；压缩默认开（CRF 默认 23）；**视频录入批量按钮为单钮纯 emoji**（ticket 146 单钮态机 + ticket 148 去文字：空闲「▶️」/运行中「⏹」，终止与终止整批靠 title hover 区分，移动端整钮隐藏）。**options 里「留空=跟随工具配置」的键（pythonPath/outputDir/ffmpegPath/ffprobePath/whisperModel/cacheDir）留空时不下发**——空串会覆盖 rc/DEFAULTS 兜底致转写环节报「未配置 pythonPath」（ticket 149）。**Python 路径可填命令名 `python`（走系统 PATH）或绝对路径**（ticket 150：DEFAULTS 已通用化，Windows 可用 `where python` 查绝对路径；ENOENT 时引导填写方式）。断点续跑、行内进度、历史归档等语义自 ADR-0065/0066/0067/0070 承继（CLI 缓存仅机械产物）。
...
**文件引用同步 (File Reference Sync)**: 笔记 rename/delete 后自动维护「引用了该笔记的数据条目」的家族机制，共三员——memo.json（备忘录）、favorites.json、review.json：各数据属主订阅域事件总线通用通道 `'vault:md-*'` 就地更新自身字段（memo/favorites 的 linkedNote/notePath/标题，review 的计划内笔记路径）。memo/favorites 持本地纯函数私有副本（ADR-0048 自 ai-agent 拆回，语义逐行等价、勿跨域 import；ADR-0092 起 memo 一员由本域承担）；review 一期已订同一通道。
...
**加密笔记 (Encrypted Note)**: 移入保险库的整篇笔记，清单内一条记录（SafeNote，kind 缺省）。正文 100% 密文化，保险库面板「加密笔记」视图管理（预览/还原/删除）。
...
**备忘录面板皮肤 (Memo Skin, ADR-0095)**: 备忘录面板两选一外观（设置键 `memoSkin`：paper 纸感手账/editorial 编辑部，issue 210 四轮默认风格下线），备忘录设置「显示」组最顶部以 `choiceCards` 视觉卡片行选择（issue 210，预览+名称无编号无描述）。机制 = 面板根挂 `.bz-memo-skin-*` 作用域就近覆盖 `--bz-*` token + 少量结构覆盖；皮肤固定明度不随 Obsidian 明暗；二级弹窗不跟随；`applyMemoSkin` 支持已开面板热切换。组件库 `uiCardChoice`（.bz-cardpick）供其他域视觉选择复用。
...
**会话冻结序 (Frozen Order, ADR-0108)**: 剪藏本目录的一次会话语义——打开面板（装载）即拍目录快照，会话内任何状态流转（打开即已读、右键标读、批量标读、收藏）都不重排目录位置，条目只变视觉与计数；**唯一重排点 = 重新打开/装载面板**。桌面端持开捕获下有兜底：目录无未读时默认展开「已收」（空则「已读」）；移动端多章平铺不套此例外。仅 news 订阅源条目适用。_Avoid_: 让位、即时沉底（指返回目录即重排的旧行为）
_Avoid_: 移动端源横滑条（chip 导航）、移动端回退标准皮肤

**统一行操作 (Unified Item Actions)**: 跨域列表卡片统一手势组件（`src/core/item-actions.ts`）——列表**不注入任何常驻或 hover 图标排**；桌面端=**右键**弹跟手菜单（preventDefault 拦原生菜单，鼠标长按不触发），移动/触屏端=**长按**弹底部抽屉（遮罩+顶部条目信息+动作逐行）。能力：keepOpen（动作后抽屉保持+refreshItemSheet 原地重建动作与头部）、附属浮层（companion，抽屉之上的域内弹窗点击不误关抽屉）、危险项红色、强调色整行。动作项布局统一：图标左对齐 → 文案 → 小字右对齐。已接入域：备忘录、日记本、剪藏本（clipbook）、收藏本、归物本（含 4 状态流转+数据文件监听自动刷新）、书库（保留双击转跳书籍，md/EPUB 通用）、复习计划（保留双击打开笔记；开始复习难度弹窗为 companion）、保险库（双击预览保留）。
