...
**语义建议 (Semantic Suggestion, ADR-0138)**: AI 对卡片正文做词/句级切分，与全库笔记的整篇/标题/段落/图/视频做语义匹配，产出「锚点 → 目标」候选（附一句话理由）；以虚线呈现在挂载树上，可右键**固定**（正文锚点处写入 `[[wikilink]]`，转为用户双链）或**取消**（候选丢弃）。**不落 related**——Obsidian 图谱只反映用户双链。**建议按卡生成**（每张卡都以自身为主卡视角获取建议集，含指向其他卡乃至父卡的条目）；画布展示时按**当前主卡**做单向流过滤——指回祖先的建议隐藏，切主卡即切换可见的建议集。_Avoid_: 自动双链（旧 link agent 术语）、related 挂载
