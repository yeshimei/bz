...
**影院 (Cinema, ADR-0087 接管影视)**: `src/cinema/` 域，命令 `bz-cinema-open`/`bz-cinema-add`/`bz-cinema-analysis`（影视分析报告 = 覆盖影院面板的一层《观影分析》26 幕，ADR-0174 / ADR-0175；层框 = 面板矩形、点遮罩 / ESC 关闭、桌面无关闭按钮、移动端保留；ADR-0090 的内嵌分析页与 issue 405 的滚动放映室两稿均已作废删除）/`bz-cinema-random-pick`「随机抽一部」（想看池空回落全量）。目录 = cinemaFolderPath（显式配置）→ 回落「我的/影视」；frontmatter 契约（tags 类型 / 评分 -1 想看 0 在看 >0 已看 / 观影日期 / 影评 / 海报 / 豆瓣字段 / 片长 / 季集）与旧 movie 域及外部 douban-poster watcher 完全一致。**ADR-0087 承接**：找同类 AI（详情弹窗按钮 → 页内结果）、补发 `movie:` 域事件（created/status/rated/review/deleted，smartcat 行为流观察依赖，载荷对齐 MovieActionEvent 契约）。AI 页标题固定「AI 荐片」（找同类结果同页呈现，M.aiTitle 字段已随六域单源清理退役）。分析页统计卡「已放映」=「已看」条数的午夜场风味同义词，口径同一。设置收敛设置面板「影院」tab。**未打类型 tag 的条目自动归类**（`type-decide.ts`，issue 393 / ADR-0174；回落口径见 ADR-0181）——Jev choice 从闭合词表（`constants.ts` 的 TYPE_GROUPS）选一个，弃权（哨兵 / 置信度不足）即留空由用户手点；**Jev 不可用一律回落单次 LLM JSON**（闭集校验），两道都不可用不阻断解析（字段已到手）。**剧集按季合并（issue 376、387 / ADR-0168）**：设置面板「影院 → 显示 → 剧集按季合并」（`cinemaMergeSeasons`，**默认开**，即时生效）——同一部剧的各季在网格里合成一张卡（海报**左下角一排季圆点**：金实=已看 / 橙实=在看 / 空心=未看，一个圆点=一季，**不出注释文字**、不占卡片高度），鼠标移入圆点即**换脸预览该季**（海报 + 名字/meta/星级，离开复原；issue 377）。**特别篇并入**（387）：片名 = `<剧名>` + 分隔符 `[\s:：·\-—－]+` + 副标题的条目（电影版/特别篇/外传，如「神探夏洛克：可恶的新娘」）并入该剧合并卡，**只按前缀认**、**「之」不作分隔符**；不占季号不进圆点、不再单独出卡，评分与聚合状态含它们。合并卡：**左键**开各季明细弹窗（季在前、特别篇顺其后，无分隔标题，头部计数按并入条目类型写「共 2 季 · 2 部电影」）；**桌面右键 / 移动长按**出跟手菜单/底部抽屉，只有一条「查看全部」；弹窗内**每行**右键/长按出该行动作（先收弹窗再执行）。合并判定单源 = `src/cinema/seasons.ts`（名称剥掉「第X季 / Season N」后同名 + 剧集/动漫组 + 库内 ≥2 季；`S1`/`第N部` 不认），**纯渲染层分组、零存储零写盘**，关掉即逐季一卡；rail 与头行计数按**卡片数**，分析页仍按笔记条数。
...
**Jev 决策通道 (Jev Decision Channel)**: 与「AIService / createAI」（**生成通道**）并存的**判定通道**（ADR-0173，2026-09-21）——TypeSafe AI 的 System One 模型 Jev 的接入封装：state + 类型化问题（Choice 从清单选一个 / Score 按量表打分 / Noul 判是或否）→ 类型化答案 + 校准概率，**不生成任何文本**，故不存在 JSON 解析失败。便于后来者检索——本术语在代码中的落点是 `src/core/jev.ts`（通道）+ `src/core/jev-fallback.ts`（判定编排，ADR-0181），键为 `jev*`。**知识盒内容生成不接入**——`note-gen.ts` 的 `domain` 与 `title`/`summary` 是同一次流式调用的产物，拆开会让属性区「分析中…」失去增量语义（ADR-0173 §8）。优先级 **Jev 优先、凡不可用一律当次回落 LLM**（未启用/未配置 → 不造材料直接回落；请求失败 / 答案畸形 → 回落；回落对用户不可见，降级不通知不落盘标记）——编排单源 = `core/jev-fallback.ts` 的 `judgeOrFallback({request,parse,fallback,signal?,config?})`，**新接入点一律消费它**（ADR-0181：影院类型归类 `decideCinemaType` 即走此路，Jev 不可用回落单次 LLM JSON，回执按**闭集校验**顶替置信度门槛；ADR-0174 §3 的「影院不回落」已作废）。**Jev 正常返回但弃权（哨兵 / 置信度不足）是有效判定，不触发回落**。两通道都不可用按域降级（关联入队重试 / 影院分类留空由用户手点），不引第三层兜底。**关联预演（录入面板「关联」行）与后台自动关联同通道、不拆参数**——`previewLinks` 与 `processNote` 一并走 Jev，仅预演的 `AbortSignal` 需透传（取消不算失败、不回落；**预取消时连 state/材料都不造**）。判定口径以 Jev 为准（关联变稀是接受的代价），**存量 related 不重跑**（有意接受的口径混血；影院分类同性质——不落盘标记产出方，复盘要靠 `console.debug`）。模型默认固定 `jev-1.13.0`，不用 `jev-latest`。配置独立一组落在设置面板 AI 分区（启用 / 端点 / 密钥 / 模型 / 超时 + **一个全局总开关**），接入点不设独立开关。
