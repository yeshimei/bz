...
**分类 (Category)**: 书库 md 书目的题材分类——书目 frontmatter 的单值 `category` 字段（缺省展示「未分类」；EPUB 条目无分类通道恒未分类）。单层题材 12 类、一书一类（ADR-0098）：文学 / 推理·悬疑 / 科幻 / 奇幻 / 武侠 / 戏剧 / 历史 / 哲学 / 心理学 / 社科 / 科学 / 艺术与摄影；书脊墙分区装箱、分类标签行筛选、读书报告分类筛均消费此字段。
_Avoid_: 标签（tags 是插件认书通道、恒 `book`，不承载分类语义）、分类文件夹（`书库/` 目录按作者/系列组织，不按题材）
...
**书脊墙换血 (Spine Wall 1:1, ADR-0096)**: 书库面板 1:1 复刻原型 p4-full.html（issue 218）——木匾刊头 + 纸质统计标签行（点选筛选再点回全）+ 检索/三档排序工具行 + 分类分区动态装箱墙体（每排按墙宽实测塞满才换排，resize 防抖重排）+ 倒叠区；书脊高度=时长、厚度=字数开方（frontmatter wordCount/pages 新解析，EPUB 回退批注密度）、主/副题双列竖排（：拆分 + upright + 14→9px + 64px 上限）、已读「讫」印/在读抽带/未读倒扣灰。旧封面网格/左栏/统计卡/月柱/纪念日卡/筛选抽屉全退役；移动端同构缩距不换形；借书卡（纸卡+印章）融全部编辑能力零回退；报告视图仅命令 bz-reading-report-open 进入（墙面无入口）；十肤（issue 216）随迁为 --bsw-* 墙变量映射，默认雪松白；bookshelfGridColumns 键退役，bookshelfSortMode 存量值零感知映射。
