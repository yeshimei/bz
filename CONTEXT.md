...
**保存至文献 (Save to Literature)**: 聚合讯阅读器对 B站视频条目（platform='B站' 且 url 非空）的保存动作（ticket 134，ADR-0067）——底栏按钮替换「保存至剪藏」，点击打开文献盒主面板 + 添加转文献任务弹窗（预填视频链接/标题/UP主）；不落剪藏、不标已读、不发 'news' 域事件（不进小橘行为流），阅读器留在本篇未读；普通文章仍是「保存至剪藏」原行为（B站条目 url 异常缺失回退剪藏按钮）。B站条目「下一篇/完成阅读」同样不进行为流（部分推翻 ticket 123「跳过也发」，普通文章保留）。_Avoid_: 保存至剪藏（指 B站条目按钮时）、剪藏视频（无此动作）
...
**文献盒 (Literature Box Panel / Bilibili Task Queue)**: bili-downloader 域的面板+队列能力（ADR-0065 起建，ADR-0066 正名「文献盒」）——条目 = 视频链接 + 起止时间（HH:MM:SS(.S)，留空=整片）+ 状态（待处理/处理中/成功/失败）+ 可选备注 + 可选标题/UP主（ticket 134 起聚合讯「保存至文献」入口预填，仅任务元数据、队列行优先展示标题、不进转换流程）+ notePath/videoPath，数据 `CONFIG/STORAGE/bili-tasks.json`；**移动端仅暂存录入**（bili 服务桌面端专属，无处理/下载按钮），**桌面端批量处理**：按列表顺序**串行逐部** spawn 工具无头批处理命令（`cli.js --batch <json>`）跑既有「快速流程」，产出「文献笔记」落「文献盒」；**行内详细进度（ADR-0066）**：✓步骤时间线 → 当前步骤 + 阶段百分比（`[bz-p] {"phase","pct"}` 行，下载/剪辑/转写/AI 分块，null=不确定）+ 进度条 + ⌛耗时，步骤文案同步落库（重开面板可读）；支持「中止整批」（杀死当前子进程、未开始项保持待处理）、失败项单独重试与「遇错即停」（设置项，失败后中断整批）；全部成功才视为批次完成。**ADR-0066 设置 ⚙️ 弹窗五项**：详细进度提示 / 保留视频原件（关=只出笔记不落视频）/ 下载清晰度（最高·1080P·720P，低档命中独立缓存键）/ 遇错即停 / 输出目录（空=跟随工具配置），键直绑落 data.json。**域事件分发（ADR-0066）**：`'bili-tasks'` 通道（added/edited/converted/failed），小橘只收 added+converted → 行为流（来源「文献盒」，不向量化不气泡）。命令 `bz-bili-tasks-open`（名称「文献盒」）。
