...
**书脊墙换血 (Spine Wall 1:1, ADR-0096)**: 书库面板 1:1 复刻原型 p4-full.html（issue 218）——行内标题「书库」与纸质统计标签行同行居左（issue 225 木匾刊头退场；点选筛选再点回全）+ 检索/三档排序工具行 + 分类分区动态装箱墙体（每排按墙宽实测塞满才换排，resize 防抖重排）+ 倒叠区；书脊高度=时长、厚度=字数开方（frontmatter wordCount/pages 新解析，EPUB 回退批注密度）、主/副题双列竖排（：拆分 + upright + 14→9px + 64px 上限）、已读「讫」印/在读抽带/未读倒扣灰。旧封面网格/左栏/统计卡/月柱/纪念日卡/筛选抽屉全退役；移动端同构缩距不换形；借书卡（纸卡+印章）融全部编辑能力零回退；报告视图仅命令 bz-reading-report-open 进入（墙面无入口）；十肤（issue 216 选肤；issue 225 全量补全 = --bsw-* 墙变量映射〔补齐 dark〕+ 逐肤结构层〔墙纹理/书脊做法/书挡隔板/检索框/排序选中态/标签随肤，移植原型 build-themes.js〕+ 设置页 .bz-skinprev-bs-* 预览重画），默认雪松白；bookshelfGridColumns 键退役，bookshelfSortMode 存量值零感知映射。
