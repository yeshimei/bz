...
**每日简报 (Daily Brief)**: 剪藏本第三个内容源（ADR-0119/issue 263）——盯住「每日简报名单」内 UP 主的新投稿，把视频内容（字幕优先、无字幕走音频转写）交 AI 总结成**一句话总结**，目录内按天分节供扫读。与聚合讯流**分开成目**；条目参与剪藏本状态机（unread/reading/read/saved），右栏阅读区 = 一句话总结 +「打开原视频」与视频元信息 + 可展开转录稿（需要细节时看折叠区全文）；保存到剪藏本写**专属目录**（默认 `归档/每日简报/`），不与 `归档/网页剪藏/` 混。**自动产出不写 vault 笔记**，纯数据驱动 UI。_Avoid_: 聚合讯（旧称）、AI 视频总结（指既有 47 篇外部工具产物时）、视频要点、要点列表（2026-09-10 前的旧形态）
...
**简报条目 (Brief Item)**: news.json `briefs` 段的一条记录，**一条视频一条**（不按天聚合）——一句话总结正文（`body`）+ 原视频链接 + 视频元信息 + 处理状态。参与剪藏本状态机与保留策略（未读永不清理；已读/已保存骨架超 `newsRetentionUnsavedDays` 默认 30 天删除，与 articles 段同口径）；失败（风控 -352/412 / 转写失败 / 模型加载失败）以**可见错误条目**留档，含原因与重跑入口。_Avoid_: 每日资讯（旧称）、简报文章（指聚合讯条目时）
