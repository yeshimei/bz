...
**三层记忆流水线 (Three-tier Memory Pipeline)**: ticket 160（ADR-0075，用户拍板推翻 158 合并池派生视图）——单向数据流：行为流（append-only 原始录像）→ 日小结（**产出改 type=observation 写入记忆流**，source=digest，evidenceIds 溯源行为条目；〔今日小结〕前缀取消）→ 记忆流观察 → 反思（**只吃记忆流观察**，产出 insight）→ 周报（**只吃本周新增 insight**，剔除 superseded，不吃具体记忆；只吃本周窗——洞察是长期结论，吃全量则周报周周雷同）。每层只吃下一层、不跨层取数；日小结产出 observation 化是管道接通的关键（insight 被反思防自指闸挡在证据池外，行为流不含小橘自身产出故无自指风险）。反思触发双闸「≥间隔 且 新素材≥阈值」，新素材 = max(pendingSinceReflect 计数, created 扫描)——计数覆盖记忆目录回填旧日期的入库与重启后增量，扫描覆盖重启恢复；158 的行为流直触发兜底与 behaviorToObservations 派生视图整体退役。反思证据池对带 ref 条目（记忆目录引用）经 refResolver 当场贴「原文摘录」（smartcatRefExcerptLimit 字，0=不附；读失败回退路径，失效自愈归记忆目录同步）。巩固参数 11 项全量上 ⚙️ 设置弹窗「记忆巩固」组（smartcatReflectIntervalHours/ReflectMinNew/ReflectEvidenceWindow/ReflectEvidenceTop/InsightCount、smartcatDigestIntervalHours/DigestMinNew/DigestMaxEvidence/DigestCount、smartcatWeeklyMinInsights、smartcatRefExcerptLimit；getConsolidationConfig 统一读取，MEMORY_CONFIG 为缺省，GA 检索权重不暴露）。断粮语义：AI 未配置 → 日小结不产出 → 下游安静等待不空转，行为流照常录像，AI 恢复后日小结自动补课。_Avoid_: 行为流直进反思证据（158 合并池已退役）、周报吃具体记忆/观察、日小结产出 insight、反思 OR 触发快车道（≥20 条）
